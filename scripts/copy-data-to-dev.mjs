...
export function getProductionUserDataPath({
  ...
  if (env.OCTOPUS_STUDIO_PROD_USER_DATA_DIR) {
    return path.resolve(env.OCTOPUS_STUDIO_PROD_USER_DATA_DIR);
  }
  ...
  if (platform === "win32") {
    if (!env.APPDATA) {
      throw new Error(
        "APPDATA is not set; cannot locate OctopusStudio's production data.",
      );
    }
    return path.join(env.APPDATA, "octopus-studio");
  }
...
export function getProcessesUsingDataDirectories(
  ...
  if (platform === "win32") {
    try {
      const output = runSync(
        "tasklist",
        ["/FI", "IMAGENAME eq octopusStudio.exe", "/FO", "CSV", "/NH"],
        {
          encoding: "utf8",
      ...
      return output
        .split(/\r?\n/)
        .map((line) => line.match(/^"octopus-studio\.exe","(\d+)"/i)?.[1])
        .filter(Boolean);
    } catch (error) {
      throw new Error(
        "Could not check whether OctopusStudio is running. Close OctopusStudio and try again.",
        { cause: error },
      );
    }
  }
  ...
  try {
    ...
    throw new Error(
      "Could not check whether OctopusStudio is running. Install lsof or close OctopusStudio and try again.",
      { cause: error },
    );
  }
}
...
export function copyProductionDataToDev({
  ...
  if (activePids.length > 0) {
    throw new Error(
      `OctopusStudio is using the production or development data (PID${activePids.length === 1 ? "" : "s"} ${activePids.join(", ")}). Close all OctopusStudio instances and try again.`,
    );
  }
...
function main() {
  ...
  console.log(
    `Copied OctopusStudio production data from:\n  ${source}\nto:\n  ${destination}`,
  );
}
