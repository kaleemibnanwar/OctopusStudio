...
async function verifyReleaseAssets() {
  try {
    ...
    const owner = "octopusStudio-sh";
    const repo = "octopus-studio";
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "octopusStudio-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `octopusStudio-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `octopusStudio-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `octopusStudio-${version}.Setup.exe`,
      `octopusStudio-darwin-arm64-${version}.zip`,
      `octopusStudio-darwin-x64-${version}.zip`,
      `octopus_studio_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      `octopus_studio_${version}_x86_64.AppImage`,
      "RELEASES",
