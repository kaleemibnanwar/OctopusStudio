...
1. `octopus-studio-{version}-1.x86_64.rpm` (Linux RPM)
2. `octopus-studio-{version}-full.nupkg` (Windows NuGet package)
3. `octopus-studio-{version}.Setup.exe` (Windows installer)
4. `octopus-studio-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `octopus-studio-darwin-x64-{version}.zip` (macOS Intel)
6. `octopus-studio_{version}_amd64.deb` (Linux DEB)
