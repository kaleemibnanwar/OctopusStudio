...
response from `<octopus-studio-write>` XML to a local-Agent fixture that invokes
...
production code must read OctopusStudio Engine/Gateway URLs at call time. If a test still
logs `POST https://engine.octopus-studio.sh/v1/... 401 (Unauthorized)`, search for
module-scope `OCTOPUS_STUDIO_ENGINE_URL` constants and switch those call sites to
`getOctopusStudioEngineBaseUrl()`.
...
- `src/testing/hybrid.setup.ts` caps electron-log's console transport at
  ...
  `OCTOPUS_STUDIO_TEST_LOG_LEVEL=debug` to see info/debug logs while debugging a test.
...
`dispose()` with `ENOTEMPTY` for a `octopus-studio-chat-flow-*` temp directory, look for a
