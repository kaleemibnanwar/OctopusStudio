...
**Custom-protocol debugging:** Before using `git bisect` on a `octopus-studio://` flow, quit every dev and packaged OctopusStudio instance and verify which build owns the protocol registration. macOS may route the link to a different running/registered build, producing a convincing but false good/bad result.
...
- For **non-bug** failures (validation, not found, auth, user refusal, etc.), prefer `OctopusStudioError` with the right `OctopusStudioErrorKind` so PostHog does not flood with `$exception` events — see [rules/octopus-studio-errors.md](octopus-studio-errors.md).
...
Automated `pnpm add` commands that run in an app root with a generated `pnpm-workspace.yaml` must pass `--ignore-workspace-root-check`. Otherwise older pnpm versions can fail with `ERR_PNPM_ADDING_TO_ROOT` even though OctopusStudio intentionally installs into that app root.
