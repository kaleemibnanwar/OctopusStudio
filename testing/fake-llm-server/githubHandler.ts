...
let mockReposRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), "octopusStudio-git-mock-"),
);
  ...
  if (repoName) {
    ...
    if (!fs.existsSync(bareRepoPath)) {
      ...
      try {
        ...
        // must start out empty: the very first push from OctopusStudio is a fresh,
        ...
        if (repoName === "existing-vite-app") {
          const tmpClone = fs.mkdtempSync(
            path.join(os.tmpdir(), "octopusStudio-git-clone-"),
          );
          try {
            ...
            execSync(
              `git -c user.name=octopusStudio -c user.email=octopusStudio@example.com commit -m "initial commit"`,
