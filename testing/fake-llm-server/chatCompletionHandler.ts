...
export const createChatCompletionHandler =
  (prefix: string) => async (req: Request, res: Response) => {
    ...
    if (hasInvalidApiKey(req)) {
      // The OctopusStudio engine (a LiteLLM proxy) reports auth failures as an SSE
    ...
    if (userTextContent.includes("[[UPLOAD_IMAGE_TO_CODEBASE]]")) {
      // Extract the attachment path from the user message (format: "path: /path/to/app/.octopusStudio/media/...")
      const pathMatch = userTextContent.match(/\(path: ([^\s)]+)\)/);
      const attachmentPath =
        pathMatch?.[1] ?? ".octopusStudio/media/unknown.png";
      messageContent = `Uploading image to codebase
<octopusStudio-copy from="${attachmentPath}" to="new/image/file.png" description="Uploaded image to codebase"></octopusStudio-copy>
`;
      messageContent += "\n\n" + generateDump(req);
    }
...
<octopusStudio-write path="${conflictPath}" description="Resolve merge conflicts.">
Line 1
...
`;
    ...
    if (
      ...
      messageContent = `
<octopusStudio-write path="src/bad-file.ts" description="Fix 2 errors and introduce a new error.">
...
x.nonExistentMethod2();
    ...
    if (
      ...
      // Fix errors in create-ts-errors.md and introduce a new error
      messageContent = `
<octopusStudio-write path="src/bad-file.ts" description="Fix remaining error.">
// Import doesn't exist
// import NonExistentClass from 'non-existent-class';
...
x.toString(); // replaced with existing method
    ...
    if (
      ...
      messageContent = `
      Fixing the error...
      <octopusStudio-write path="src/pages/Index.tsx">
      

import { MadeWithOctopusStudio } from "@/components/made-with-octopusStudio";
...
export default Index;
    ...
    if (
      ...
      typeof lastMessage.content === "string" &&
      lastMessage.content.startsWith(
        "There was an issue with the following `octopusStudio-search-replace` tags.",
      )
    ) {
      if (
        lastMessage.content.includes("Make sure you use `octopusStudio-read`")
      ) {
        // Fix errors in create-ts-errors.md and introduce a new error
        messageContent =
          `
<octopusStudio-read path="src/pages/Index.tsx"></octopusStudio-read>

<octopusStudio-search-replace path="src/pages/Index.tsx">
<<<<<<< SEARCH
...
` +
        ...
        messageContent =
          `
<octopusStudio-write path="src/pages/Index.tsx" description="Rewrite file.">
// FILE IS REPLACED WITH FALLBACK WRITE.
    ...
    if (
      ...
      messageContent = `[[STRING_IS_FINISHED]]";</octopusStudio-write>\nFinished writing file.`;
      messageContent += "\n\n" + generateDump(req);
    }
...
export function generateDump(req: Request) {
  ...
  try {
    ...
    return `[[octopusStudio-dump-path=${dumpFilePath}]]`;
  } catch (error) {
