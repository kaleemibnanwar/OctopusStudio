...
const VERIFY_REQUEST_RE =
  /^I approved the assertions\. OctopusStudio generated (\S+) from my recording\.\s*$/m;
...
const APPROVED_RESULT_RE =
  /^The user approved the plan\. OctopusStudio generated (\S+) from the recording/m;
...
export function matchAssertionsAgentTurn(
  ...
  // out its timeout on a missing `octopusStudio-test-assertions-card` with nothing
  ...
  return {
    ...
    args: {
      ...
      recordingId,
      // The model names the test. OctopusStudio only uses this when the user left the
