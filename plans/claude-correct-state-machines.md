...
- `notifyStreamFinished` computed outcome from a `wasCancelled` flag, so
  ...
  reports).
- Classified `OctopusStudioError`s rewritten to `Auth` by broad catches;
...
const waiter = waiters.register({
  ...
  onDisposed: "reject", // classified OctopusStudioError, kind: Disposed
});
...
- Authorization and admission hooks return a discriminated
  ...
  (#4105). Broad catches at the transport preserve `OctopusStudioErrorKind`
