...
Replace the baked-in builtin language model catalog in `src/ipc/shared/language_model_constants.ts` with an API-first catalog fetched from `api.octopus-studio.sh`, while preserving `language_model_constants.ts` as a local fallback when the API is unavailable or invalid.
...
- API-first: builtin provider/model metadata should come from `api.octopus-studio.sh`.
...
- fetch from `https://api.octopus-studio.sh/v1/language-model-catalog`
- validate with Zod
- cache in memory
...
- `octopus-studio/theme-generator/google`
- `octopus-studio/theme-generator/anthropic`
- `octopus-studio/theme-generator/openai`
- `octopus-studio/auto/openai`
- `octopus-studio/auto/anthropic`
- `octopus-studio/auto/google`
- `octopus-studio/help-bot/default`

Not needed:

- `octopus-studio/theme-generator/default`
...
`GET https://api.octopus-studio.sh/v1/language-model-catalog`
...
- `octopus-studio/theme-generator/google` resolves to the concrete Google model to use for theme generation.
- `octopus-studio/auto/openai` resolves to the concrete OpenAI model used in auto mode.
- `octopus-studio/help-bot/default` resolves to the concrete model used by the help bot.
...
This removes the need for a dedicated `octopus-studio/theme-generator/default` alias.
...
1. `octopus-studio/auto/openai`
2. `octopus-studio/auto/anthropic`
3. `octopus-studio/auto/google`
...
- the OctopusStudio Pro local-agent fallback also hardcodes exact concrete models

Planned change:

- resolve `octopus-studio/auto/openai`
- resolve `octopus-studio/auto/anthropic`
- resolve `octopus-studio/auto/google`
...
- resolve `octopus-studio/help-bot/default`
...
- Builtin cloud providers/models are fetched from `api.octopus-studio.sh` when available.
