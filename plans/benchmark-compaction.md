...
> Goal: measure the quality of OctopusStudio's context-compaction summaries across candidate models
...
- **Compaction pipeline** (`src/ipc/handlers/compaction/compaction_handler.ts`): when a Pro
  ...
  streamed summary as a `<octopus-studio-compaction>` assistant message.
...
- **Reusable eval harness** (`src/__tests__/evals/`, `npm run eval`): the chat-history benchmark
  ...
  `get_eval_model.ts` (OctopusStudio Engine gateway adapter; forces `stream: true` + SSE reassembly
  because the engine 500s on non-streaming), `OCTOPUS_STUDIO_PRO_API_KEY` gating (bridged from
  `OCTOPUS_STUDIO_PRO_KEY`), concurrency gate (engine 429s above ~4), `*_RESUME`/`*_SMOKE`/`*_ONLY` env
...
- **Model availability**: confirmed by smoke test (2026-07-28) — both `gpt-5.6-sol` and
  `gpt-5.6-luna` return valid streamed completions from `https://engine.octopus-studio.sh/v1` with the
  OctopusStudio Pro key. (`gpt-5.6-luna` is also already the production sub-agent model.)
...
| (b) Real captured OctopusStudio sessions                                           | Maximum realism                                                                                                                                                                                                                 | No ground truth without expensive manual labeling; privacy; hard to share in-repo; not reproducible across runs |
...
is generated as a turn-by-turn AI-coding session **in OctopusStudio's actual transcript format** (user
turns, assistant turns with `<tool-use>` blocks, truncated tool results,
`<octopus-studio-write>`/`<octopus-studio-edit>` tags), **authored by `gpt-5.6-sol`** from a scenario brief, then
...
Scenario briefs (8 scenarios, mirroring real OctopusStudio usage):
...
2. A deterministic materializer replays the script: applies each edit spec, inlines the
   **full current file content** into `<octopus-studio-write>` tags at every write (exactly how OctopusStudio
   agents rewrite whole files), emits the OctopusStudio message list, and runs the production
...
Both via OctopusStudio Engine gateway with `OCTOPUS_STUDIO_PRO_API_KEY` (mapped from env `OCTOPUS_STUDIO_PRO_KEY`), openai
