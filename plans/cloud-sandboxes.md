...
- OctopusStudio Engine endpoints for sandbox provisioning (authenticates Pro user, manages Vercel credentials)
- Auto-hibernate sandboxes after 15 minutes of inactivity
- Maximum 1 concurrent cloud sandbox per user (v1)
- Stale sandbox reconciliation on OctopusStudio launch (clean up orphans from previous sessions)
...
1. **As a new OctopusStudio user**, I want to preview my AI-generated app without installing Node.js, so that I can start building immediately after downloading OctopusStudio.
2. **As a OctopusStudio user on a slow machine**, I want to offload app execution to the cloud, so that my laptop doesn't slow down while building.
3. **As a OctopusStudio Pro user**, I want to switch between local and cloud runtime with one click in Settings, so that I can choose the mode that works best for my situation.
...
2. **Selection**: User selects "Cloud Sandbox (Pro)" from the dropdown.
   - If **not Pro**: Inline upgrade prompt ("Cloud sandboxes are a OctopusStudio Pro feature. [Upgrade to Pro]"). No further action possible.
...
4. **Provisioning**: System creates a Vercel sandbox via OctopusStudio Engine. Loading state: "Starting cloud sandbox..." with a simple spinner. If provisioning takes >5 seconds, show a multi-step stepper: "Creating sandbox..." -> "Installing dependencies..." -> "Starting server..."
...
- **Error - Auth/Backend**: "OctopusStudio cloud services are temporarily unavailable. Switch to local mode to keep building. [Switch to Local]"
...
executeApp() decision point (app_handlers.ts)
    ...
    └── "cloud"  → executeAppInCloud() [NEW]
                ...
                Local Proxy Server (start_proxy_server.ts)
                      |  (injects OctopusStudio scripts, serves to iframe)
                      v
...
- **`src/ipc/utils/start_proxy_server.ts`** — Support proxying to remote HTTPS URLs (currently targets localhost only). Handle SSL, inject OctopusStudio scripts into response HTML.
- **`src/components/RuntimeModeSelector.tsx`** — Add "Cloud Sandbox (Pro)" option. Pro gate with upgrade prompt for free users.
- **`src/hooks/useRunApp.ts`** — Handle cloud preview URL delivery. Emit synthetic `app:output` with `[octopus-studio-proxy-server]` format for consistency.
...
- **New: `src/ipc/utils/cloud_sandbox_provider.ts`** — Provider interface and Vercel implementation:
  ...
  ```
- **New: OctopusStudio Engine endpoints** — Sandbox CRUD (create, destroy, status). Authenticates Pro subscription, manages Vercel credentials server-side.

### Data Model Changes

**MVP: No database schema changes.** Sandbox state is stored in-memory in the `runningApps` Map. Sandboxes are ephemeral and don't survive OctopusStudio restarts. On restart, the user simply re-runs the app (creates a new sandbox).
...
**New OctopusStudio Engine API endpoints:**
...
### Phase 2: OctopusStudio Backend Endpoints

- [ ] Determine if existing OctopusStudio Pro backend can be extended (or if new infrastructure is needed)
...
- [ ] Implement Vercel Sandbox SDK provider against the OctopusStudio Engine endpoints
...
- [ ] Implement graceful sandbox teardown on OctopusStudio quit (iterate `runningApps`, destroy cloud entries)
- [ ] Implement stale sandbox reconciliation on OctopusStudio startup
...
| ------------------------------------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------------------------------------------------------------------- |
| OctopusStudio Engine dependency — if Engine is down, cloud sandboxes are broken | Medium     | High   | Monitoring, clear error messages ("OctopusStudio cloud services temporarily unavailable"), fallback to local mode suggestion |
| Orphaned sandboxes consuming OctopusStudio's cloud resources                    | High       | High   | Auto-hibernate after 15 min, max 1 concurrent sandbox, startup reconciliation, graceful teardown on quit                     |
...
- **OctopusStudio Engine integration**: Sandbox management will be implemented in OctopusStudio Engine (the existing backend service). Specific endpoint design and Vercel SDK integration details need to be worked out.
...
| **OctopusStudio Pro feature** (OctopusStudio manages infrastructure) | Simplifies UX (no Vercel account setup needed), controls costs, creates Pro upsell opportunity. Requires OctopusStudio Engine for sandbox management.                                                   |
...
| **Auto-hibernate after 15 min**                                      | Prevents orphaned sandboxes from running up OctopusStudio's cloud costs. Balance between convenience and cost control.                                                                                  |
