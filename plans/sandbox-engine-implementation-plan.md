...
This document scopes the OctopusStudio Engine work needed to support the new cloud sandbox runtime mode in the desktop app.

The desktop app now has a client-side cloud execution path:

- `runtimeMode2: "cloud"`
- sandbox provisioning via OctopusStudio Engine
- remote preview proxying through the local OctopusStudio proxy
...
- Provide a stable OctopusStudio Engine API for cloud sandbox creation and teardown.
- Keep provider-specific details out of the desktop app.
- Enforce OctopusStudio Pro access and usage limits server-side.
- Preserve OctopusStudio’s current preview model:
...
The desktop app currently expects these OctopusStudio Engine endpoints under `OCTOPUS_STUDIO_ENGINE_URL`:
...
Even with one provider, keep this boundary. It aligns with OctopusStudio’s backend-flexible principle and avoids leaking Vercel specifics into route handlers.
...
- bearer auth using OctopusStudio Pro credentials
...
Turn on for internal users first, then a small OctopusStudio Pro cohort.
