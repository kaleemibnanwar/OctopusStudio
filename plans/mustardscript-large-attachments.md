...
- **On-disk attachments (A, local-agent only).** When the turn is handled by `src/pro/main/ipc/handlers/local_agent/local_agent_handler.ts`, every user attachment (text and binary) is copied to `.octopus-studio/media/<sha256>.<ext>` at send time. No size threshold — uniform rule. Text attachments are no longer inlined in this path.
...
- **Output-cap split.** Tool result returns `{ value (≤64KB for LLM), truncated, fullOutputPath?, executionMs, instructionsUsed, heapBytesUsed }`. Outputs larger than 64KB are additionally written to `.octopus-studio/media/script-output-<hash>.txt` and the path is surfaced to both the LLM and the UI — the user-visible card can load the full result (up to ~1MB virtualized).
...
- **First-run education.** One-time, dismissible _inline_ info strip anchored to the **first Script card a user ever sees** (not install-time, not a modal). Copy: _"OctopusStudio just ran a small script to read your file. You'll see each one here. Not into this? Turn it off in Settings → Chat → Scripts."_ Dismissed forever after one click. Plus a one-time composer-level tip on the user's first local-agent attachment: _"Attachments stay on disk — OctopusStudio reads what it needs when you send."_
- **Transparency UI.** `ScriptCard` component (mustard-amber accent), label **"Script"** (no "sandbox"), reuses `OctopusStudioCard` + `OctopusStudioMcpToolCall` expand/collapse. Collapsed by default on success, auto-expanded on error. Header auto-populates from the tool call's `description` field (_"Read last 500 lines of server.log"_), falling back to _"Ran a script on `server.log`"_. Overflow menu on every card: _Re-run · Copy script · Copy output · Manage scripts in Settings_. Truncated outputs show _"LLM saw X of Y"_ badge.
...
- **Lifecycle.** Reuse `cleanupOldMediaFiles()` in `src/main.ts` for `.octopus-studio/media/` attachments (including `script-output-*.txt`). `.octopus-studio` is already added to `.gitignore` via `ensureOctopusStudioGitignored()`.
- **Power-user settings surface.** _Open `.octopus-studio/media/`_ button (using the literal path, not a euphemistic label), timeout ceiling configuration (2s default, up to 10s), consent toggle (always-allow ↔ ask ↔ never).
...
- As a default-chat user, I want existing attachment behavior to remain stable — if I am not using the local-agent path, OctopusStudio still inlines attachments into my message and does not show script/tool UI.
- As a power user, I want the "Open `.octopus-studio/media/`" settings button so I can inspect or share the raw files directly.
...
3. In the main process, because this turn is handled by `local_agent_handler.ts`, the file is copied to `.octopus-studio/media/<sha256>.log`. The outgoing user message gains an attachment-info `TextPart`:
   ```
...
6. If this is the user's **very first Script card ever**, a small dismissible strip sits above it for onboarding: _"OctopusStudio just ran a small script to read your file. You'll see each one here. Not into this? Turn it off in Settings → Chat → Scripts."_ **[Got it]** **[Settings]**
...
- **Attachment chip (local-agent):** uniform design across all types; no badge, no size split. Hover tooltip: _"Stored at `.octopus-studio/media/server.log`. OctopusStudio reads what it needs."_ Default chat keeps existing inline-attachment semantics.
...
- **Script card — truncated output:** _"LLM saw 42KB of 850KB — [Open full output]"_ linking to the side pane backed by `.octopus-studio/media/script-output-*.txt`.
- **Script card — error:** auto-expanded, red accent, one-line error + "Re-run" + "Retry with guidance" buttons.
- **Script card — empty result:** neutral accent, _"Script returned empty — OctopusStudio will try again"_ (softer than a dead end; common now that small files also use scripts).
...
- **Settings → Chat → Scripts:** script consent toggle (always-allow | ask | never), timeout ceiling slider (2s–10s), button _Open `.octopus-studio/media/`_.

### Interaction Details

- **Collapsed-by-default on success**, auto-expanded on error — progressive disclosure.
- **Keyboard:** card is a `<button>` with `aria-expanded`; Enter/Space toggles. Focus ring matches existing `OctopusStudioCard`.
...
- `src/pro/main/ipc/handlers/local_agent/local_agent_handler.ts` — switch local-agent attachment handling to `.octopus-studio/media/` references and the attachment-info user-message block.
...
- `src/components/chat/ScriptCard.tsx` — new component (reuses `OctopusStudioCard` + `OctopusStudioMcpToolCall` patterns), label "Script", overflow menu, stats strip.
...
- `src/pages/settings/*` — Settings → Chat → Scripts section with consent toggle, timeout ceiling, "Open `.octopus-studio/media/`" button.
...
- **On-disk:** `.octopus-studio/media/` continues to hold attachment files; adds `.octopus-studio/media/script-output-<hash>.txt` for oversized script returns. `.octopus-studio/` already in gitignore.
...
// Output (stringified JSON as tool result)
{
  ...
  truncated: boolean;
  fullOutputPath?: string;   // `.octopus-studio/media/script-output-<hash>.txt` if truncated
  executionMs: number;
...
- [ ] Install `mustardscript` in OctopusStudio; verify optional binding downloads correctly on mac-arm64, mac-x64, linux-x64, win-x64.
...
- [ ] Oversized-output spill to `.octopus-studio/media/script-output-<hash>.txt`, with path surfaced in tool result.
...
- [ ] Managed-model cost delta forecast (if OctopusStudio subsidizes any model calls).
...
- [ ] Release notes: "Large files now work in agent mode — upload anything, OctopusStudio reads just what it needs." Explicitly call out: legacy chats retain inline attachments; new local-agent uploads are on-disk; default chat still inlines attachments.
- [ ] Flag removal.
...
| Users surprised by MustardScript v0.1.1 alpha status / maintainership                                                                 | Low        | Med    | Pin exact version; add a OctopusStudio-CI canary that re-runs MustardScript's own tests on each bump                                                                                                                                                          |
...
| `.octopus-studio/media/` grows unbounded across sessions                                                                              | Med        | Low    | Reuse `cleanupOldMediaFiles()`; Settings "Manage attachments" roadmapped for follow-up                                                                                                                                                                        |
...
- **Keep `.octopus-studio/media/` on disk; alias as `attachments:` in local-agent user-message attachment info only** (user). Trade-off: filesystem name and LLM-facing name diverge. Gain: zero rename churn across 15+ files. UI copy uses whichever reads naturally; "Open `.octopus-studio/media/`" button uses the literal path so power users see the transition consistently.
...
- **"Open `.octopus-studio/media/`" button uses the literal path** (UX). Prevents confusion at the point where the naming split does surface.
