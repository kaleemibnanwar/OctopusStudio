...
- **Four missed preview-error writers** in `PreviewIframe.tsx` (`:415-426`
  cloud-sandbox errors with source `octopus-studio-app`, `:438-445` cloud sync errors,
  `:449-451` sync-recovery clear, `:1501` dismiss-any). `octopus-studio-app` errors
...
{type:"idle"})` (hook returns `StreamState | undefined` — every
  ...
  - `PromoMessage.tsx:155`, `OctopusStudioOutput.tsx:28`,
    `OctopusStudioMarkdownParser.tsx:132` → same hook + fallback.
  ...
  - Tests: `manager.test.ts:175/183/210/215`,
    ...
    (fake chatStream facade), `OctopusStudioMarkdownParser.test.tsx:164`,
...
- `OctopusStudioMarkdownParser.tsx:248` → `useChatStreamPreview(chatId)`;
...
- **No machine owns the full state**: three sources across six writer
  sites. app_run `commands.ts:57-62` (`octopus-studio-app`); preview_iframe
  `commands.ts:45` clear (cross-machine, runs in beforeNotify —
  synchronous facade calls forbidden); `useRunApp.ts:175-186/:191-195`
  (`octopus-studio-sync`, priority-merge updaters); PreviewIframe `setErrorMessage`
  (`:223-231`) with call sites `:884/:901` (`preview-app`) **plus the four
  the trace missed**: `:415-426` cloud-sandbox errors with source
  `octopus-studio-app` (so app_run state alone can never replace the reader),
  `:438-445` cloud sync errors (`octopus-studio-sync`, same clobber guard),
...
- Owner decision: **preview_iframe** — it already issues both clears and
  ...
  command pipeline).
- Encode in transitions: source-priority updater semantics (octopus-studio-sync must
  not clobber preview-app/octopus-studio-app; only-clear-own-source on recovery),
...
- Reader `PreviewIframe.tsx:221` → `selectPreviewError(iframeState)` (it
  ...
  is load-bearing (`:1515` hasStartupError only for `octopus-studio-app`).
...
`useStreamChat.test.tsx`, `OctopusStudioMarkdownParser.test.tsx`,
`explore_chat_history_streaming.integration.test.tsx`,
