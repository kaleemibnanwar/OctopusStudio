...
Add a server layer (Nitro) to Vite/React apps in OctopusStudio through an **AI-callable tool** that the agent invokes automatically when the user's prompt indicates server-side needs (database access, secrets, API routes, webhooks). This closes the architectural gap between Next.js (which has built-in API routes) and Vite for database-backed apps — specifically unblocking Neon database integration for Vite projects, which requires server-side access to `DATABASE_URL`. Nitro-specific coding conventions are appended to the project's `AI_RULES.md` so all future chats know the patterns.

## Problem Statement

OctopusStudio users building React/Vite apps have no server layer. This creates three concrete problems:
...
Since Vite is the default template, this affects the majority of OctopusStudio apps.
...
4. Tool execution renders a chat-visible XML tag (e.g., `<octopus-studio-enable-nitro />`) similar to `<octopus-studio-add-integration>`, so the user sees the scaffolding action in the message stream
...
`enable_nitro` is **not registered in legacy build mode**. Build mode uses the text-stream + `octopus-studio-write` XML-tag pipeline ([chat_stream_handlers.ts:1330](src/ipc/handlers/chat_stream_handlers.ts#L1330)), not real model tool calls. Two options for build-mode users in v1:
...
- **Option B (deferred)**: Add a parallel build-mode path that scaffolds Nitro via a `<octopus-studio-enable-nitro />` XML tag the way `octopus-studio-add-integration` already works. Skipped because it duplicates logic and build mode is being phased toward local-agent.
...
- **Tool-call message in chat** — Renders like other agent tool calls (consistent with existing `octopus-studio-write` / MCP tool UX) so the user sees what's being scaffolded.
...
| **AI Tool**             | `src/pro/main/ipc/handlers/local_agent/tools/enable_nitro.ts` (new)                                              | `enable_nitro` tool definition matching the existing `ToolDefinition` shape (mirrors `add_integration.ts` for signaling, `add_dependency.ts` for the install flow). Fields: `name`, `description` (carries post-call setup steps), `inputSchema`, `defaultConsent: "always"`, `modifiesState: true`, `isEnabled: (ctx) => ctx.frameworkType === "vite" && !ctx.nitroEnabled`, `getConsentPreview`, `buildXml` (renders `<octopus-studio-enable-nitro />`), `execute` (the scaffolding logic) |
...
| **Chat UI renderer**    | `src/components/chat/OctopusStudioMarkdownParser.tsx` + `src/components/chat/OctopusStudioEnableNitro.tsx` (new) | Add `octopus-studio-enable-nitro` to the `OCTOPUS_STUDIO_CUSTOM_TAGS` allowlist and a `case "octopus-studio-enable-nitro":` in `renderCustomTag` (mirror the `octopus-studio-add-integration` case). Render a minimal status card — no interactive UI; the tool's `execute` already did the scaffolding.                                                                                                                                                                                     |
...
Example final vite.config.ts:
     ...
     import { nitro } from "nitro/vite";
     import { octopus-studioComponentTagger } from "@octopus-studio-sh/react-vite-component-tagger";

     export default defineConfig({
       plugins: [octopus-studioComponentTagger(), react(), nitro()],
     });
...
3. Then write the user-requested API route(s) following the conventions documented
  ...
  buildXml: (_args, _isComplete) => `<octopus-studio-enable-nitro />`,
...
- [ ] `buildXml: () => "<octopus-studio-enable-nitro />"` so the action renders in the chat stream
...
- [ ] Add `octopus-studio-enable-nitro` to the `OCTOPUS_STUDIO_CUSTOM_TAGS` allowlist in `src/components/chat/OctopusStudioMarkdownParser.tsx` and a `case "octopus-studio-enable-nitro":` in `renderCustomTag` (mirror the `octopus-studio-add-integration` case); create `src/components/chat/OctopusStudioEnableNitro.tsx` for the minimal status-card render
...
- [ ] Verify `octopus-studioComponentTagger` plugin doesn't conflict with Nitro plugin (rules dictate Nitro-last ordering so Vite's module-transform middleware runs before Nitro's SPA fallback)
...
| `octopus-studioComponentTagger` plugin conflicts with Nitro plugin                    | Low        | Low    | AI rules dictate Nitro-last ordering in the plugins array so Vite's module-transform middleware handles `/src/*`, `/@vite/*`, `/@fs/*` before Nitro's SPA fallback                                        |
...
- **`DATABASE_URL` injection during local dev** (follow-up): When Neon is connected, how does the dev server get the connection string? Options: OctopusStudio writes `.env` file, OctopusStudio sets env var on process, or system prompt instructs agent. Resolve before Phase 5.
...
| **Local-agent mode only (build mode gets a switch-mode nudge)**        | Real model-dispatched tool calls only flow through `handleLocalAgentStream`; legacy build mode uses text-stream + `octopus-studio-write` XML tags. Adding a parallel build-mode XML scaffolder duplicates logic for a code path that's being phased out. A prompt-level nudge in build mode keeps users informed without forking the implementation. |
