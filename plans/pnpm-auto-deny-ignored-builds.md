...
When a user installs a package whose build scripts are not in OctopusStudio's curated allow-list (e.g. `core-js`), pnpm skips the build and — under pnpm 11's `strictDepBuilds: true` default — any later **fresh** `pnpm install` that lacks OctopusStudio's `--config.strictDepBuilds=false` flag fails hard with `ERR_PNPM_IGNORED_BUILDS` (exit 1). Users get stuck on the install screen with no recoverable action, and exported repos fail on Vercel/Netlify/CI.

Fix: after every OctopusStudio-run install, record an explicit `pkg: false` decision in `pnpm-workspace.yaml`'s `allowBuilds` map for each ignored build (outside the OctopusStudio-managed block, with a OctopusStudio marker), commit it, and emit telemetry so frequently-denied-but-legit packages can be promoted to the remote allow-list.
...
- With `--config.strictDepBuilds=false` (OctopusStudio's `PNPM_INSTALL_POLICY_ARGS`), the same install exits 0 with only a warning box.
...
- Known pnpm quirk (out of scope): `allowBuilds` name-matching does not work for `file:` dependencies — even `pkg: true` fails a strict install with `ERR_PNPM_IGNORED_BUILDS: pkg@file:...`. OctopusStudio apps use registry deps, so this is noted but unhandled.

### Why users get stuck

1. User asks for a feature; AI runs `<octopus-studio-add-dependency packages="core-js">`. Install succeeds (OctopusStudio passes the policy flags) with an "Ignored build scripts" warning. `node_modules/.modules.yaml` records `ignoredBuilds: ["core-js@3.49.0"]`. Nothing is recorded in the repo.
2. The app now works — until a **fresh install** happens through any path that doesn't carry `--config.strictDepBuilds=false`:
   ...
   - **Everything outside OctopusStudio**: Vercel/Netlify deploys of the exported repo, GitHub Actions, the user's own terminal, other editors.
...
Once an unlisted-build package is in the lockfile, the repo is poisoned for every standard `pnpm install` consumer. The only official fix (`pnpm approve-builds`) is interactive and unreachable from OctopusStudio's UI. Most OctopusStudio users cannot evaluate "should this package run install scripts" — they just need the app to keep working with the same security posture OctopusStudio already applies (builds skipped unless allow-listed).
...
- Feed telemetry so the remote allow-list (`https://api.octopus-studio.sh/v1/default-approve-builds.txt`, 1h TTL) can be curated from real-world denial frequency.
...
- **Tag each auto-written line** with a trailing `# octopus-studio-auto-denied` comment. This distinguishes OctopusStudio's automatic decision from a human's deliberate `pkg: false`. Critical because `updatePnpmAllowBuildsConfigContentWithSource` filters managed entries that already exist outside the block — without the marker, a later remote-list promotion of that package to `true` would be permanently shadowed by our own auto-denial.
- **Promotion**: when the resolved allow-list (remote or local) contains a package that currently has a `# octopus-studio-auto-denied` entry, remove the denial so the managed `pkg: true` takes effect. Never touch untagged (user-authored) entries. See "Promotion & repair" below for how the skipped build then actually gets run.
...
1. **T0**: user installs `core-js-2` (unlisted, build genuinely needed). Install succeeds under OctopusStudio flags; proactive pass writes `core-js-2: false # octopus-studio-auto-denied` outside the managed block, commits, emits telemetry. App installs cleanly everywhere; if the build was load-bearing the package misbehaves at runtime (status quo today, but now visible in telemetry).
...
Without the promotion pass this lifecycle **deadlocks at T2**: `parseAllowBuildsExistingKeys` filters any managed-list package that already exists outside the block, so OctopusStudio's own T0 denial would shadow the curated `true` forever. The `# octopus-studio-auto-denied` marker is what distinguishes revisable-by-OctopusStudio entries from human decisions.

Ownership semantics for unmanaged entries when the curated list later adds the same package:

| Unmanaged entry at T2                      | Outcome                                                                                            |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| `pkg: false # octopus-studio-auto-denied`  | Promoted: line removed, managed `true` takes over, `pnpm rebuild pkg` runs                         |
...
| Denial shadows a future curated promotion                                                           | `# octopus-studio-auto-denied` marker + promotion pass in §2                                                                                                                                                                       |
