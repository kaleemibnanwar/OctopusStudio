...
- **Keep full-snapshot renderer writes to `.octopus-studio/queue`.** The current store
...
- Legacy `.octopus-studio/queue` migration stages and validates all attachment payloads
...
5. **Create main queue/intent persistence and acceptance transaction.** Migrate
   `.octopus-studio/queue` entries once, paused, into the main aggregate. Keep the old
...
4. Legacy `.octopus-studio/queue` files are imported once and intentionally retained
