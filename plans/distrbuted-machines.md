...
OctopusStudio currently has several related but distinct state-machine architectures:
...
`OctopusStudioError`/`OctopusStudioErrorKind` where the existing IPC error path is more
...
and renderer ALWAYS ship together in production — octopus-studio updates via
...
- `OctopusStudioError` preservation;
...
- Multiple main processes or networked OctopusStudio instances.
...
- Building Erlang/OTP, Akka, or XState inside OctopusStudio.
