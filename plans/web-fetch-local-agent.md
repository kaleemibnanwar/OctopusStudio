...
When users paste a URL into the OctopusStudio chat (e.g., "Help me integrate this API: https://docs.stripe.com/api"), the agent cannot access the content behind that URL. Users must manually copy-paste page content, breaking their flow. This is especially painful for developers building with APIs, following tutorials, or referencing documentation — the most common use cases for OctopusStudio's target audience. The existing `web_crawl` tool only activates for "clone/copy/replicate" intent and requires OctopusStudio Pro, leaving a gap for the broader "read this page for context" use case.
...
- Available to **all users** (free + Pro) — no `isOctopusStudioPro` gate
...
- XML streaming preview via `<octopus-studio-web-fetch>` tag
...
5. A `<octopus-studio-web-fetch>` card appears in the chat showing the URL being fetched with a loading state
...
- **Loading**: Card with URL, spinner, "Fetching..." label (use existing `OctopusStudioStateIndicator` pattern)
...
- **Warning — Unsupported type**: Amber/warning state (not red error): "PDF files cannot be fetched as text. Try copying the relevant content and pasting it into the chat." (Use `<octopus-studio-output type="warning">`)
...
- Expandable cards: Enter/Space to toggle (existing `OctopusStudioCard` pattern)
...
- **New file (renderer):** `OctopusStudioWebFetch` component for rendering the `<octopus-studio-web-fetch>` XML tag in chat
...
- New XML tag `<octopus-studio-web-fetch>` for renderer
...
export const webFetchTool: ToolDefinition<z.infer<typeof webFetchSchema>> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    let xml = `<octopus-studio-web-fetch url="${escapeXmlContent(args.url)}">`;
    if (isComplete) xml += "</octopus-studio-web-fetch>";
    return xml;
  },
...
- [ ] Create `OctopusStudioWebFetch` component to render `<octopus-studio-web-fetch>` XML tags
...
| Allow private/localhost IPs                              | OctopusStudio runs locally; SSRF is a server-side threat model. Fetching localhost:3000 or internal docs is a legitimate use case. Consent dialog provides sufficient protection. |
