 ...
 */
export default function octopusStudioTagger(): Plugin {
  return {
    name: "vite-plugin-octopusStudio-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-octopusStudio-id ───────────────
              ...
              console.warn(
                `[octopusStudio-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[octopusStudio-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
