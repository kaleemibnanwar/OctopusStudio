...
/**
 * A webpack loader that adds `data-octopusStudio-*` attributes to JSX elements.
 */
export default function octopusStudioTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
          ...
          try {
            ...
            const alreadyTagged = node.attributes?.some(
              (attr: any) =>
                ...
                attr.name?.name === "data-octopusStudio-id",
            );
            if (alreadyTagged) return;

            // Build the octopus-studio ID
            const loc = node.loc?.start;
            if (!loc) return;
            const octopusStudioId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-octopusStudio-id="${octopusStudioId}" data-octopusStudio-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[octopusStudio-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[octopusStudio-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(
        `[octopusStudio-tagger] ERROR in ${this.resourcePath}:`,
        err,
      );
