...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp, electronConfig }, use, testInfo) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$octopusStudioUserDataDir,
  ...
  electronApp: [
    async ({ electronConfig }, use, testInfo) => {
      ...
      process.env.OCTOPUS_STUDIO_E2E_PORT_BLOCK_INDEX = String(
      ...
      process.env.OCTOPUS_STUDIO_ENGINE_URL = `http://localhost:${fakeLlmPort}/engine/v1`;
      process.env.OCTOPUS_STUDIO_GATEWAY_URL = `http://localhost:${fakeLlmPort}/gateway/v1`;
      ...
      const userDataDir = path.join(
        baseTmpDir,
        `octopusStudio-e2e-tests-worker-${testInfo.parallelIndex}-${Date.now()}`,
      );
      ...
      (electronApp as any).$octopusStudioUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          ...
          console.warn(
            "Failed to kill octopusStudio.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
