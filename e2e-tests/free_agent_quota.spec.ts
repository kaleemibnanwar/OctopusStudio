...
testSkipIfWindows(
  ...
  async ({ po }) => {
    // Set up WITHOUT OctopusStudio Pro - use test provider instead
    ...
    await expect(
      po.page.getByRole("button", { name: "Upgrade to OctopusStudio Pro" }),
    ).toBeVisible();
    await expect(
    ...
    await po.sendPrompt("[octopusStudio-qa=write] create a simple file");
    await po.chatActions.waitForChatCompletion();
  },
);
