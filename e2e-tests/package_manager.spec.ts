...
const originalTestPnpmVersion = process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION;
...
async function createUpgradeablePnpmShim(userDataDir: string) {
  ...
  await fs.writeFile(
    ...
    [
      "#!/bin/sh",
      'if [ "$1" = "--version" ]; then',
      '  echo "${OCTOPUS_STUDIO_TEST_PNPM_VERSION:-10.15.0}"',
      "  exit 0",
      "fi",
      `exec "${systemPnpmPath}" "$@"`,
      "",
...
async function restorePackageManagerCache() {
  ...
  if (originalTestPnpmVersion === undefined) {
    delete process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION;
  } else {
    process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION = originalTestPnpmVersion;
  }

  if (originalTestInstallPnpmVersion === undefined) {
    delete process.env.OCTOPUS_STUDIO_TEST_INSTALL_PNPM_VERSION;
  } else {
    process.env.OCTOPUS_STUDIO_TEST_INSTALL_PNPM_VERSION =
  ...
  if (originalDefaultApproveBuildsUrl === undefined) {
    delete process.env.OCTOPUS_STUDIO_DEFAULT_APPROVE_BUILDS_URL;
  } else {
    process.env.OCTOPUS_STUDIO_DEFAULT_APPROVE_BUILDS_URL =
...
const testSkipIfWindows = testWithConfigSkipIfWindows({
  preLaunchHook: async ({ userDataDir, fakeLlmPort }) => {
    ...
    process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION = "11.1.2";
    process.env.OCTOPUS_STUDIO_DEFAULT_APPROVE_BUILDS_URL = `http://localhost:${fakeLlmPort}/api/default-approve-builds.txt`;
...
const oldPnpmTestSkipIfWindows = testWithConfigSkipIfWindows({
  ...
  preLaunchHook: async ({ userDataDir, fakeLlmPort }) => {
    ...
    process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION = "10.15.0";
    process.env.OCTOPUS_STUDIO_DEFAULT_APPROVE_BUILDS_URL = `http://localhost:${fakeLlmPort}/api/default-approve-builds.txt`;
  },
  postLaunchHook: restorePackageManagerCache,
});
...
const upgradePnpmTestSkipIfWindows = testWithConfigSkipIfWindows({
  ...
  preLaunchHook: async ({ userDataDir, fakeLlmPort }) => {
    ...
    process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION = "10.15.0";
    process.env.OCTOPUS_STUDIO_TEST_INSTALL_PNPM_VERSION = "11.1.2";
    process.env.OCTOPUS_STUDIO_DEFAULT_APPROVE_BUILDS_URL = `http://localhost:${fakeLlmPort}/api/default-approve-builds.txt`;
  },
  postLaunchHook: restorePackageManagerCache,
});
...
testSkipIfWindows(
  ...
  async ({ po }, testInfo) => {
    ...
    expect(pnpmWorkspaceConfig).toContain(
      "# octopusStudio-default-allow-builds begin",
    );
    expect(pnpmWorkspaceConfig).toContain(
      "# octopusStudio-default-allow-builds-schema=v1",
    );
    expect(pnpmWorkspaceConfig).toContain(
      "# octopusStudio-default-allow-builds-data-version=2026-05-21.2",
    );
    expect(pnpmWorkspaceConfig).toContain(
      "# octopusStudio-default-allow-builds-channel=remote",
    );
    expect(pnpmWorkspaceConfig).toContain(
      "# octopusStudio-default-allow-builds end",
    );
...
realPnpmStrictBuildsTestSkipIfWindows(
  ...
  async ({ po }, testInfo) => {
    ...
    expect(pnpmWorkspaceConfig).toContain(
      "fake-build-dep: false # octopusStudio-auto-denied",
    );
    await expect(async () => {
...
realPnpmStrictBuildsTestSkipIfWindows(
  ...
  async ({ po }, testInfo) => {
    ...
    // read the 9.0 lockfile OctopusStudio writes. This is the migration trigger.
