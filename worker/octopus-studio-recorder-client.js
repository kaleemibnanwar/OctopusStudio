 ...
 *
 * Protocol:
 *   down (from parent): { type: "activate-octopusStudio-recorder" | "deactivate-octopusStudio-recorder", token }
 *                       { type: "flush-octopusStudio-recorder", token, requestId }
 *   up   (to parent):   { type: "octopusStudio-recorder-initialized" }
 *                       { type: "octopusStudio-recorder-action", action: RecordedAction }
 *                       { type: "octopusStudio-recorder-flushed", requestId }
 *
 * Known limitation — the top-level preview document only. This script is
 * injected into nested frames too, but OctopusStudio activates only the frame it embeds,
...
(() => {
  ...
  const allowUntrustedEvents =
    window.__OCTOPUS_STUDIO_RECORDER_ALLOW_UNTRUSTED__ === true;
  ...
  const recorderToken =
    document.currentScript?.dataset.octopusStudioRecorderToken;
  try {
    document.currentScript?.removeAttribute(
      "data-octopusStudio-recorder-token",
    );
  } catch {
    // Best effort: the value is already snapshotted above either way.
  }
  try {
    delete window.__OCTOPUS_STUDIO_RECORDER_ALLOW_UNTRUSTED__;
  } catch {
  ...
  const OVERLAY_CLASS = "__octopus_studio_recorder_overlay__";
  ...
  function buildCandidates(el) {
    ...
    // `data-octopusStudio-id` is deliberately NOT a candidate: it encodes a source
    // location, so it moves whenever the file it points into is edited, and the
    // OctopusStudio plugin that injects it doesn't run in the build the test replays
  ...
  function emit(action) {
    ...
    window.parent.postMessage(
      { type: "octopusStudio-recorder-action", action },
      "*",
    );
  }
  ...
  // whoever asked. This script only has a job inside OctopusStudio's preview frame.
  ...
  window.addEventListener("message", (e) => {
    ...
    if (type === "activate-octopusStudio-recorder") activate();
    else if (type === "deactivate-octopusStudio-recorder") deactivate();
    else if (
      type === "flush-octopusStudio-recorder" &&
      typeof e.data.requestId === "string"
    ) {
      window.parent.postMessage(
        { type: "octopusStudio-recorder-flushed", requestId: e.data.requestId },
        "*",
      );
    }
  });
  ...
  function init() {
    window.parent.postMessage(
      { type: "octopusStudio-recorder-initialized" },
      "*",
    );
    console.debug("OctopusStudio recorder client initialized");
  }
