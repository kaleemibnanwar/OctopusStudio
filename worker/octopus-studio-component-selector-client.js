(() => {
  const OVERLAY_CLASS = "__octopus_studio_overlay__";
  let overlays = [];
  ...
  function updateOverlay(el, isSelected = false, isHighlighted = false) {
    ...
    const name = el.dataset.octopusStudioName || "<unknown>";
    const file = (el.dataset.octopusStudioId || "").split(":")[0];
    ...
    if (highlightedElement) {
      ...
      if (highlightedItem) {
        ...
        window.parent.postMessage(
          {
            type: "octopusStudio-component-coordinates-updated",
            coordinates: {
              top: rect.top,
              left: rect.left,
              width: rect.width,
  ...
  function removeOverlayById(componentId) {
    ...
    overlays.forEach((item, index) => {
      if (item.el.dataset.octopusStudioId === componentId) {
    ...
    if (
      highlightedElement &&
      highlightedElement.dataset.octopusStudioId === componentId
    ) {
      highlightedElement = null;
    }
  }
   ...
   * Returns the parent octopusStudio-tagged element if the current one is an overlay,
  ...
  function skipOverlayElement(el) {
    ...
    // This looks like an overlay — walk up to the parent with a octopusStudio-id
    let parent = el.parentElement;
    while (parent && !parent.dataset.octopusStudioId)
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.octopusStudioId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    const clickedComponentId = state.element.dataset.octopusStudioId;
    ...
    if (selectedItem && (highlightedElement === state.element || !isProMode)) {
      ...
      window.parent.postMessage(
        {
          type: "octopusStudio-component-deselected",
    ...
    if (!state.element.dataset.octopusStudioRuntimeId) {
      state.element.dataset.octopusStudioRuntimeId = `octopusStudio-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }

    const rect = state.element.getBoundingClientRect();
    window.parent.postMessage(
      {
        type: "octopusStudio-component-selected",
        component: {
          id: clickedComponentId,
          name: state.element.dataset.octopusStudioName,
          runtimeId: state.element.dataset.octopusStudioRuntimeId,
        },
        coordinates: {
          top: rect.top,
          left: rect.left,
  ...
  function onKeyDown(e) {
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "octopusStudio-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "octopusStudio-pro-mode") {
      isProMode = e.data.enabled;
    }
    if (e.data.type === "activate-octopusStudio-component-selector") activate();
    if (e.data.type === "deactivate-octopusStudio-component-selector")
      deactivate();
    if (e.data.type === "activate-octopusStudio-visual-editing") {
      activate();
    }
    if (e.data.type === "deactivate-octopusStudio-visual-editing") {
      deactivate();
      clearOverlays();
    }
    if (e.data.type === "clear-octopusStudio-component-overlays")
      clearOverlays();
    if (e.data.type === "update-octopusStudio-overlay-positions") {
    ...
    if (
      e.data.type === "remove-octopusStudio-component-overlay" ||
      e.data.type === "deselect-octopusStudio-component"
    ) {
      if (e.data.componentId) {
        removeOverlayById(e.data.componentId);
      }
    }
    if (e.data.type === "restore-octopusStudio-component-overlays") {
      const componentIds = e.data.componentIds;
      if (Array.isArray(componentIds)) {
        clearOverlays();
        for (const id of componentIds) {
          const el = document.querySelector(
            `[data-octopusStudio-id="${CSS.escape(id)}"]`,
          );
          if (el) {
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "OctopusStudio component selector initialization failed: document.body not found.",
      );
      return;
    }
    ...
    //
    // see: https://github.com/octopusStudio-sh/octopusStudio/issues/2231
    const INIT_TIMEOUT_MS = 60_000; // Wait up to 60 seconds for tagged elements
    let observer = null;
    let timeoutId = null;

    function checkForTaggedElements() {
      if (document.body.querySelector("[data-octopusStudio-id]")) {
        ...
        window.parent.postMessage(
          {
            type: "octopusStudio-component-selector-initialized",
          },
          "*",
        );
        console.debug("OctopusStudio component selector initialized");
        return true;
      }
      return false;
    }
    ...
    setTimeout(() => {
      ...
      console.debug(
        "OctopusStudio component selector waiting for tagged elements to appear...",
      );

      observer = new MutationObserver((mutations) => {
        // Filter mutations to only process relevant changes
        const hasRelevantMutation = mutations.some((mutation) => {
          // Attribute mutation on data-octopusStudio-id (already filtered by attributeFilter)
          if (mutation.type === "attributes") {
            return true;
          }
          // Check if any added nodes have data-octopusStudio-id
          if (mutation.type === "childList") {
            for (const node of mutation.addedNodes) {
              if (node.nodeType === Node.ELEMENT_NODE) {
                if (
                  node.hasAttribute("data-octopusStudio-id") ||
                  node.querySelector("[data-octopusStudio-id]")
                ) {
                  return true;
                }
              }
      ...
      observer.observe(document.body, {
        ...
        subtree: true,
        attributes: true,
        attributeFilter: ["data-octopusStudio-id"],
      });
      ...
      timeoutId = setTimeout(() => {
        ...
        if (!document.body.querySelector("[data-octopusStudio-id]")) {
          console.warn(
            "OctopusStudio component selector not initialized because no DOM elements were tagged",
          );
        }
      }, INIT_TIMEOUT_MS);
    }, 0);
