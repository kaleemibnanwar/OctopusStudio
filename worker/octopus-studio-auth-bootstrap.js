 ...
 * session starts, the OctopusStudio renderer sends the isolated test user's credentials
 ...
 *   down (from parent): { type: "octopusStudio-auth-login", auth, nonce, token }
 *   up   (to parent):   { type: "octopusStudio-auth-bootstrap-ready", pendingNonce? }
 *                       { type: "octopusStudio-auth-ready", ok: boolean, error?: string,
 *                         path?: string }
 *
 ...
 * lives in sessionStorage — scoped to the long-lived OctopusStudio window, not to the
 * attempt. The nonce is how the new document tells "the marker this attempt just
 * wrote" from "one an earlier attempt abandoned".
 */
(() => {
  // A source check proves only which window sent the message, not that OctopusStudio is
  ...
  const authBootstrapToken =
    document.currentScript?.dataset.octopusStudioAuthToken;
  try {
    document.currentScript?.removeAttribute("data-octopusStudio-auth-token");
  } catch {
    // Best effort: the value is already snapshotted above either way.
  }
  const PENDING_KEY = "__octopus_studio_auth_pending__";
  ...
  function post(ok, error, path) {
    window.parent.postMessage(
      { type: "octopusStudio-auth-ready", ok, error, path, nonce: activeNonce },
      "*",
    );
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (
      data &&
      data.type === "octopusStudio-auth-login" &&
      data.auth &&
      authBootstrapToken &&
  ...
  function checkPendingOnLoad() {
    ...
    // parent resends `octopusStudio-auth-login` in response.
    window.parent.postMessage(
      {
        type: "octopusStudio-auth-bootstrap-ready",
