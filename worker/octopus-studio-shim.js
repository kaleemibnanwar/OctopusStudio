(function () {
  console.debug("octopusStudio-shim.js loaded via proxy v0.6.0");
   ...
   * one case that matters, OctopusStudio's own `location.replace` navigation.
   */
  ...
  window.parent.postMessage(
    {
      type: "octopusStudio-document-loaded",
      payload: {
      ...
      if (url && typeof url === "string") {
        try {
          ...
          if (
            ...
            console.warn(
              "[octopusStudio-shim] Blocked navigation to unsafe URL protocol:",
              parsedUrl.protocol,
            );
          }
        } catch (e) {
          console.error("[octopusStudio-shim] Invalid navigation URL:", e);
        }
      } else if (url) {
        console.warn("[octopusStudio-shim] Invalid URL type:", typeof url);
      } else {
