(() => {
  ...
  function findElementByOctopusStudioId(octopusStudioId, runtimeId) {
    ...
    // Fall back to finding by octopusStudio-id (will get first match)
    const escaped = CSS.escape(octopusStudioId);
    return document.querySelector(`[data-octopusStudio-id="${escaped}"]`);
  }
  ...
  function applyStyles(element, styles) {
    ...
    console.debug(
      `[OctopusStudio Visual Editor] Applying styles:`,
      styles,
      "to element:",
      element,
    );
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementByOctopusStudioId(elementId, runtimeId);
    if (element) {
      ...
      window.parent.postMessage(
        {
          type: "octopusStudio-component-styles",
          data: styles,
        },
        "*",
      );
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementByOctopusStudioId(componentId, runtimeId);
    if (element) {
      ...
      const onInput = () => {
        ...
        window.parent.postMessage(
          {
            type: "octopusStudio-text-updated",
            componentId,
            text: currentText,
          },
          "*",
      ...
      const cleanup = () => {
        ...
        window.parent.postMessage(
          {
            type: "octopusStudio-text-finalized",
            componentId,
            text: finalText,
          },
          "*",
  ...
  function handleGetTextContent(data) {
    ...
    window.parent.postMessage(
      {
        type: "octopusStudio-text-content-response",
        componentId,
  ...
  function handleModifyImageSrc(data) {
    ...
    if (imgEl) {
      ...
      if (imgEl._octopusStudioAbort) imgEl._octopusStudioAbort.abort();
      const controller = new AbortController();
      imgEl._octopusStudioAbort = controller;
      ...
      imgEl.addEventListener(
        ...
        () => {
          ...
          window.parent.postMessage(
            {
              type: "octopusStudio-image-load-error",
              elementId,
              src,
            },
            "*",
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-octopusStudio-component-styles":
        handleGetStyles(data);
        break;
      case "modify-octopusStudio-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-octopusStudio-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-octopusStudio-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-octopusStudio-text-content":
        handleGetTextContent(data);
        break;
      case "modify-octopusStudio-image-src":
