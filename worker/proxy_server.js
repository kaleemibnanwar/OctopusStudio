...
let octopusStudioShimContent = null;
let octopusStudioComponentSelectorClientContent = null;
let octopusStudioRecorderClientContent = null;
let octopusStudioAuthBootstrapContent = null;
let octopusStudioScreenshotClientContent = null;
let htmlToImageContent = null;
let octopusStudioVisualEditorClientContent = null;
let octopusStudioLogsContent = null;
...
try {
  const octopusStudioShimPath = path.join(__dirname, "octopusStudio-shim.js");
  octopusStudioShimContent = fs.readFileSync(octopusStudioShimPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] octopusStudio-shim.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-shim.js: ${error.message}`,
  );
}

try {
  const octopusStudioComponentSelectorClientPath = path.join(
    __dirname,
    "octopusStudio-component-selector-client.js",
  );
  octopusStudioComponentSelectorClientContent = fs.readFileSync(
    octopusStudioComponentSelectorClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] octopusStudio-component-selector-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-component-selector-client.js: ${error.message}`,
  );
}

try {
  const octopusStudioRecorderClientPath = path.join(
    __dirname,
    "octopusStudio-recorder-client.js",
  );
  octopusStudioRecorderClientContent = fs.readFileSync(
    octopusStudioRecorderClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] octopusStudio-recorder-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-recorder-client.js: ${error.message}`,
  );
}

try {
  const octopusStudioAuthBootstrapPath = path.join(
    __dirname,
    "octopusStudio-auth-bootstrap.js",
  );
  octopusStudioAuthBootstrapContent = fs.readFileSync(
    octopusStudioAuthBootstrapPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] octopusStudio-auth-bootstrap.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-auth-bootstrap.js: ${error.message}`,
  );
}

try {
  const octopusStudioScreenshotClientPath = path.join(
    __dirname,
    "octopusStudio-screenshot-client.js",
  );
  octopusStudioScreenshotClientContent = fs.readFileSync(
    octopusStudioScreenshotClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] octopusStudio-screenshot-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-screenshot-client.js: ${error.message}`,
  );
}

try {
  const octopusStudioVisualEditorClientPath = path.join(
    __dirname,
    "octopusStudio-visual-editor-client.js",
  );
  octopusStudioVisualEditorClientContent = fs.readFileSync(
    octopusStudioVisualEditorClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] octopusStudio-visual-editor-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-visual-editor-client.js: ${error.message}`,
  );
}

try {
  const octopusStudioLogsPath = path.join(__dirname, "octopus_studio_logs.js");
  octopusStudioLogsContent = fs.readFileSync(octopusStudioLogsPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] octopus_studio_logs.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopus_studio_logs.js: ${error.message}`,
  );
}

// Load Service Worker files
let octopusStudioSwContent = null;
let octopusStudioSwRegisterContent = null;

try {
  const octopusStudioSwPath = path.join(__dirname, "octopusStudio-sw.js");
  octopusStudioSwContent = fs.readFileSync(octopusStudioSwPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] octopusStudio-sw.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-sw.js: ${error.message}`,
  );
}

try {
  const octopusStudioSwRegisterPath = path.join(
    __dirname,
    "octopusStudio-sw-register.js",
  );
  octopusStudioSwRegisterContent = fs.readFileSync(
    octopusStudioSwRegisterPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] octopusStudio-sw-register.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read octopusStudio-sw-register.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  // These are strings that were used since the first version of the octopus-studio shim.
  // If the octopus-studio shim is used from legacy apps which came pre-baked with the shim
  ...
  if (!legacyAppWithShim) {
    ...
    if (octopusStudioShimContent) {
      scripts.push(`<script>${octopusStudioShimContent}</script>`);
    } else {
      scripts.push(
        '<script>console.warn("[proxy-worker] octopus-studio shim was not injected.");</script>',
      );
    }
  }
  if (octopusStudioComponentSelectorClientContent) {
    scripts.push(
      `<script>${octopusStudioComponentSelectorClientContent}</script>`,
    );
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopus-studio component selector client was not injected.");</script>',
    );
  }
  if (octopusStudioRecorderClientContent) {
    scripts.push(
      `<script data-octopusStudio-recorder-token="${escapedProxyToken}">${octopusStudioRecorderClientContent}</script>`,
    );
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopus-studio recorder client was not injected.");</script>',
    );
  }
  if (octopusStudioAuthBootstrapContent) {
    scripts.push(
      `<script data-octopusStudio-auth-token="${escapedProxyToken}">${octopusStudioAuthBootstrapContent}</script>`,
    );
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopus-studio auth bootstrap was not injected.");</script>',
    );
  }
  ...
  if (octopusStudioScreenshotClientContent) {
    scripts.push(`<script>${octopusStudioScreenshotClientContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopus-studio screenshot client was not injected.");</script>',
    );
  }
  if (octopusStudioVisualEditorClientContent) {
    scripts.push(`<script>${octopusStudioVisualEditorClientContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopus-studio visual editor client was not injected.");</script>',
    );
  }
  if (octopusStudioLogsContent) {
    scripts.push(`<script>${octopusStudioLogsContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopus_studio_logs.js was not injected.");</script>',
    );
  }
  if (octopusStudioSwRegisterContent) {
    scripts.push(`<script>${octopusStudioSwRegisterContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] octopusStudio-sw-register.js was not injected.");</script>',
    );
  }
...
/*                                                                         */
/* In a packaged build the OctopusStudio shell loads from file://, a cross-site     */
...
const server = http.createServer((clientReq, clientRes) => {
  ...
  if (clientReq.url === "/octopusStudio-sw.js") {
    if (octopusStudioSwContent) {
      clientRes.writeHead(200, {
        "content-type": "application/javascript",
        "service-worker-allowed": "/",
        "cache-control": "no-cache",
      });
      clientRes.end(octopusStudioSwContent);
      return;
    } else {
  ...
  const upReq = lib.request(upOpts, (upRes) => {
    ...
    upRes.on("end", () => {
      try {
        ...
        // recorder control and `octopusStudio-auth-login` messages sent with it are
