...
/**
 * Per-chat queued-prompt files live inside the app's OctopusStudio-managed `.octopusStudio/`
 * folder, mirroring how agent todos are persisted
 * (`<appPath>/.octopusStudio/todos/<chatId>.json`). Keeping them here means they are
 * scoped to their app and cleaned up automatically when the app is deleted.
 *
 * Layout: `<appPath>/.octopusStudio/queue/<chatId>.json`
 */
function getChatQueueDir(appPath: string): string {
  return path.join(appPath, ".octopusStudio", "queue");
}
...
function listAppPaths(): string[] {
  return getDb()
    ...
    .from(apps)
    .all()
    .map((row) => getOctopusStudioAppPath(row.path));
}
