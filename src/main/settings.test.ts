...
describe("writeSettings", () => {
    ...
    expect(mockSend).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        action: {
          ...
          url: "https://www.octopusStudio.sh/docs/guides/migrate-restore#restoring-settings-from-backup",
        },
  ...
  it("throws a classified error when the settings file cannot be written", () => {
    ...
    expect(thrown).toBeInstanceOf(OctopusStudioError);
    expect((thrown as OctopusStudioError).kind).toBe(
      OctopusStudioErrorKind.External,
    );
    expect((thrown as OctopusStudioError).cause).toBeInstanceOf(Error);
  ...
  it("classifies settings validation failures and preserves the cause", () => {
    ...
    expect((thrown as OctopusStudioError).cause).toBeInstanceOf(ZodError);
  });
