...
import {
  buildOctopusStudioMediaUrlForApp,
  buildOctopusStudioMediaThumbnailUrl,
  buildOctopusStudioMediaUrl,
} from "../lib/octopusStudioMediaUrl";
import {
...
import { createOctopusStudioMediaProtocolHandler } from "./octopus_studio_media_protocol";
...
describe("octopus-studio-media thumbnail protocol", () => {
  let root: string;
  let appPath: string;
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(
      path.join(os.tmpdir(), "octopus-studio-media-protocol-"),
    );
    appPath = path.join(root, "app");
    mediaPath = path.join(appPath, ".octopusStudio", "media");
  ...
  function makeHandler({
    ...
    const handler = createOctopusStudioMediaProtocolHandler({
      cacheRoot,
  ...
  it("resolves opaque app IDs without exposing the app path in the URL", async () => {
    ...
    const url = buildOctopusStudioMediaUrlForApp(7, "image.png");
  ...
  it("preserves legacy media URLs for an app folder named app-id", async () => {
    const legacyMediaPath = path.join(
      root,
      "app-id",
      ".octopusStudio",
      "media",
    ...
    await expect(
      handler(new Request(buildOctopusStudioMediaUrl("app-id", "legacy.png"))),
  ...
  it("serves originals only when a thumbnail was not requested", async () => {
    ...
    const result = await handler(
      new Request(buildOctopusStudioMediaUrl(appPath, "image.png")),
    );
  ...
  it("creates a bounded derivative once and reuses its mtime-keyed cache", async () => {
    ...
    const requestUrl = buildOctopusStudioMediaThumbnailUrl(
      appPath,
      "image.png",
      stat.mtimeMs,
      stat.size,
    ...
    expect(first.headers.get("x-octopus-studio-thumbnail-cache")).toBe("miss");
    expect(first.headers.get("cache-control")).toContain("immutable");
    expect(second.headers.get("x-octopus-studio-thumbnail-cache")).toBe("hit");
  ...
  it("reports coalesced cache misses accurately", async () => {
    ...
    expect(first.headers.get("x-octopus-studio-thumbnail-cache")).toBe("miss");
    expect(second.headers.get("x-octopus-studio-thumbnail-cache")).toBe("miss");
    expect(third.headers.get("x-octopus-studio-thumbnail-cache")).toBe("hit");
    expect(createThumbnailFromPath).toHaveBeenCalledTimes(1);
  });
  ...
  it("rejects oversized image dimensions before native decoding", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "oversized.png")}?thumbnail=1`,
      ),
    );
  ...
  it("accepts JPEGs whose metadata places dimensions after 128 KiB", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "metadata-heavy.jpg")}?thumbnail=1`,
      ),
    );
  ...
  it("rejects oversized source files before reading or decoding them", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "too-large.png")}?thumbnail=1`,
      ),
    );
  ...
  it("reports empty image files separately from oversized files", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "empty.png")}?thumbnail=1`,
      ),
    );
  ...
  it("falls back safely for corrupt images and oversized output", async () => {
    ...
    const corruptResult = await corrupt.handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "corrupt.png")}?thumbnail=1`,
      ),
    );
    ...
    const outputResult = await oversizedOutput.handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "large-output.png")}?thumbnail=1`,
      ),
    );
    ...
    const invalidOutputResult = await invalidOutput.handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "invalid-output.png")}?thumbnail=1`,
      ),
    );
  ...
  it("decodes a bounded snapshot when the source changes during generation", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "image.png")}?thumbnail=1`,
      ),
    );
  ...
  it("rejects symlinks that escape the media directory", async () => {
    ...
    const result = await handler(
      new Request(
        `${buildOctopusStudioMediaUrl(appPath, "escape.png")}?thumbnail=1`,
      ),
    );
