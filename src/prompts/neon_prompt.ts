...
- **no-manual-migrations**: NEVER write SQL migration files manually. Always use the execute SQL tool (\`<octopus-studio-execute-sql>\`) to run schema changes against the Neon database.
...
- Environment variables (\`.env.local\` in OctopusStudio-generated Next.js apps)
...
# Neon Database (injected by Octopus Studio)
