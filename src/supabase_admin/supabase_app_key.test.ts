...
function makeApp(key: string | null): string {
  const appPath = fs.mkdtempSync(
    path.join(os.tmpdir(), "octopus-studio-app-key-"),
  );
...
beforeEach(() => {
  ...
  // OctopusStudio's alone to commit.
...
describe("switchAppToPublishableKey", () => {
  ...
  // OctopusStudio's own one-line edit shouldn't land the user in the "uncommitted
  ...
  it("leaves a client.ts that symlinks outside the app alone", async () => {
    ...
    const outside = path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), "octopus-studio-outside-")),
      "client.ts",
    );
  ...
  // not the hunk OctopusStudio changed, so committing a file the user was mid-edit in
