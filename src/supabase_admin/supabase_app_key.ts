 ...
 * already exist. So the key outlives the format OctopusStudio writes today, and keeps
 ...
 * has to reach the renderer as a `OctopusStudioError` with a kind, not as a raw `ENOENT`
 * that PostHog then files as an unclassified product exception
 * (`rules/octopus-studio-errors.md`).
 */
...
/**
 * Was the client file untouched before OctopusStudio rewrote it?
 *
 * Decides whether the rewrite may be auto-committed. `git commit -- <path>`
 * records the whole working-tree version of that path, not the single hunk OctopusStudio
 ...
 *
 * OctopusStudio's own edit is not something the user needs to review: leaving it in the
