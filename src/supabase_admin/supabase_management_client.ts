...
} from "@octopus-studio-sh/supabase-management-js";
...
async function refreshSupabaseTokenOnce(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(
      "https://supabase-oauth.octopusStudio.sh/api/connect-supabase/refresh",
      {
        method: "POST",
        headers: {
 ...
 *
 * Existing `OctopusStudioError`s pass through with their kind intact. A 401/403 means the
...
export function classifyManagementApiError(
  ...
  if (
    ...
    return new OctopusStudioError(
      `Supabase would not authorize OctopusStudio to ${action}. Reconnect your Supabase account in Settings, or check that this organization still has access to the project. Original error: ${message}`,
      OctopusStudioErrorKind.Auth,
    );
  }
  return error;
