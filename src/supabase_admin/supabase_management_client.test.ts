...
import { SupabaseManagementAPIError } from "@octopus-studio-sh/supabase-management-js";
import {
...
describe("getProjectApiKeys", () => {
  ...
  it("rejects a 200 whose body isn't a list of keys", async () => {
    ...
    expect(isOctopusStudioError(error)).toBe(true);
...
describe("classifyManagementApiError", () => {
  it("maps a Management API 401/403 to an Auth OctopusStudioError", () => {
  ...
  it("preserves an already-classified OctopusStudioError", () => {
    const original = new OctopusStudioError(
      "nope",
