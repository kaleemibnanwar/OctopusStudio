...
/**
 * Credentials for the stock-image search providers exposed through the
 * `search_images` agent tool. Pexels and Pixabay take a plain API key;
 * Openverse uses OAuth2 client credentials (exchanged for a short-lived
 * access token at search time).
 */
export const ImageSearchSettingsSchema = z.object({
  pexelsApiKey: SecretSchema.optional(),
  pixabayApiKey: SecretSchema.optional(),
  openverseClientId: SecretSchema.optional(),
  openverseClientSecret: SecretSchema.optional(),
});
export type ImageSearchSettings = z.infer<typeof ImageSearchSettingsSchema>;
...
export const OctopusStudioProBudgetSchema = z.object({
  budgetResetAt: z.string(),
  maxBudget: z.number(),
});
export type OctopusStudioProBudget = z.infer<
  typeof OctopusStudioProBudgetSchema
>;
...
const BaseUserSettingsFields = {
  ...
  octopusStudioProBudget: OctopusStudioProBudgetSchema.optional(),
  ...
  // Model used for AI image generation (the "Generate Image" dialog and the
  // generate_image agent tool). Lets non-Pro users point image generation at
  // any configured provider (including custom OpenAI-compatible providers)
  // instead of requiring the OctopusStudio Pro engine.
  selectedImageModel: LargeLanguageModelSchema.optional(),
  ...
  imageSearch: ImageSearchSettingsSchema.optional(),
  autoApproveChanges: z.boolean().optional(),
  telemetryConsent: z.enum(["opted_in", "opted_out", "unset"]).optional(),
  telemetryUserId: z.string().optional(),
  hasRunBefore: z.boolean().optional(),
  enableOctopusStudioPro: z.boolean().optional(),
...
export const StoredUserSettingsSchema = z
  .object({
    ...
    // Deprecated: OctopusStudio always uses the bundled Dugite Git backend.
...
export function isOctopusStudioProEnabled(_settings: UserSettings): boolean {
  return true;
}

export function hasOctopusStudioProKey(settings: UserSettings): boolean {
