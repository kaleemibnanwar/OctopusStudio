...
describe("generateTestUserFixtureSource", () => {
  it("generates a Better Auth (Neon) sign-in helper", () => {
    ...
    expect(source).toContain("process.env.OCTOPUS_STUDIO_TEST_USER_EMAIL");
    expect(source).toContain("process.env.OCTOPUS_STUDIO_TEST_USER_PASSWORD");
    ...
    expect(source).toContain("process.env.OCTOPUS_STUDIO_TEST_BASE_URL");
    expect(source).toContain("headers: { origin, referer: `${origin}/` }");
    // Should NOT reference Supabase-only env vars.
    expect(source).not.toContain("OCTOPUS_STUDIO_TEST_SUPABASE_ANON_KEY");
  });
  ...
  it("generates a Supabase password-grant sign-in helper", () => {
    ...
    expect(source).toContain("process.env.OCTOPUS_STUDIO_TEST_SUPABASE_URL");
    expect(source).toContain(
      "process.env.OCTOPUS_STUDIO_TEST_SUPABASE_ANON_KEY",
    );
  ...
  it("reports a user-authored fixture as not OctopusStudio-generated", () => {
