 ...
 * (`worker/octopus-studio-recorder-client.js`) posts `RecordedAction`s to the renderer.
 ...
 */
const NAVIGATE_BASE = "http://octopusStudio.invalid";
...
function isAppRelativePath(value: string): boolean {
  ...
  // `"/\t/octopusStudio.invalid/x"` is parsed as the authority-relative
  // `"//octopusStudio.invalid/x"` — reading the raw second character here would find the
  ...
  // resolved origins can't see it: `//octopusStudio.invalid/x` resolves *onto* the
...
export const RecordedActionSchema = z.discriminatedUnion("kind", [
  ...
  // Synthesized in the renderer when the user navigates from OctopusStudio's own chrome:
...
/**
 * Validate an untrusted `octopus-studio-recorder-action` payload; null when malformed.
 *
