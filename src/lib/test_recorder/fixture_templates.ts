 ...
 * the helper reads the isolated-session credentials OctopusStudio injects into the run.
 */
...
export function fixtureMarker(mode: Exclude<RecorderAuthMode, "none">): string {
  return `// octopus-studio-generated-fixture: ${mode}`;
}

/** The auth mode a fixture declares, or null when it isn't OctopusStudio-generated. */
...
const NEON_BETTER_AUTH_FIXTURE: string[] = [
  ...
  ``,
  `/**`,
  ` * Sign in the OctopusStudio-provisioned test user by driving the app's own Better Auth`,
  ` * endpoint. The request shares the browser context's cookie jar, so the`,
  ` * session cookie rides along on subsequent navigations.`,
  ` *`,
  ` * OctopusStudio provisions an isolated user per test run and injects its credentials`,
  ` * via OCTOPUS_STUDIO_TEST_USER_EMAIL / OCTOPUS_STUDIO_TEST_USER_PASSWORD.`,
  ` */`,
  `export async function signIn(page: Page): Promise<void> {`,
  `  const email = process.env.OCTOPUS_STUDIO_TEST_USER_EMAIL;`,
  `  const password = process.env.OCTOPUS_STUDIO_TEST_USER_PASSWORD;`,
  `  if (!email || !password) {`,
  `    throw new Error(`,
  `      "OCTOPUS_STUDIO_TEST_USER_EMAIL / OCTOPUS_STUDIO_TEST_USER_PASSWORD are not set. Run this test from OctopusStudio's Tests panel so an isolated user is provisioned.",`,
  `    );`,
  `  }`,
  ...
  `    process.env.OCTOPUS_STUDIO_TEST_BASE_URL || "http://localhost:32100",`,
  `  ).origin;`,
...
const SUPABASE_PASSWORD_FIXTURE: string[] = [
  ...
  ``,
  `/**`,
  ` * Sign in the OctopusStudio-provisioned Supabase test user via the password grant,`,
  ` * then seed supabase-js's session into localStorage before the app loads so`,
  ` * it boots authenticated. OctopusStudio injects the project URL, anon key, and the`,
  ` * isolated user's credentials.`,
  ` */`,
  `export async function signIn(page: Page): Promise<void> {`,
  `  const url = process.env.OCTOPUS_STUDIO_TEST_SUPABASE_URL;`,
  `  const anonKey = process.env.OCTOPUS_STUDIO_TEST_SUPABASE_ANON_KEY;`,
  `  const email = process.env.OCTOPUS_STUDIO_TEST_USER_EMAIL;`,
  `  const password = process.env.OCTOPUS_STUDIO_TEST_USER_PASSWORD;`,
  `  if (!url || !anonKey || !email || !password) {`,
  `    throw new Error(`,
  `      "Supabase test credentials are not set. Run this test from OctopusStudio's Tests panel so an isolated user is provisioned.",`,
  `    );`,
  `  }`,
