 ...
 * isn't listening costs nothing — the re-arm on `octopus-studio-recorder-initialized`
...
export function useTestRecorder({
  ...
  >(null);
  // Settled by the `octopus-studio-recorder-initialized` of the document the recording is
  ...
  /**
   * Record a `page.goto` for a navigation the user made in OctopusStudio's own chrome:
  ...
  const postRecorderControl = useCallback(
    (targetAppId: number, type: "activate" | "deactivate") => {
      ...
      postToIframe({ type: `${type}-octopus-studio-recorder`, token }, origin);
    },
  ...
  const flushRecorder = useCallback(
    (targetAppId: number) => {
      ...
      return new Promise<void>((resolve) => {
        ...
        postToIframe(
          { type: "flush-octopus-studio-recorder", token, requestId },
          origin,
        );
      });
    },
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      ...
      switch (data.type) {
        case "octopus-studio-recorder-action": {
          record(data.action);
          break;
        }
        case "octopus-studio-recorder-initialized": {
        ...
        case "octopus-studio-recorder-flushed": {
          const waiting = recorderFlushRef.current;
          if (
            waiting &&
        ...
        case "octopus-studio-auth-bootstrap-ready": {
          ...
          if (pending && pending.appId === currentAppId) {
            postToIframe(
              {
                type: "octopus-studio-auth-login",
                auth: pending.auth,
                nonce: pending.nonce,
        ...
        case "octopus-studio-auth-ready": {
  ...
  // mid-load; this effect plus the re-arm on `octopus-studio-recorder-initialized` make
  ...
  // the bootstrap's `octopus-studio-auth-bootstrap-ready` announce is no rescue: the
  ...
  const beginRecording = useCallback(
    async (
      ...
      if (auth.mode !== "none") {
        ...
        // could announce — the re-arm on `octopus-studio-recorder-initialized` picks up
