...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as OctopusStudioMarkdownParser but convert to clean text
      const formattedContent =
        convertOctopusStudioContentToMarkdown(messageContent);
  ...
  // Convert OctopusStudio content to clean markdown using the same parsing logic as OctopusStudioMarkdownParser
  const convertOctopusStudioContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from OctopusStudioMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from OctopusStudioMarkdownParser)
  ...
  // Reuse the same parsing functions from OctopusStudioMarkdownParser but simplified
