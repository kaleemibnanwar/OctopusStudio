      ...
      if (currentPermission !== "granted") {
        if (!consentDeniedWarningShownRef.current) {
          ...
          const target = params.sourceLabel
            ...
            : "";
          showWarning(
            `"${params.toolName}"${target} needs your approval. Enable notifications for Octopus Studio in your operating system's notification settings.`,
          );
        }
        return;
      }
    ...
    for (const [requestId, request] of projectedUserInputRequests) {
      ...
      if (
        ...
        startConsentNotification({
          ...
          tagPrefix: "octopus-studio-mcp-consent",
        });
      }
    }
    ...
    void (async () => {
      if (window.Notification.permission === "granted") {
        ...
        showNativeNotification({
          ...
          title: appName,
          body: trimmed,
          tag: `octopus-studio-chat-complete-${chatId}`,
          autoClose: true,
        });
        return;
      }
      ...
      if (window.Notification.permission === "denied") {
        if (!completionDeniedWarningShownRef.current) {
          ...
          showWarning(
            "Enable notifications for Octopus Studio in your operating system's notification settings to receive chat completion alerts.",
          );
        }
        return;
      }
    ...
    const unsubscribe = ipc.events.userInput.onRequested((descriptor) => {
      ...
      if (descriptor.kind === "agent-consent") {
        startConsentNotification({
          ...
          tagPrefix: "octopus-studio-agent-consent",
        });
        ...
        startConsentNotification({
          ...
          tagPrefix: "octopus-studio-plan-questionnaire",
        });
    ...
    const unsubscribe = ipc.events.userInput.onSettled(({ requestId }) => {
      ...
      resolveConsentNotification(
        descriptor.kind === "agent-consent"
          ? "octopus-studio-agent-consent"
          : descriptor.kind === "mcp-consent"
            ? "octopus-studio-mcp-consent"
            : "octopus-studio-plan-questionnaire",
        requestId,
      );
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    const handleFocus = () => {
      ...
      for (const [tag, notification] of notificationsRef.current.entries()) {
        ...
        if (tag.startsWith("octopus-studio-chat-complete-")) {
          notification.close();
          const timer = autoCloseTimersRef.current.get(tag);
          if (timer) clearTimeout(timer);
          autoCloseTimersRef.current.delete(tag);
        ...
        // to avoid OS + in-app UI duplication
          ...
          (tag.startsWith("octopus-studio-agent-consent-") ||
            tag.startsWith("octopus-studio-mcp-consent-") ||
            tag.startsWith("octopus-studio-plan-questionnaire-")) &&
          currentChatId
        ) {
