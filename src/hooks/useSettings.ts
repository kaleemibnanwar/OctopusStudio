...
import { type UserSettings, hasOctopusStudioProKey } from "@/lib/schemas";
import {
...
const TELEMETRY_CONSENT_KEY = "octopusStudioTelemetryConsent";
const TELEMETRY_USER_ID_KEY = "octopusStudioTelemetryUserId";
const OCTOPUS_STUDIO_PRO_STATUS_KEY = "octopusStudioProStatus";
...
export function isOctopusStudioProUser(): boolean {
  return window.localStorage.getItem(OCTOPUS_STUDIO_PRO_STATUS_KEY) === "true";
}
...
function processSettingsForTelemetry(settings: UserSettings) {
  ...
  window.localStorage.setItem(
    OCTOPUS_STUDIO_PRO_STATUS_KEY,
    hasOctopusStudioProKey(settings) ? "true" : "false",
  );
}
