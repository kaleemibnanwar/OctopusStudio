  ...
  const contentWindow = {
    postMessage: (message: unknown, targetOrigin?: string) => {
      ...
      if (
        ...
        message !== null &&
        "type" in message &&
        message.type === "flush-octopus-studio-recorder" &&
        "requestId" in message
      ) {
        queueMicrotask(() =>
          send({
            type: "octopus-studio-recorder-flushed",
...
function reloadAnnouncing(iframe?: ReturnType<typeof makeIframe>) {
  return () => iframe?.send({ type: "octopus-studio-recorder-initialized" });
}
...
describe("useTestRecorder", () => {
  ...
  // A route the user picked through OctopusStudio's chrome IS the starting point, and it
  ...
  it("flushes an iframe action already queued when review is requested", async () => {
    ...
    const flush = iframe.posted.find(
      (message) => message.type === "flush-octopus-studio-recorder",
    );
    ...
    act(() => {
      ...
      iframe.send({
        type: "octopus-studio-recorder-action",
        action: {
          kind: "click",
  ...
  it("waits for the reloaded preview before it says it is recording", async () => {
    ...
    expect(iframe.posted).not.toContainEqual({
      type: "activate-octopus-studio-recorder",
    });

    await act(async () => {
      iframe.send({ type: "octopus-studio-recorder-initialized" });
      await started;
    });
  ...
  it("ignores navigations the app makes on its own", async () => {
    ...
    // The shim (worker/octopus-studio-shim.js) reports every history change the app makes,
  ...
  it("records a navigation made from OctopusStudio's own address bar", async () => {
    const { result } = await recordingSession({
      iframe: makeIframe(),
      appUrl: true,
    });
  ...
  function findLogin(iframe: ReturnType<typeof makeIframe>) {
    const index = iframe.posted.findIndex(
      (message: any) => message?.type === "octopus-studio-auth-login",
    );
    return index === -1
      ? null
  ...
  it("signs the preview in at the app's own origin", async () => {
    ...
    await act(async () => {
      iframe.send({
        type: "octopus-studio-auth-ready",
        ok: true,
  ...
  it("withholds credentials until the preview's origin is known", async () => {
    ...
    act(() => {
      iframe.send({ type: "octopus-studio-auth-bootstrap-ready" });
    });
  ...
  it("refuses the review when teardown reported the environment broken", async () => {
    ...
    stopRecordingMock.mockImplementationOnce(async () => {
      onEnded({
        ...
        reason: "error",
        message:
          "OctopusStudio couldn't restore your app's real database settings",
      });
      return { ok: true };
    });
