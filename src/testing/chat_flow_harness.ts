 ...
 * exercises the REAL octopus-studio chat flow without launching Electron.
 *
 ...
 * response processor (octopus-studio-tag parsing, file writes, git commits, db messages).
 *
...
const HARNESS_ENV_KEYS = [
  "OCTOPUS_STUDIO_DEV_USER_DATA_DIR",
  "FAKE_LLM_DUMP_DIR",
  "FAKE_LLM_FIXTURES_DIR",
  "FAKE_LLM_QUIET",
  "OCTOPUS_STUDIO_LANGUAGE_MODEL_CATALOG_URL",
  "OCTOPUS_STUDIO_ENGINE_URL",
  "OCTOPUS_STUDIO_GATEWAY_URL",
  "OCTOPUS_STUDIO_USER_INFO_URL",
  "OCTOPUS_STUDIO_SUBSCRIPTION_STATUS_URL",
] as const;
...
export interface ChatFlowHarnessOptions {
  ...
  /**
   * Point OctopusStudio Engine and Gateway calls at this harness's fake LLM server.
...
export async function setupChatFlowHarness(
  ...
  try {
    ...
    tmpRoot = path.join(
      os.tmpdir(),
      `octopus-studio-chat-flow-${process.pid}-${Math.random().toString(36).slice(2, 10)}`,
    );
    ...
    process.env.OCTOPUS_STUDIO_DEV_USER_DATA_DIR = userDataDir;
    ...
    if (options.useFakeCatalog !== false) {
      process.env.OCTOPUS_STUDIO_LANGUAGE_MODEL_CATALOG_URL = `${fakeLlmUrl}/api/language-model-catalog`;
    }
    // Always fake the OctopusStudio Pro user-info endpoint: any test that configures an
    // auto API key would otherwise send get-user-budget requests to the real
    // api.octopusStudio.sh.
    process.env.OCTOPUS_STUDIO_USER_INFO_URL = `${fakeLlmUrl}/api/user/info`;
    if (options.engine) {
      process.env.OCTOPUS_STUDIO_ENGINE_URL = `${fakeLlmUrl}/engine/v1`;
      process.env.OCTOPUS_STUDIO_GATEWAY_URL = `${fakeLlmUrl}/gateway/v1`;
    }
