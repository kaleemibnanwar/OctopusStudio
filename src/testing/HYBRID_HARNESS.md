...
describe("my UI feature (hybrid)", () => {
  ...
  it("renders the streamed message", async () => {
    harness.mount();
    const { send } = await harness.typeInChat("tc=octopus-studio-write-angle");
    send();
...
- **OctopusStudio Pro / engine routes** — pass `engine: true` so
  `OCTOPUS_STUDIO_ENGINE_URL` / `OCTOPUS_STUDIO_GATEWAY_URL` point at the harness fake server.
...
- **Seeding attachments**: use `setChatAttachments([{ name, content, mimeType }])`
  ...
  conversion to IPC attachments and `.octopus-studio/media` persistence.
...
- **`get-proposal` / `checkProblems` / cloud-sandbox / desktop-config log lines**
  ...
  `api.octopus-studio.sh/v1/desktop-config`). They are caught and logged, not failures. The act, pnpm-install, and DB-teardown noise **is** handled (§6, §8).
...
- **Implicit pnpm install** — the UI's `nodejs-status` query, when pnpm is
  ...
  `process.env.OCTOPUS_STUDIO_SKIP_MANAGED_PNPM_INSTALL === "true"` (the harness sets it).
...
- **Monaco eager init** — `src/components/chat/monaco.ts` runs `loader.init()` at
  ...
  transitively (`OctopusStudioWrite` → `FileEditor`), so a hybrid test pulls it in even
...
Pass `engine: true` to route OctopusStudio Engine and Gateway calls to the harness fake
...
- **`chatMode` option trap.** The harness `chatMode` option only seeds
  ...
  effective default, and with OctopusStudio Pro enabled the effective default is
