 ...
 *     harness via the `OCTOPUS_STUDIO_DEV_USER_DATA_DIR` env var (read at call time so the
...
function resolveUserDataPath(): string {
  return (
    process.env.OCTOPUS_STUDIO_DEV_USER_DATA_DIR ||
    process.env.OCTOPUS_STUDIO_TEST_USER_DATA_DIR ||
    `${process.env.TMPDIR || "/tmp"}/octopus-studio-vitest-userdata-${process.pid}`
  );
}
...
export function createElectronMock(shared: ElectronMockShared) {
  ...
  return {
    app: {
      ...
      // whenReady() above resolves immediately, so the app is always "ready"
      // by the time test code can observe it.
      isReady: vi.fn(() => true),
