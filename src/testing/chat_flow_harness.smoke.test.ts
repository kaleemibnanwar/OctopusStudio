...
describe("chat flow harness (smoke)", () => {
  ...
  it("streams octopus-studio tags, writes files, commits, and records messages", async () => {
    ...
    // File write from the <octopus-studio-write> tag in octopus-studio-write-angle.md.
    ...
    expect(userMessage.content).toBe("tc=octopus-studio-write-angle");
    expect(assistantMessage.content).toContain("<octopus-studio-write");
