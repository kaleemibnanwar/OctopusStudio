...
const EVENTS: readonly PreviewIframeEvent[] = [
  ...
  // provenance, and ignoring OctopusStudio's own load — is asserted directly below.
  {
    type: "NAVIGATED_IN_APP",
    kind: "documentLoad",
    url: `${URL}/account`,
...
describe("preview iframe transition", () => {
  ...
  it("deactivates an active picker and ignores repeated deactivation", () => {
    ...
    expect(commandsOf(deactivated)).toEqual([
      ...
      {
        type: "post-to-iframe",
        message: { type: "deactivate-octopus-studio-component-selector" },
      },
    ]);
  ...
  it("distinguishes a route OctopusStudio selected from one the app navigated to", () => {
    ...
    expect(typedIn.currentUrlSource).toBe("octopus-studio");
    ...
    for (const kind of ["pushState", "replaceState", "documentLoad"] as const) {
      ...
      // Going back through OctopusStudio's chrome makes it the user's choice again.
      expect(
        transition(redirected, { type: "GO_BACK" }).state.currentUrlSource,
      ).toBe("octopus-studio");
    }

    // OctopusStudio's own navigation loads a document too. That load reports the route
    // OctopusStudio just set, so it must not downgrade the selection it belongs to.
  ...
  it("restores route provenance with the presentation", () => {
    const restored = transition(INITIAL_PREVIEW_IFRAME_STATE, {
      ...
      history: [URL, `${URL}/settings`],
      position: 1,
      source: "octopus-studio",
    });
    expect(restored.state.currentUrlSource).toBe("octopus-studio");
  ...
  it("keeps higher-priority iframe and app errors ahead of sync errors", () => {
    for (const source of ["preview-app", "octopus-studio-app"] as const) {
