...
export function transition(
  ...
  switch (event.type) {
      ...
      return applied(
        {
          ...
          // Restoring it as "octopus-studio" regardless would hand a redirect destination
      ...
      return applied(
        {
          ...
          currentUrlSource: "octopus-studio",
    ...
    case "NAVIGATED_IN_APP": {
      if (!event.url) return ignore(state, "empty-url");
      // A document load caused by OctopusStudio's own navigation must not be misread as
    ...
    case "SYNC_ERROR":
      if (state.error && state.error.source !== "octopus-studio-sync") {
        return ignore(state, "higher-priority-error");
      }
      if (
        state.error?.source === "octopus-studio-sync" &&
        state.error.message === event.message
      ) {
        return ignore(state, "same-preview-error");
      }
      return applied({
        ...state,
        error: { message: event.message, source: "octopus-studio-sync" },
      });
    case "SYNC_RECOVERED":
      return state.error?.source === "octopus-studio-sync"
        ? applied({ ...state, error: undefined })
        : ignore(state, "no-sync-error");
    case "APP_ERROR":
      if (
        state.error?.source === "octopus-studio-app" &&
        state.error.message === event.message
      ) {
        return ignore(state, "same-preview-error");
      }
      return applied({
        ...state,
        error: { message: event.message, source: "octopus-studio-app" },
      });
    ...
    case "PICKER_TOGGLED": {
      ...
      commands.push({
        ...
        message: {
          type: picking
            ? "activate-octopus-studio-component-selector"
            : "deactivate-octopus-studio-component-selector",
        },
      });
