...
describe("preview iframe command adapter", () => {
  it("routes machine messages and leaves component routes claimable", () => {
    ...
    const selectorMessage = {
      source: contentWindow,
      data: { type: "octopus-studio-component-selector-initialized" },
    } as unknown as MessageEvent;
    routePreviewIframeMessage({
    ...
    const responseMessage = {
      source: contentWindow,
      data: {
        type: "octopus-studio-screenshot-response",
        requestId: "capture:1",
        success: true,
  ...
  // Without it the preview keeps reporting the last route OctopusStudio selected, and a
