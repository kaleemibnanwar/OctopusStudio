...
export interface PreviewIframeCommandAdapter extends PreviewIframeCommandRunner {
  ...
  post(
    ...
    message:
      ...
      | { type: "octopus-studio-take-screenshot"; requestId: string },
  ): void;
}
    ...
    execute(appId, command, emit) {
      ...
      target.postMessage(
        componentIds.length === 0
          ...
          : {
              type: "restore-octopus-studio-component-overlays",
              componentIds,
            },
        "*",
      );
...
export type PreviewIframeMachineMessageType =
  | "octopus-studio-component-selector-initialized"
  | "octopus-studio-screenshot-response"
  | "pushState"
  | "replaceState"
  | "octopus-studio-document-loaded";
...
export const PREVIEW_IFRAME_MESSAGE_ROUTES: Readonly<
  ...
  >
> = {
  "octopus-studio-component-selector-initialized": "shared-and-component",
  ...
  "octopus-studio-screenshot-response": "shared-and-component",
  pushState: "machine",
  ...
  "octopus-studio-document-loaded": "machine",
};
...
export function routePreviewIframeMessage(input: {
  ...
  if (type === "octopus-studio-component-selector-initialized") {
    send({ type: "SELECTOR_READY" });
    onSharedMachineEvent({ type: "SELECTOR_READY" });
  } else if (type === "octopus-studio-screenshot-response") {
    ...
    type === "pushState" ||
    type === "replaceState" ||
    type === "octopus-studio-document-loaded"
  ) {
