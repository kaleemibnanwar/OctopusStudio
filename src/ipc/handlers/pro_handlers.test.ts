...
const mocks = vi.hoisted(() => ({
  ...
  readSettings: vi.fn(),
  transcribeWithOctopusStudioEngine: vi.fn(),
  fetch: vi.fn(),
  openExternal: vi.fn(),
  logger: {
    debug: vi.fn(),
...
vi.mock("../utils/llm_engine_provider", () => ({
  transcribeWithOctopusStudioEngine: mocks.transcribeWithOctopusStudioEngine,
}));

vi.mock("../utils/octopus_studio_engine_url", () => ({
  getOctopusStudioEngineBaseUrl: () => "https://engine.example/v1",
}));
...
describe("pro audio transcription handler", () => {
  beforeEach(() => {
    ...
    mocks.transcribeWithOctopusStudioEngine.mockReset();
    mocks.transcribeWithOctopusStudioEngine.mockResolvedValue(
      "transcribed text",
    );
  });
  ...
  it("transcribes a bounded typed array through a zero-copy Buffer view", async () => {
    ...
    expect(mocks.transcribeWithOctopusStudioEngine).toHaveBeenCalledTimes(1);
    const audioBuffer =
      mocks.transcribeWithOctopusStudioEngine.mock.calls[0][0];
    expect(Buffer.isBuffer(audioBuffer)).toBe(true);
    expect([...audioBuffer]).toEqual([1, 2, 3, 4]);
    audioData[0] = 9;
    expect(audioBuffer[0]).toBe(9);
    expect(mocks.transcribeWithOctopusStudioEngine).toHaveBeenCalledWith(
      audioBuffer,
      "recording.webm",
      "request-123",
  ...
  it.each([
    ...
    expect(mocks.transcribeWithOctopusStudioEngine).not.toHaveBeenCalled();
  });
  ...
  it("classifies a missing Pro subscription as an auth error", async () => {
    ...
    ).rejects.toMatchObject({ kind: OctopusStudioErrorKind.Auth });
    expect(mocks.transcribeWithOctopusStudioEngine).not.toHaveBeenCalled();
  });
  ...
  it("rejects transcription IPC from an untrusted renderer", async () => {
    ...
    expect(() => unwrapIpcEnvelope(envelope as never)).toThrow(
      "trusted Octopus Studio renderer",
    );
...
describe("subscription status handlers", () => {
  ...
  afterEach(() => {
    delete process.env.OCTOPUS_STUDIO_SUBSCRIPTION_STATUS_FIXTURE_API_KEY;
  });
  ...
  it.each([
    "http://academy.octopusStudio.sh/subscription",
    "https://example.com/subscription",
    "https://user:pass@academy.octopusStudio.sh/subscription",
    "https://academy.octopusStudio.sh:8443/subscription",
    "not a URL",
  ...
  it("accepts and opens an Academy HTTPS billing URL", async () => {
    const url = "https://academy.octopusStudio.sh/subscription?source=app";
