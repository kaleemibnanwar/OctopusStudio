...
const TEMP_BASE = path.join(
  os.tmpdir(),
  "octopus-studio-app-naming-handler-tests",
);
...
vi.mock("electron", () => ({
  ...
  app: {
    getPath: vi.fn(() =>
      path.join(os.tmpdir(), "octopus-studio-app-naming-user-data"),
    ),
...
vi.mock("@/ipc/handlers/gitignoreUtils", () => ({
  ensureOctopusStudioGitignored: vi.fn(async () => {}),
}));
...
describe("app naming handlers", () => {
  ...
  describe("create-app", () => {
    ...
    it("rejects duplicate display names with Conflict", async () => {
      ...
      ).rejects.toMatchObject({ kind: OctopusStudioErrorKind.Conflict });
    });
  ...
  describe("run-app", () => {
    it("rehydrates the recovery gate from a persisted test branch", async () => {
      ...
      // No in-memory mark exists: this models a fresh OctopusStudio process whose
  ...
  describe("rename-app", () => {
    ...
    it("rejects invalid folder names with Validation", async () => {
      ...
      for (const invalidPath of ["bad|name", "CON", "name.", ".."]) {
        ...
        ).rejects.toMatchObject({ kind: OctopusStudioErrorKind.Validation });
      }
    });
