...
export async function runAppTestsCore({
  ...
  // Always select OctopusStudio's config by name. Playwright auto-resolves
  ...
  // one that honors OCTOPUS_STUDIO_TEST_BASE_URL, so it's passed explicitly rather than
  // OctopusStudio taking over the canonical config name.
  const args = [
    "playwright",
    "test",
    "--config",
  ...
  if (normalizedTestFile) {
    ...
    // Existing user configs can point at a different testDir. OctopusStudio's panel only
  ...
  // baseURL is passed via the OCTOPUS_STUDIO_TEST_BASE_URL env var, not a CLI flag —
...
export async function runAppTestsWithIsolation({
  ...
  const withEnvRestoreWarning = (
    ...
    const restoreMessage =
      "Octopus Studio couldn't restore your app's real database settings after the test run. Restore .env.local before running the app again.";
    return {
      ...result,
  ...
  try {
    ...
    throw isOctopusStudioError(error)
      ? error
