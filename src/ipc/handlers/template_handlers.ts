...
const PRESERVED_TEMPLATE_PATHS = new Set([".git", ".octopusStudio"]);
...
async function allocateNewAppPath({
  ...
  return { newSlug, newAbsPath: getOctopusStudioAppPath(newSlug) };
}
...
async function applyTemplateInPlace({
  ...
  const tempRoot = await fsPromises.mkdtemp(
    path.join(os.tmpdir(), "octopus-studio-template-"),
  );
    ...
    try {
      ...
      if (appWasStopped) {
        throw new OctopusStudioError(
          ...
          OctopusStudioErrorKind.Unknown,
        );
      }
      throw error;
    }
  ...
  createTypedHandler(templateContracts.applyAppTemplate, async (_, params) => {
    ...
    return appOperationCoordinator.run(
      ...
      async () => {
        ...
        const oldAbsPath = getOctopusStudioAppPath(appRecord.path);
        ...
        if (didPathSwap && newAbsPath && newSlug) {
          ...
          // preserved files (.git, .octopusStudio, .env*) from the old directory, update
          ...
          try {
            ...
            // The new template's `.gitignore` likely doesn't contain `.octopusStudio/`,
            // so re-apply it before staging to keep internal metadata out of
            // git.
            await ensureOctopusStudioGitignored(newAbsPath);
        ...
        // The new template's `.gitignore` likely doesn't contain `.octopusStudio/`, so
        // re-apply it before staging to keep internal metadata out of git.
        await ensureOctopusStudioGitignored(workingPath);
