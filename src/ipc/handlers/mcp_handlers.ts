...
import { getMcpCatalog } from "@/ipc/shared/mcp_catalog_source";
import { db } from "../../db";
...
export function registerMcpHandlers() {
  ...
  createTypedHandler(
    ...
    async (_, { slug, expectedStdioConfig }) => {
      ...
      // Surface that as a connectivity problem instead of a not-found. In
      // practice this branch is now effectively unreachable while the local
      // catalog is non-empty, which is the point: a bundled entry stays
      // installable even fully offline.
      if (entries.length === 0) {
        throw new OctopusStudioError(
