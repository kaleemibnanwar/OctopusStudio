...
import {
  getChatProjectRootPath,
  getOctopusStudioAppPath,
} from "../../paths/paths";
import { buildOctopusStudioMediaUrl } from "../../lib/octopusStudioMediaUrl";
...
import { getOctopusStudioExecuteSqlTags } from "../utils/octopus_studio_tag_parser";
...
import { getMaxOutputTokens, getTemperature } from "../utils/token_utils";
import {
  ECONOMY_MAX_CHAT_TURNS,
...
export async function processStreamChunks({
  ...
  for await (const part of fullStream) {
    ...
    if (isModelRefusal(part)) {
      ...
      chunk += escapeOctopusStudioTags(part.text);
    } else if (part.type === "tool-call") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeOctopusStudioTags(JSON.stringify(part.input));
      chunk = `<octopus-studio-mcp-tool-call server="${escapeXmlAttr(serverName)}" tool="${escapeXmlAttr(toolName)}" call-id="${escapeXmlAttr(part.toolCallId)}">\n${content}\n</octopus-studio-mcp-tool-call>\n`;
    } else if (part.type === "tool-result") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeXmlContent(part.output);
      chunk = `<octopus-studio-mcp-tool-result server="${escapeXmlAttr(serverName)}" tool="${escapeXmlAttr(toolName)}" call-id="${escapeXmlAttr(part.toolCallId)}">\n${content}\n</octopus-studio-mcp-tool-result>\n`;
      ...
      chunk = `<octopus-studio-mcp-tool-result server="${escapeXmlAttr(serverName)}" tool="${escapeXmlAttr(toolName)}" call-id="${escapeXmlAttr(part.toolCallId)}" is-error="true">\n${content}\n</octopus-studio-mcp-tool-result>\n`;
    }
  ...
  // Close a thinking block left open when the stream ends (or is aborted)
  // while still emitting reasoning deltas. Otherwise the unclosed `<think>`
  // tag persists in the saved response and the renderer treats the rest of
  // the message as part of an in-progress reasoning block forever.
  if (inThinkingBlock) {
    const closingChunk = "</think>";
    fullResponse += closingChunk;
    incrementalResponse += closingChunk;
    fullResponse = cleanFullResponse(fullResponse);
    fullResponse = await processResponseChunkUpdate({
      fullResponse,
    });
...
export function registerChatStreamHandlers() {
  ...
  const chatStreamHandler = async (
    ...
    try {
      ...
      let octopusStudioRequestId: string | undefined;
      trackedStream = {
        abortController,
        sender: event.sender,
      ...
      let attachmentInfo = "";
      // Display-only attachment info uses <octopus-studio-attachment> tags for inline rendering
      ...
      const appPath =
        chat.app.type === "chat"
          ? getChatProjectRootPath(chat.app.directory, chat.app.id)
          : getOctopusStudioAppPath(chat.app.path);
      ...
      if (incomingAttachments && incomingAttachments.length > 0) {
        ...
        // Create persistent .octopusStudio/media directory for this app
        ...
        for (const attachment of incomingAttachments) {
          ...
          // Save to .octopusStudio/media dir
          ...
          // Build octopus-studio-media:// URL for display
          ...
          const mediaUrl = `octopus-studio-media://media/${encodeURIComponent(chat.app.path)}/.octopusStudio/media/${encodeURIComponent(filename)}`;

          // Build display tag for inline rendering (escape attribute values)
          displayAttachmentInfo += `\n<octopus-studio-attachment name="${escapeXmlAttr(attachment.name)}" type="${escapeXmlAttr(attachment.type)}" url="${escapeXmlAttr(mediaUrl)}" path="${escapeXmlAttr(persistentPath)}" attachment-type="${escapeXmlAttr(attachment.attachmentType)}"></octopus-studio-attachment>\n`;

          if (attachment.attachmentType === "upload-to-codebase") {
            // Provide the .octopusStudio/media path so the AI can copy it into the codebase
            attachmentInfo += `\n\nFile to upload to codebase: "${attachment.name}" (path: ${persistentPath})\nUse the copy_file tool when tools are available, or emit a <octopus-studio-copy> tag otherwise, to copy this file into the codebase at the appropriate location.\n`;
          } else {
            ...
            if (await isTextFile(persistentPath)) {
              try {
                attachmentInfo += `<octopus-studio-text-attachment filename="${escapeXmlAttr(attachment.name)}" type="${escapeXmlAttr(attachment.type)}" path="${escapeXmlAttr(persistentPath)}">
                </octopus-studio-text-attachment>
                \n\n`;
              } catch (err) {
      ...
      // Build the display prompt (with <octopus-studio-attachment> tags for inline rendering)
      ...
      if (mediaRefs.length > 0) {
        try {
          ...
          for (const media of resolvedMedia) {
            ...
            const mediaUrl = buildOctopusStudioMediaUrl(
              chat.app.path,
              media.fileName,
            );
            mediaDisplayInfo += `\n<octopus-studio-attachment name="${escapeXmlAttr(media.fileName)}" type="${escapeXmlAttr(media.mimeType)}" url="${escapeXmlAttr(mediaUrl)}" path="${escapeXmlAttr(media.filePath)}" attachment-type="chat-context"></octopus-studio-attachment>\n`;
          }
      ...
      if (implementPlanMatch) {
        try {
          ...
          const appPath = getOctopusStudioAppPath(chat.app.path);
          ...
          const planPath = `.octopusStudio/plans/${planSlug}.md`;
...
`;
      ...
      // Only OctopusStudio Pro requests have request ids.
      if (settings.enableOctopusStudioPro) {
        // Generate requestId early so it can be saved with the message
        octopusStudioRequestId = uuidv4();
      }
      ...
      const [placeholderAssistantMessage] = await db
        ...
        .values({
          ...
          requestId: octopusStudioRequestId,
          model: settings.selectedModel.name,
          sourceCommitHash:
            chat.app.type === "chat"
              ? null
              : await getCurrentCommitHash({
                  path: getOctopusStudioAppPath(chat.app.path),
                }),
        })
        .returning();
      ...
      if (testResponse) {
        ...
        // Chat projects have no codebase but do have a workspace folder whose
        // documents are their context; resolve that folder so codebase
        // extraction and AI rules read the right place instead of the base
        // apps directory (which `getOctopusStudioAppPath("")` would resolve to).
        const appPath =
          updatedChat.app.type === "chat"
            ? getChatProjectRootPath(
                updatedChat.app.directory,
                updatedChat.app.id,
              )
            : getOctopusStudioAppPath(updatedChat.app.path);
        ...
        // Extract codebase for current app if not a chat-only project
        let codebaseInfo = "";
        let files: CodebaseFile[] = [];
        if (updatedChat.app.type !== "chat") {
        ...
        // additional files through <octopus-studio-read> as needed.
        if (
          isSmartContextEnabled &&
          req.selectedComponents &&
          req.selectedComponents.length > 0
        ...
        // context suppresses `octopusStudioFiles` in favor of `octopusStudioVersionedFiles`, and
        // the engine's deep pipeline does not read `octopusStudioMentionedApps`, so a
        ...
        // or clean <octopus-studio-attachment> tags). Replace the last user message with the
        ...
        // For OctopusStudio Pro + Deep Context, we set to 200 chat turns (+1)
        ...
        // Economy mode caps context turns to cut token usage.
        ...
        const codeExplorerAvailable =
          isOctopusStudioProEnabled(settings) &&
        ...
        let systemPrompt = constructSystemPrompt({
          ...
          chatProject: updatedChat.app?.type === "chat",
          economyMode,
        });
        ...
        if (isSecurityReviewIntent) {
          ...
          try {
            const appPath = getOctopusStudioAppPath(updatedChat.app.path);
...
<octopus-studio-copy from="/absolute/path/to/.octopusStudio/media/source.ext" to="path/to/destination/filename.ext" description="Upload file to codebase"></octopus-studio-copy>
...
`;
        ...
        const limitedHistoryChatMessages = limitedMessageHistory.map((msg) => ({
          ...
          content:
            selectedChatMode === "ask"
              ? removeOctopusStudioTags(removeNonEssentialTags(msg.content))
        ...
        const simpleStreamText = async ({
          ...
          octopusStudioDisableFiles = false,
          files,
        }: {
          ...
          const providerOptions = getProviderOptions({
            octopusStudioAppId: updatedChat.app.id,
            octopusStudioRequestId,
          ...
          const streamResult = streamText({
            ...
            maxOutputTokens: await getMaxOutputTokens(
            ...
            onFinish: async (response) => {
              ...
              if (typeof totalTokens === "number") {
                ...
                logger.log(
                  `Total tokens used (aggregated for message ${placeholderAssistantMessage.id}): ${maxTokensUsed}, output tokens: ${outputTokens ?? "unknown"}`,
                );
              } else {
            ...
            onError: (error: any) => {
              ...
              const requestIdPrefix = isEngineEnabled
                ? `[Request ID: ${octopusStudioRequestId}] `
                : "";
              logger.error(
        ...
        // bytes inside an in-progress octopus-studio-tag's attribute values, so we
        ...
        try {
          ...
          if (!modelRefused && isTurboEditsV2Enabled(settings)) {
            let issues = await dryRunSearchReplace({
              fullResponse,
              appPath: getOctopusStudioAppPath(updatedChat.app.path),
            });
            ...
            while (
              ...
              fullResponse += `<octopus-studio-output type="warning" message="Could not apply Turbo Edits properly for some of the files; re-generating code...">${formattedSearchReplaceIssues}</octopus-studio-output>`;
              ...
              const fixSearchReplacePrompt =
                searchReplaceFixAttempts === 0
                  ? `There was an issue with the following \`octopus-studio-search-replace\` tags. Make sure you use \`octopus-studio-read\` to read the latest version of the file and then trying to do search & replace again.`
                  : `There was an issue with the following \`octopus-studio-search-replace\` tags. Please fix the errors by generating the code changes using \`octopus-studio-write\` tags instead.`;
          ...
          if (
            !modelRefused &&
            !abortController.signal.aborted &&
            hasUnclosedOctopusStudioWrite(fullResponse)
          ) {
            ...
            while (
              ...
              logger.warn(
                `Received unclosed octopus-studio-write tag, attempting to continue, attempt #${continuationAttempts + 1}`,
              );
      ...
      if (!abortController.signal.aborted && fullResponse) {
        // Scrape from: <octopus-studio-chat-summary>Renaming profile file</octopus-studio-chat-title>
        const chatTitle = fullResponse.match(
          /<octopus-studio-chat-summary>(.*?)<\/octopus-studio-chat-summary>/,
        );
        if (chatTitle) {
          await db
            .update(chats)
        ...
        const hasDestructiveSql =
          shouldAutoApply &&
          getOctopusStudioExecuteSqlTags(fullResponse).some((query) =>
      ...
      const errorMessage = isOctopusStudioError(error)
        ? error.message
        : String(error);
  ...
  try {
    if (await isTextFile(filePath)) {
      ...
      const tagPattern = new RegExp(
        `<octopus-studio-text-attachment filename="[^"]*" type="[^"]*" path="${escapedPath}">\\s*<\\/octopus-studio-text-attachment>`,
        "g",
      );
...
export function removeProblemReportTags(text: string): string {
  const problemReportRegex =
    /<octopus-studio-problem-report[^>]*>[\s\S]*?<\/octopus-studio-problem-report>/g;
  return text.replace(problemReportRegex, "").trim();
}

export function removeOctopusStudioTags(text: string): string {
  const octopusStudioRegex =
    /<octopus-studio-[^>]*>[\s\S]*?<\/octopus-studio-[^>]*>/g;
  return text.replace(octopusStudioRegex, "").trim();
}

export function hasUnclosedOctopusStudioWrite(text: string): boolean {
  // Find the last opening octopus-studio-write tag
  const openRegex = /<octopus-studio-write[^>]*>/g;
  let lastOpenIndex = -1;
  let match;
  ...
  const hasClosingTag = /<\/octopus-studio-write>/.test(textAfterLastOpen);

  return !hasClosingTag;
}

function escapeOctopusStudioTags(text: string): string {
  // Escape octopus-studio tags in reasoning content
  // We are replacing the opening tag with a look-alike character
  // to avoid issues where thinking content includes octopus-studio tags
  // and are mishandled by:
  // 1. FE markdown parser
  // 2. Main process response processor
  return text
    .replace(/<octopusStudio/g, "＜octopus-studio")
    .replace(/<\/octopusStudio/g, "＜/octopusStudio");
}
