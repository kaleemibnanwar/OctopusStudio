...
const TEMP_BASE = path.join(os.tmpdir(), "octopus-studio-tests-handler-tests");

vi.mock("electron", () => ({
  ipcMain: { handle: vi.fn(), on: vi.fn() },
  app: {
    getPath: vi.fn(() =>
      path.join(os.tmpdir(), "octopus-studio-tests-handler-user-data"),
    ),
...
vi.mock("@/paths/paths", async (importOriginal) => {
  ...
  const base = nodePath.join(
    nodeOs.tmpdir(),
    "octopus-studio-tests-handler-tests",
  );
  return {
    ...actual,
...
describe("tests handlers", () => {
  ...
  describe("tests:delete", () => {
    ...
    it("reports a missing spec as not found", async () => {
      ...
      ).rejects.toMatchObject({ kind: OctopusStudioErrorKind.NotFound });
