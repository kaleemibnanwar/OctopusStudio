  ...
  createTypedHandler(helpContracts.start, async (event, params) => {
    ...
    try {
      ...
      const provider = createOpenAI({
        baseURL: "https://helpchat.octopusStudio.sh/v1",
        apiKey,
        ...getTestFetchOption(),
      });
      ...
      if (!helpBotModel || helpBotModel.providerId !== "openai") {
        ...
        // responses API via a custom baseURL (helpchat.octopusStudio.sh).
        throw new Error(
          `Help bot requires an OpenAI model (got provider: ${helpBotModel?.providerId ?? "none"}). ` +
            `The 'octopusStudio/help-bot/default' alias must resolve to an OpenAI model.`,
        );
      }
