...
export function registerNeonHandlers() {
  ...
  createLockedHandler(neonContracts.createProject, async (_, params) => {
    ...
    const resolvedAppPath = getOctopusStudioAppPath(appPath);
    ...
    try {
      ...
      throw new OctopusStudioError(message, OctopusStudioErrorKind.External);
    }
  });
  ...
  // Link an existing Neon project to a OctopusStudio app
  ...
  // Unlink a Neon project from a OctopusStudio app
  ...
  testOnlyHandle("neon:fake-connect", async () => {
    ...
    // Connect click) advances just like a real octopusStudio://neon-oauth-return.
