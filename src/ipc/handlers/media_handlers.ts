...
function getMediaFilePath(appPath: string, fileName: string): string {
  ...
  return safeJoin(appPath, OCTOPUS_STUDIO_MEDIA_DIR_NAME, fileName);
}
  ...
  createTypedHandler(mediaContracts.moveMediaFile, async (_, params) => {
    ...
    await withMediaLock(
      ...
      async () => {
        ...
        const sourceAppPath = getOctopusStudioAppPath(sourceApp.path);
        const targetAppPath = getOctopusStudioAppPath(targetApp.path);
        ...
        await ensureOctopusStudioGitignored(targetAppPath);
