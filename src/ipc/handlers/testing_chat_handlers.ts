...
${Array.from(
  { length: 5000 },
  (_, i) =>
    `<octopus-studio-write path="src/stress/file_${i}.ts" description="stress file ${i}">
export const id${i} = ${i};
...
${Array.from({ length: 10000 }, (_, i) => {
  ...
  return `<octopus-studio-write path="src/stress/file_${i}.ts" description="stress file ${i}">
export const id${i} = ${i};
export const name${i} = "file_${i}";
...
// explicit [octopus-studio-qa=stress-...] prompt fires, not at module import.
const TEST_RESPONSES: Record<string, ResponseEntry> = {
  ...
  <octopus-studio-add-dependency packages="deno"></octopus-studio-add-dependency>

  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.

  <octopus-studio-add-dependency packages="@angular/does-not-exist"></octopus-studio-add-dependency>

  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.

  <octopus-studio-add-dependency packages="react-router-dom react-query"></octopus-studio-add-dependency>

  EOM`,
  write: `Hello world
  <octopus-studio-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[octopus-studio-qa=([^\]]+)\]/);
  if (match) {
 ...
 * is local to fully-formed `<octopus-studio-*>` tags and idempotent, so the cleaned
