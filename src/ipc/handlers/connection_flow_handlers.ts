 ...
 * newer flow.
 *
 * The Supabase/Neon deep-link returns cannot pass it: the octopusStudio.sh OAuth
 * proxy's login endpoints take no client-supplied state parameter, so the
 * octopusStudio://…-oauth-return URL carries only tokens — there is nothing to
  ...
  createTypedHandler(connectionFlowContracts.start, async (event, params) => {
    ...
    if (result.started) {
      ...
      if (starter) {
        ...
        // the flow goes straight to awaiting the octopusStudio:// return.
