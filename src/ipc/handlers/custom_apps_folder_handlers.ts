...
export function registerCustomAppsFolderHandlers() {
  createTypedHandler(systemContracts.getCustomAppsFolder, async () => {
    invalidateOctopusStudioAppsBaseDirectoryCache(); // ensure UI is up-to-date
    const directory = getOctopusStudioAppsBaseDirectory();
  ...
  createTypedHandler(systemContracts.selectCustomAppsFolder, async () => {
    const { filePaths, canceled } = await dialog.showOpenDialog({
      ...
      message: "Select the folder where Octopus Studio apps should be stored",
    });
  ...
  createTypedHandler(systemContracts.setCustomAppsFolder, async (_, input) => {
    ...
    const prevPath = getOctopusStudioAppsBaseDirectory();
    let newOctopusStudioAppsBaseDir = getDefaultOctopusStudioAppsDirectory();
    ...
    if (input) {
      ...
      newOctopusStudioAppsBaseDir = normalize(input);
      updatedSettingValue = newOctopusStudioAppsBaseDir;
    } else {
      // Resetting to default
      await mkdir(newOctopusStudioAppsBaseDir, { recursive: true });
    }
    ...
    if (newOctopusStudioAppsBaseDir !== prevPath) {
      ...
      const directory =
        updatedSettingValue ?? getDefaultOctopusStudioAppsDirectory();
