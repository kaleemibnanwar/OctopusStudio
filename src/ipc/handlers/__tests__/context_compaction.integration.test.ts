...
// conversion restores that surface (the <octopus-studio-compaction> "Conversation
// compacted" card rendered in the messages list) while keeping every
// db-visible assertion (the <octopus-studio-compaction> marker, the "Key Decisions Made"
...
// of a stream error. OctopusStudio Engine calls are routed to the harness fake server
// via `engine: true`.
...
describe("context compaction (integration)", () => {
  ...
  it("compaction triggers and shows summary", async () => {
    ...
    expect(contents).toContain(
      '<octopus-studio-compaction title="Conversation compacted"',
    );
