...
// (`getByTestId("octopus-studio-search-replace")` visible), which the hybrid harness lets
...
describe("local agent search_replace (hybrid)", () => {
  ...
  it("applies a targeted search_replace edit and renders the tool card", async () => {
    ...
    // getByTestId("octopus-studio-search-replace").
    await waitFor(
      () =>
        expect(
          screen.getByTestId("octopus-studio-search-replace"),
        ).toBeTruthy(),
      { timeout: 20_000 },
    );
...
export default App;`,
    ...
    expect(assistant.content).toContain("octopus-studio-search-replace");
