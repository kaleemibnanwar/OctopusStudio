...
// appends a <octopus-studio-step-limit> notice instead of reaching the fixture's final
// "All steps completed." turn. The OctopusStudioStepLimit card ("Paused after 100 tool
...
describe("local agent step limit (integration)", () => {
  ...
  it("pauses after 100 tool calls, holds a queued prompt, and drains it after Continue", async () => {
    ...
    // The OctopusStudioStepLimit pause card renders in the DOM — the same surface the
    ...
    expect(assistant.content).toContain(
      '<octopus-studio-step-limit steps="100"',
    );
    expect(assistant.content).toContain(
    ...
    // Click the REAL OctopusStudioStepLimit "Continue" button — it streams a plain
    ...
    const continueAssistant = continueMessages
      ...
      .at(-1)!;
    expect(continueAssistant.content).not.toContain(
