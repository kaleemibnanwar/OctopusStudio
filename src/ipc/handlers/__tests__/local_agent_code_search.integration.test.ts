...
// the (fake) OctopusStudio Engine /tools/code-search endpoint, and the resulting
// <octopus-studio-code-search> XML with the relevant files lands in the assistant
// message — now also asserted as the rendered Code Search tool card in the
// DOM. code_search requires OctopusStudio Pro and uses the harness fake server via
// `engine: true`.
...
describe("local-agent code_search (integration)", () => {
  ...
  it("searches the codebase via the engine code-search endpoint", async () => {
    ...
    const xmlMatch = content.match(
      /<octopus-studio-code-search query="React component rendering">([\s\S]*?)<\/octopus-studio-code-search>/,
    );
