...
// the turn, streaming `<octopus-studio-status>` progress updates
...
// renders as the real <octopus-studio-status> card in the messages list (the surface
...
describe("local agent supabase deploy progress (integration)", () => {
  ...
  it("shows Supabase deploy queue progress and completes 20/20", async () => {
    ...
    // The finished <octopus-studio-status> card renders in the messages list — the same
    ...
    expect(assistant.content).toContain(
      '<octopus-studio-status title="Supabase functions deployed: 20/20 complete" state="finished">',
    );
