...
import { invalidateOctopusStudioAppsBaseDirectoryCache } from "@/paths/paths";
import {
...
describe("GitHub import dialog (integration)", () => {
  ...
  it("skips the tagger upgrade when 'Optimize for OctopusStudio' is unchecked", async () => {
    await harness.github.resetRepos();
    writeSettings({
    ...
    // Reveal the advanced options so the "Optimize for OctopusStudio" checkbox mounts,
    ...
    expect(config).not.toContain("octopusStudioComponentTagger");

    const pkg = fs.readFileSync(path.join(appDir, "package.json"), "utf8");
    expect(pkg).not.toContain("@octopus-studio-sh/react-vite-component-tagger");
  }, 90_000);
  ...
  it("serializes concurrent imports with the same app name", async () => {
    ...
    const params = {
      ...
      optimizeForOctopusStudio: false,
    };
