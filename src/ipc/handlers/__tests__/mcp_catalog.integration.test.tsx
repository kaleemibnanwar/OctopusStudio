...
describe("Plugins catalog (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    if (typeof address === "object" && address) {
      process.env.OCTOPUS_STUDIO_MCP_CATALOG_URL = `http://127.0.0.1:${address.port}/`;
    }
  ...
  afterAll(async () => {
    delete process.env.OCTOPUS_STUDIO_MCP_CATALOG_URL;
  ...
  it("features flagged entries at the top and still lists them by category", async () => {
    ...
    const previousUrl = process.env.OCTOPUS_STUDIO_MCP_CATALOG_URL;
    try {
      ...
      process.env.OCTOPUS_STUDIO_MCP_CATALOG_URL = previousUrl;
      ...
      servers: [
        {
          ...
          transport: "stdio",
          command: "npx",
          args: ["-y", "@octopus-studio-sh/e2e-nonexistent-mcp@1.0.0"],
        },
      ],
    };
    clearMcpCatalogCacheForTests();
    try {
      ...
      await expect(
        ipc.mcp.addFromCatalog({
          ...
          expectedStdioConfig: {
            command: "npx",
            args: ["-y", "@octopus-studio-sh/e2e-nonexistent-mcp@2.0.0"],
          },
        }),
      ).rejects.toThrow(/changed/i);
      expect(await ipc.mcp.listServers()).toHaveLength(0);
