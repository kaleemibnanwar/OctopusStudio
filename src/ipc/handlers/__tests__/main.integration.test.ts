...
describe("main chat flow (hybrid)", () => {
  ...
  it("simple message to custom test model", async () => {
    ...
    // ...then the streamed assistant response. The canned response's octopus-studio-write
    // tag is parsed into a OctopusStudioWrite card; the trailing literal text ("EOM")
    ...
    expect(assistantMessage.content).toContain(
      '<octopus-studio-write path="file1.txt">',
    );
    expect(assistantMessage.content).toContain("EOM");

    // The octopus-studio-write was applied and committed (auto-approve).
