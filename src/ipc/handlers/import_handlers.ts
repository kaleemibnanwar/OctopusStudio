...
import { db } from "@/db";
import { chats, chatSearchDirtyChats } from "@/db/schema";
  ...
  handle(
    ...
    async (
      ...
      if (!skipCopy) {
        ...
        // Copy the app folder to the OctopusStudio apps directory.
      ...
      try {
        ...
        logger.error("Error getting git revision for imported app:", error);
      }
      ...
      // Trigger a background index of the new chat
      await db.insert(chatSearchDirtyChats).values({ chatId: chat.id });
