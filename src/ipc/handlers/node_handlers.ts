...
async function reloadNodePath() {
  ...
  if (platform() === "win32") {
    ...
    // %PATH%`) can never observe PATH entries an installer added while OctopusStudio
...
async function installManagedPnpm(): Promise<string> {
  ...
  if (!existsSync(managedPackageJsonPath)) {
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "octopus-studio-managed-pnpm", private: true }, null, 2)}\n`,
    );
  }
...
function scheduleManagedPnpmInstall(currentPnpmVersion: string | null): void {
  ...
  // OCTOPUS_STUDIO_TEST_* escapes) so a stray env var can't disable the install in a
  // shipped build, and logged so a skip is never silent.
  if (
    (IS_TEST_BUILD || process.env.VITEST) &&
    process.env.OCTOPUS_STUDIO_SKIP_MANAGED_PNPM_INSTALL === "true"
  ) {
    logger.info(
      "Skipping implicit OctopusStudio-managed pnpm install (OCTOPUS_STUDIO_SKIP_MANAGED_PNPM_INSTALL).",
    );
    return;
  }
  if (managedPnpmInstallPromise) {
    logger.info("OctopusStudio-managed pnpm install is already in progress.");
    return;
  }
  if (managedPnpmImplicitInstallFailed) {
    logger.info(
      "Skipping implicit OctopusStudio-managed pnpm install because it already failed this session.",
    );
    return;
  }
  ...
  if (currentPnpmVersion) {
    logger.info(
      `Existing pnpm ${currentPnpmVersion} is older than ${PNPM_MINIMUM_RELEASE_AGE_VERSION}; installing OctopusStudio-managed pnpm in the background.`,
    );
  } else {
    logger.info(
      "pnpm not found; installing OctopusStudio-managed pnpm in the background.",
    );
  }
  ...
  void getManagedPnpmInstallPromise()
    .then((managedPnpmVersion) => {
      ...
      logger.info(
        `Installed OctopusStudio-managed pnpm ${managedPnpmVersion}.`,
      );
    })
    .catch((error) => {
...
export function registerNodeHandlers() {
  ...
  createTypedHandler(systemContracts.getNodejsStatus, async () => {
    ...
    const devNodejsStatus = process.env.OCTOPUS_STUDIO_DEV_NODEJS_STATUS;
    ...
    if (IS_TEST_BUILD && mockNodeInstalled !== null) {
      ...
      if (settings.nodeRuntimePreference === "managed") {
        ...
        if (managedNodeInstalled && managedNodeVersion) {
          return {
            nodeVersion: managedNodeVersion,
            pnpmVersion: process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION ?? null,
            nodeDownloadUrl,
            source: "managed" as const,
            nodePath: getManagedNodeBinaryPath(),
            managedNodeInstalled,
  ...
  createTypedHandler(systemContracts.installManagedNode, async (event) => {
    ...
    try {
      ...
      writeSettings({
        ...
        // switches OctopusStudio to the newly installed managed runtime.
  ...
  createTypedHandler(systemContracts.installPnpm, async () => {
    try {
      const testInstallPnpmVersion = IS_TEST_BUILD
        ? process.env.OCTOPUS_STUDIO_TEST_INSTALL_PNPM_VERSION
        : undefined;
      if (testInstallPnpmVersion) {
        process.env.OCTOPUS_STUDIO_TEST_PNPM_VERSION = testInstallPnpmVersion;
        await reloadNodePath();
