...
const {
  ...
  mockStorePreCompactionMessages: vi.fn(
    async () => ".octopusStudio/chats/1/compaction-test.md",
  ),
...
vi.mock("@/ipc/utils/provider_options", () => ({
  OCTOPUS_STUDIO_INTERNAL_REQUEST_ID_HEADER: "x-octopus-studio-request-id",
  getAiHeaders: () => ({}),
  getProviderOptions: () => ({}),
}));
...
describe("performCompaction", () => {
  ...
  it("preserves the normal compaction path", async () => {
    ...
    expect(result).toMatchObject({
      ...
      backupPath: ".octopusStudio/chats/1/compaction-test.md",
    });
    await expect(loadSummaryMessages()).resolves.toHaveLength(1);
    await expect(loadChat()).resolves.toMatchObject({
      pendingCompaction: false,
      compactionBackupPath: ".octopusStudio/chats/1/compaction-test.md",
    });
  ...
  it("pins the benchmarked compaction model for OctopusStudio Pro users", async () => {
    settingsState.current = {
      selectedModel: { provider: "anthropic", name: "test-model" },
