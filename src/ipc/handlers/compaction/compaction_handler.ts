...
// reachable through the OctopusStudio Engine gateway.
...
export async function performCompaction(
  ...
  try {
    ...
    const compactionModel =
      storedSettings.enableOctopusStudioPro &&
      hasOctopusStudioProKey(storedSettings)
    ...
    // Store readable transcript backup in the app's .octopusStudio/chats/ directory
    ...
    const summaryResult = streamText({
      ...
      headers: {
        ...
        [OCTOPUS_STUDIO_INTERNAL_REQUEST_ID_HEADER]: octopusStudioRequestId,
      },
      providerOptions: getProviderOptions({
        octopusStudioAppId: 0,
        octopusStudioRequestId,
        octopusStudioDisableFiles: true,
        files: [],
    ...
    const compactionMessageContent = `<octopus-studio-compaction title="Conversation compacted" state="finished">
