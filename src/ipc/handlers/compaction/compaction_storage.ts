...
import { ensureOctopusStudioGitignored } from "@/ipc/handlers/gitignoreUtils";
...
/**
 * Get the backup directory for a specific chat within the app's .octopusStudio/chats/ directory.
 */
function getChatBackupDir(appPath: string, chatId: number): string {
  return path.join(appPath, ".octopusStudio", "chats", String(chatId));
}

/**
 * Transform octopus-studio-specific tool XML tags to shorter, LLM-friendly equivalents
 * and truncate large tool results for token efficiency.
 */
export function transformToolTags(content: string): string {
  // Transform <octopus-studio-mcp-tool-call> to <tool-use>. Tolerates extra attributes
  ...
  // Transform <octopus-studio-mcp-tool-result> to <tool-result> with truncation
...
export async function storePreCompactionMessages(
  ...
  // Ensure directory exists and .octopusStudio is gitignored
