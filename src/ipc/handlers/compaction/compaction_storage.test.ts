...
<octopus-studio-mcp-tool-call server="filesystem" tool="read_file">
{"path": "/src/index.ts"}
    ...
    expect(result).not.toContain("octopus-studio-mcp-tool-call");
  });

  it("transforms tool-call tags that carry extra attributes", () => {
    const content = `<octopus-studio-mcp-tool-call server="filesystem" tool="read_file" auto-approved-reason="Reads a project file.">
{"path": "/src/index.ts"}
</octopus-studio-mcp-tool-call>`;
  ...
  it("transforms tool-result tags and includes char count", () => {
    const content = `<octopus-studio-mcp-tool-result server="filesystem" tool="read_file">
short result
  ...
  it("transforms tags that carry a call-id attribute", () => {
    const content = `<octopus-studio-mcp-tool-call server="filesystem" tool="read_file" call-id="abc-123">
{"path": "/a.ts"}
</octopus-studio-mcp-tool-call>
<octopus-studio-mcp-tool-result server="filesystem" tool="read_file" call-id="abc-123">
ok
</octopus-studio-mcp-tool-result>`;
    ...
    expect(result).not.toContain("octopus-studio-mcp-tool-call");
    expect(result).not.toContain("octopus-studio-mcp-tool-result");
...
${longContent}
  ...
  it("does not truncate results at exactly the limit", () => {
    ...
    const content = `<octopus-studio-mcp-tool-result server="fs" tool="read">
${exactContent}
</octopus-studio-mcp-tool-result>`;
...
<octopus-studio-mcp-tool-call server="fs" tool="read_file">
{"path": "/a.ts"}
</octopus-studio-mcp-tool-call>
<octopus-studio-mcp-tool-result server="fs" tool="read_file">
contents of a
</octopus-studio-mcp-tool-result>
<octopus-studio-mcp-tool-call server="fs" tool="read_file">
{"path": "/b.ts"}
...
contents of b
    ...
    expect(result).not.toContain("octopus-studio-mcp");
  });
...
<octopus-studio-mcp-tool-call server="fs" tool="write_file">
{"path": "/a.ts", "content": "new"}
</octopus-studio-mcp-tool-call>
<octopus-studio-mcp-tool-result server="fs" tool="write_file">
success
</octopus-studio-mcp-tool-result>`;
...
describe("formatAsTranscript", () => {
  ...
  it("handles multi-turn conversation with interleaved tool use", () => {
    ...
    // No original octopus-studio tags remain
    expect(result).not.toContain("octopus-studio-mcp");
  ...
  it("handles assistant messages mixing multiple servers", () => {
    const messages: CompactionMessage[] = [
      { role: "user", content: "Search and then write a file" },
      {
        ...
        content: [
          "I'll search first.",
          '<octopus-studio-mcp-tool-call server="search" tool="grep">',
          '{"pattern": "TODO"}',
          "</octopus-studio-mcp-tool-call>",
          '<octopus-studio-mcp-tool-result server="search" tool="grep">',
          "src/a.ts:3: // TODO fix",
          "</octopus-studio-mcp-tool-result>",
          "Found one. Now writing the fix.",
          '<octopus-studio-mcp-tool-call server="filesystem" tool="write_file">',
          '{"path": "/src/a.ts", "content": "fixed"}',
          "</octopus-studio-mcp-tool-call>",
          '<octopus-studio-mcp-tool-result server="filesystem" tool="write_file">',
          "ok",
          "</octopus-studio-mcp-tool-result>",
        ].join("\n"),
      },
