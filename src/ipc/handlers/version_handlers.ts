  ...
  if (preserveDirtyTree && !(await isGitStatusClean({ path: appPath }))) {
    ...
    logger.log(
      `Preserving dirty tree in a checkpoint commit before restoring app ${appId}. ` +
        ...
        ". OctopusStudio-managed runtime files may also be included in the checkpoint.",
    );
...
export function registerVersionHandlers() {
  ...
  createTypedHandler(versionContracts.getCurrentBranch, async (_, params) => {
    ...
    // Chat projects (and any app without a git repo) have no current branch.
    if (!fs.existsSync(path.join(appPath, ".git"))) {
  ...
  createTypedHandler(versionContracts.getVersionChanges, async (_, params) => {
    ...
    try {
      ...
      // Preserve the original error kind for OctopusStudioErrors thrown by inner
      // functions; only wrap unexpected (non-OctopusStudio) failures as External.
        ...
        if (restoreCodebase) {
          ...
          try {
            ...
            if (
              ...
              error.kind === OctopusStudioErrorKind.NotFound
            ) {
              return {
                status: "warn" as const,
                warningMessage:
    ...
    try {
      ...
      return await appOperationCoordinator.run(
        ...
        async () => {
          ...
          const latestAppPath = getOctopusStudioAppPath(latestApp.path);
              ...
              tx.insert(messages)
                .values(
                  ...
                  messagesBeforeInInsertOrder.map((m) => ({
                    ...
                    outputTokensUsed: m.outputTokensUsed,
                    model: m.model,
  ...
  const checkoutVersionHandler = async (
    ...
    return appOperationCoordinator.run(
      ...
      async () => {
        ...
        const fullAppPath = getOctopusStudioAppPath(app.path);
        await gitCheckout({
          path: fullAppPath,
          ref: gitRef,
        });
