...
/**
 * Ensures `.octopusStudio/` is listed in the project's `.gitignore`.
 * Creates `.gitignore` if it doesn't exist.
 */
export async function ensureOctopusStudioGitignored(
  appPath: string,
): Promise<void> {
  await ensureGitignored(appPath, [".octopusStudio/"]);
}
