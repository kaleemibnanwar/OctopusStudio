...
describe("mcp catalog handlers", () => {
  ...
  it("lists catalog entries with already-added slugs", async () => {
    ...
    // Entries are the bundled local catalog merged with the (mocked) remote
    // fixture [FIGMA, CONTEXT7]; FIGMA's slug collides with a local entry
    // (the remote one wins), so only CONTEXT7 is net-new.
    expect(result.entries).toHaveLength(LOCAL_MCP_CATALOG.length + 1);
  ...
  it("still adds a bundled local-catalog entry when the remote catalog is unreachable", async () => {
    // The whole point of the bundled local catalog: a remote fetch failure
    // (mocked here as an empty array) no longer blocks adding a plugin that
    // ships with the app.
    catalogEntries = [];
    const created = await invoke<{ catalogSlug: string | null }>(
      "mcp:add-from-catalog",
    ...
    expect(created.catalogSlug).toBe("canva");
  });

  it("rejects a slug that exists in neither catalog", async () => {
    catalogEntries = [];
    await expect(
      invoke("mcp:add-from-catalog", { slug: "not-in-catalog" }),
    ).rejects.toThrow(/Unknown catalog entry/);
