...
async function getSystemDebugInfo({
  ...
  // Get OctopusStudio version from package.json
  const packageJsonPath = path.resolve(__dirname, "..", "..", "package.json");
  let octopusStudioVersion = "unknown";
  try {
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"));
    octopusStudioVersion = packageJson.version;
  } catch (err) {
...
function sanitizeSettingsForDebug(
  ...
  return {
    ...
    enableOctopusStudioPro: settings.enableOctopusStudioPro ?? null,
