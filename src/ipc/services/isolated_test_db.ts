  ...
  const teardown = async (
    ...
    if (envModified) {
      try {
        ...
        emit(
          "Warning: Octopus Studio couldn't restore your real database settings, so the temporary Neon branch was kept tracked for retry. Restore .env.local before running more tests.\n",
          "setup",
        );
      }
      if (envRestored && !options.skipRestart) {
        try {
          ...
          emit(
            "Warning: Octopus Studio restored your real database settings, but couldn't restart the preview. Restart the app manually before continuing.\n",
            "setup",
          );
        }
      }
  ...
  try {
    ...
    if (branch.neonAuthBaseUrl) {
      try {
        ...
        testCredentials = {
          OCTOPUS_STUDIO_TEST_USER_EMAIL: account.email,
          OCTOPUS_STUDIO_TEST_USER_PASSWORD: account.password,
        };
        authSetup = {
...
async function prepareSupabaseTestUserIsolation({
  ...
  try {
    ...
    const testCredentials: Record<string, string> = {
      OCTOPUS_STUDIO_TEST_USER_EMAIL: testUser.email,
      OCTOPUS_STUDIO_TEST_USER_PASSWORD: testUser.password,
      OCTOPUS_STUDIO_TEST_SUPABASE_URL: testUser.projectUrl,
    };
    let authSetup: IsolationAuthSetup | undefined;
    if (anonKey) {
      testCredentials.OCTOPUS_STUDIO_TEST_SUPABASE_ANON_KEY = anonKey;
      authSetup = {
        mode: "supabase-password",
...
function buildRlsWarning(rls: {
  ...
  if (rls.unverified) {
    return "Tests ran as an isolated test user, but Octopus Studio couldn't verify Row-Level Security — some real data may be reachable.";
  }
