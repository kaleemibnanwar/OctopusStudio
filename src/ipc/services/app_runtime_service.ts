...
export function formatCloudSandboxError(error: unknown) {
  ...
  switch (error.code) {
    case "sandbox_pro_required":
      return "OctopusStudio Pro is required to use cloud sandboxes.";
    ...
    case "sandbox_billing_unavailable":
      return "OctopusStudio couldn’t verify sandbox billing right now. Please try again.";
    ...
    default:
      ...
      if (error.status === 401 || error.status === 403) {
        return "OctopusStudio couldn’t authorize the cloud sandbox request. Please try again.";
      }
      if (error.status === 429) {
        return "OctopusStudio is rate limiting cloud sandbox requests right now. Please try again.";
      }
      if (typeof error.status === "number" && error.status >= 500) {
        return "OctopusStudio’s cloud sandbox service is temporarily unavailable. Please try again.";
      }
...
// OctopusStudio (CI, deploys, teammates), so surface it in the console the user is
...
function notifyPnpmVersionMigrationAvailable({
  ...
  try {
    ...
    if (!pnpmVersionMigrationNotifiedAppIds.has(appId)) {
      output.send({
        type: "stdout",
        message: `This pnpm app needs a pnpm ${managedMajor} migration (pre-9 lockfile or pnpm <= 8 pin). OctopusStudio already runs pnpm ${managedMajor}, so deploys, CI, and teammates' installs can drift without the matching project pin. Open App Details -> App Upgrades and apply "Migrate to pnpm ${managedMajor}".`,
        appId,
      });
    ...
    output.send({
      ...
      message: `This app pins an older pnpm that can't read the lockfile OctopusStudio writes. Migrate to pnpm ${managedMajor} so CI, deploys, and teammates can install it reliably.`,
      appId,
    });
  } catch (error) {
...
export function emitProxyServerStarted({
  ...
  output.send({
    type: "stdout",
    message: `[octopus-studio-proxy-server]started=[${proxyUrl}] original=[${originalUrl}] mode=[${mode}]`,
    appId,
    invocationRef,
  });
}
...
export async function ensureProxyForRunningApp({
  ...
  // already taken (by a foreign service, or another OctopusStudio app in the rare 10k
  ...
  const proxyWorker = await startProxy(originalUrl, {
    ...
    onError: (error) => {
      ...
      output.send({
        type: "stderr",
        message: `[octopus-studio-proxy-server] ${error.message}`,
        appId,
      });
    },
    fixedHeaders:
...
`,
  ...
  const customPreviewUrl = previewUrl?.trim() || undefined;
  if (customPreviewUrl) {
    // A configured preview URL (e.g. Django's `http://127.0.0.1:10202`)
    // doesn't wait for stdout detection: the backend may print an address the
    // detector can't parse, so start the proxy up front.
    await ensureProxyForRunningApp({
      appId,
      output,
      originalUrl: customPreviewUrl,
      mode: "host",
      invocationRef,
    });
  }
  ...
  listenToProcess({
    ...
    output,
    invocationRef,
    preferredOriginalUrl: customPreviewUrl,
      ...
      command.isCustom && !ignoredBuildsSelfHealAttempted
        ? async (processOutput) => {
            ...
            output.send({
              type: "stdout",
              message:
                "pnpm blocked dependency build scripts. OctopusStudio recorded the decision in pnpm-workspace.yaml and is reinstalling...",
              appId,
            });
...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    if (isInputRequest) {
      ...
      if (urlMatch) {
        const originalUrl = preferredOriginalUrl ?? urlMatch[1];
...
async function executeAppInDocker({
  ...
  const containerName = `octopus-studio-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.octopusStudio");
...
`;
  ...
  const buildProcess = spawn(
    ...
    [
      "build",
      "-f",
      "Dockerfile.octopusStudio",
      "-t",
      `octopus-studio-app-${appId}`,
      ".",
    ],
    {
      cwd: appPath,
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `octopus-studio-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
