...
function reconcileRestore(
  ...
  return {
    ...
    error: {
      message:
        "Octopus Studio restarted during an interrupted version restore. Inspect and repair the repository before continuing.",
    },
      ...
      state =
        !ownsCheckout || reachedSafeBranch
          ...
          : {
              ...
              error: {
                message:
                  "Octopus Studio restarted while switching branches. Return to the original branch before continuing.",
              },
            };
    } else if (
      current.type === "restoring" ||
      ...
      state =
        event.branch === current.session.originBranch
          ...
          : {
              ...
              error: {
                message:
                  "Octopus Studio restarted during a version checkout. Return to the original branch before continuing.",
              },
            };
    }
    return {
...
async function authorizeApp(appId: number): Promise<void> {
  ...
  if (!(await appExists(appId))) {
    throw new OctopusStudioError("App not found", OctopusStudioErrorKind.Auth);
  }
}
