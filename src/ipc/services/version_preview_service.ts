  ...
  resolveOriginBranch(appId: number): Promise<{ branch: string | null }> {
    ...
    return this.track(
      ...
      (async () => {
        ...
        // Chat projects (and any app without a git repo) have no branch.
  ...
  reconcile(appId: number): Promise<{
    ...
    return this.track(
      ...
      (async () => {
        ...
        const appPath = getOctopusStudioAppPath(app.path);
        // Chat projects (and any app without a git repo) have nothing to
        // reconcile; report a clean, empty state instead of throwing.
        if (!fs.existsSync(path.join(appPath, ".git"))) {
          return { branch: null, headOid: null, isClean: true };
        }
