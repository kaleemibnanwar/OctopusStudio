...
export interface TempTestUser {
  ...
  userId: string;
  /** Login email, of the form `octopus-studio-test+<appId>-<ts>@octopusStudio.test`. */
  email: string;
 ...
 * next launch.
 *
 * Throws `OctopusStudioError` if the app isn't connected to a Supabase project/org.
 */
export async function createTempTestUser(
  ...
  const email = `octopus-studio-test+${appData.id}-${Date.now()}@octopusStudio.test`;
  const password = crypto.randomBytes(24).toString("base64url");

  if (IS_TEST_BUILD) {
    // Don't hit the network in OctopusStudio's own E2E build (fake Supabase project).
  ...
  const response = await fetchWithRetry(
    `${projectUrl}/auth/v1/admin/users`,
    {
      ...
      body: JSON.stringify({
        ...
        email_confirm: true,
        app_metadata: {
          octopus_studio_test: true,
          octopus_studio_app_id: appData.id,
        },
      }),
    },
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && /legacy api keys/i.test(detail)) {
      throw new OctopusStudioError(
        "This Supabase project has its legacy API keys (anon, service_role) disabled, and Octopus Studio couldn't find a secret key to use instead. Create a secret key in Supabase under Settings → API Keys, then run the tests again.",
        OctopusStudioErrorKind.Precondition,
      );
    }
    ...
    if (
      ...
      throw new OctopusStudioError(
        `Supabase rejected the ${adminKey.isLegacyJwt ? "legacy service_role" : "secret"} key Octopus Studio used to create the test user (${response.status}). ${detail}`,
        OctopusStudioErrorKind.External,
      );
    }
    throw new OctopusStudioError(
...
WHERE table_schema = 'public'
  AND column_name IN (${OWNER_COLUMNS.map((c) => `'${c}'`).join(", ")});`;
    ...
    for (const row of rows) {
      ...
      // Belt-and-suspenders: the static `$octopus_studio_cleanup$` dollar-quote tag below
      ...
      try {
        ...
        await executeSupabaseSql({
          supabaseProjectId: projectId,
          query: `DO $octopus_studio_cleanup$ BEGIN EXECUTE format('DELETE FROM public.%I WHERE %I = %L', '${table}', '${column}', '${userId}'); END $octopus_studio_cleanup$;`,
          organizationSlug,
        });
      } catch (error) {
        logger.warn(
