  ...
  if (has("pnpm-lock.yaml")) {
    return {
      ...
      args: [
        ...PNPM_INSTALL_POLICY_ARGS,
        "add",
        // OctopusStudio can generate a pnpm-workspace.yaml (for install policy), which
...
export const TEST_BASE_URL_ENV = "OCTOPUS_STUDIO_TEST_BASE_URL";

/**
 * OctopusStudio's own Playwright config. Deliberately NOT `playwright.config.ts`: that
 ...
 * name and is selected explicitly with `--config` on every run, so OctopusStudio never
 * has to rename, back up, or overwrite a config it doesn't own.
 */
export const OCTOPUS_STUDIO_CONFIG_FILENAME =
  "playwright-octopusStudio.config.ts";
 ...
 */
const OCTOPUS_STUDIO_CONFIG_SENTINEL = "Generated by OctopusStudio";

/** A browser channel that drives a system-installed browser, or bundled. */
export type BrowserChannel = "chrome" | "msedge";

/**
 * Builds the `playwright-octopusStudio.config.ts` source. When `channel` is set, the
...
// ${OCTOPUS_STUDIO_CONFIG_SENTINEL}. The dev server is started separately by OctopusStudio's
...
/** Absolute path of OctopusStudio's own config. The user's config is never touched. */
function octopusStudioConfigPath(appPath: string): string {
  return path.join(appPath, OCTOPUS_STUDIO_CONFIG_FILENAME);
}

function hasOctopusStudioPlaywrightConfig(appPath: string): boolean {
  return fs.existsSync(octopusStudioConfigPath(appPath));
}

/** Read OctopusStudio's config source, or null if we haven't written one yet. */
function readOctopusStudioConfigText(appPath: string): string | null {
  const file = octopusStudioConfigPath(appPath);
...
/** True when OctopusStudio's config drives a system browser via `channel`. */
function configUsesChannel(appPath: string): boolean {
  const text = readOctopusStudioConfigText(appPath);
  return (
    text != null &&
...
/** True when OctopusStudio's config is still pure template output (safe to rewrite). */
function isOctopusStudioGeneratedConfig(appPath: string): boolean {
  const text = readOctopusStudioConfigText(appPath);
  return text != null && text.includes(OCTOPUS_STUDIO_CONFIG_SENTINEL);
}
...
function writePlaywrightConfig(
  ...
  fs.writeFileSync(
    octopusStudioConfigPath(appPath),
    buildPlaywrightConfig(channel),
  );
  logger.info(
    `Wrote ${OCTOPUS_STUDIO_CONFIG_FILENAME} (${channel ? `channel: ${channel}` : "bundled chromium"})`,
  );
}

/**
 * Rewrite a OctopusStudio-generated config that still points `testDir` at the legacy
...
function migrateConfigTestDir(appPath: string): void {
  if (!isOctopusStudioGeneratedConfig(appPath)) return;
  const text = readOctopusStudioConfigText(appPath);
  ...
  if (text && text.includes(legacy)) {
    fs.writeFileSync(
      octopusStudioConfigPath(appPath),
      text.replace(legacy, current),
    );
    logger.info(
      `Migrated ${OCTOPUS_STUDIO_CONFIG_FILENAME} testDir to ./${E2E_TEST_DIR}`,
    );
  }
}
...
function ensureTestScript(appPath: string): void {
  ...
  try {
    ...
    if (!pkg.scripts.test) {
      // Point at OctopusStudio's config explicitly — Playwright only auto-resolves
      // `playwright.config.ts`, which is the app's own file, not ours.
      pkg.scripts.test = `playwright test --config ${OCTOPUS_STUDIO_CONFIG_FILENAME}`;
      ...
      // Migrate the old OctopusStudio-generated bare script, but only when the app has
      ...
      // global setup, and break `npm test` outside OctopusStudio. With no config of
      // their own, the bare script can only be OctopusStudio's older template output.
      // (Scripts with flags, env vars, or extra commands are never touched.)
      pkg.scripts.test = `playwright test --config ${OCTOPUS_STUDIO_CONFIG_FILENAME}`;
      fs.writeFileSync(packageJsonPath, `${JSON.stringify(pkg, null, 2)}\n`);
      logger.info("Updated OctopusStudio test script to use explicit config");
    }
  } catch (err) {
 ...
 *
 * Throws a OctopusStudioError (kind External) on install failure — typically offline,
...
export async function ensurePlaywrightBootstrap({
  ...
  if (!packageInstalled) {
    ...
    const installDep = await spawnStreaming({
      ...
      // OctopusStudio-managed package-manager paths (executeAddDependency, runtime).
  ...
  // OctopusStudio's config lives under its own name and every run selects it with
  ...
  if (!hasOctopusStudioPlaywrightConfig(appPath)) {
    ...
    if (
      !usesChannel &&
      detectedChannel &&
      isOctopusStudioGeneratedConfig(appPath)
    ) {
  ...
  // Bring an older OctopusStudio config's testDir up to date (./tests -> ./e2e-tests) so
