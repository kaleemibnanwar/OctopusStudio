...
describe("parseFilesFromMessage", () => {
  describe("octopus-studio-read tags", () => {
    it("should parse a single octopus-studio-read tag", () => {
      const input =
        '<octopus-studio-read path="src/components/Button.tsx"></octopus-studio-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    });

    it("should parse multiple octopus-studio-read tags", () => {
      const input = `
        <octopus-studio-read path="src/components/Button.tsx"></octopus-studio-read>
        <octopus-studio-read path="src/utils/helpers.ts"></octopus-studio-read>
        <octopus-studio-read path="src/styles/main.css"></octopus-studio-read>
      `;
      const result = parseFilesFromMessage(input);
    ...
    it("should trim whitespace from file paths in octopus-studio-read tags", () => {
      const input =
        '<octopus-studio-read path="  src/components/Button.tsx  "></octopus-studio-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    ...
    it("should skip empty path attributes", () => {
      const input = `
        <octopus-studio-read path="src/components/Button.tsx"></octopus-studio-read>
        <octopus-studio-read path=""></octopus-studio-read>
        <octopus-studio-read path="src/utils/helpers.ts"></octopus-studio-read>
      `;
      const result = parseFilesFromMessage(input);
      expect(result).toEqual([
    ...
    it("should handle file paths with special characters", () => {
      const input =
        '<octopus-studio-read path="src/components/@special/Button-v2.tsx"></octopus-studio-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/@special/Button-v2.tsx"]);
    });
  });

  describe("octopus-studio-code-search-result tags", () => {
    it("should parse a single file from octopus-studio-code-search-result", () => {
      const input = `<octopus-studio-code-search-result>
src/components/Button.tsx
</octopus-studio-code-search-result>`;
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    });

    it("should parse multiple files from octopus-studio-code-search-result", () => {
      const input = `<octopus-studio-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should skip empty lines in octopus-studio-code-search-result", () => {
      const input = `<octopus-studio-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should handle multiple octopus-studio-code-search-result tags", () => {
      const input = `<octopus-studio-code-search-result>
src/components/Button.tsx
src/components/Input.tsx
</octopus-studio-code-search-result>
...
src/styles/main.css
  ...
  describe("mixed tags", () => {
    it("should parse both octopus-studio-read and octopus-studio-code-search-result tags", () => {
      const input = `
<octopus-studio-read path="src/config/app.ts"></octopus-studio-read>
...
<octopus-studio-read path="src/components/Header.tsx"></octopus-studio-read>
...
<octopus-studio-read path="src/config/site.ts"></octopus-studio-read>
...
`;
  ...
  describe("edge cases", () => {
    ...
    it("should handle malformed tags gracefully", () => {
      const input = `
<octopus-studio-read path="src/file1.ts"
<octopus-studio-code-search-result>
src/file2.ts
`;
      const result = parseFilesFromMessage(input);
    ...
    it("should handle nested angle brackets in file paths", () => {
      const input =
        '<octopus-studio-read path="src/components/Generic<T>.tsx"></octopus-studio-read>';
      const result = parseFilesFromMessage(input);
...
SRC/COMPONENTS/BUTTON.TSX
    ...
    it("should handle very long file paths", () => {
      ...
      const input = `<octopus-studio-read path="${longPath}"></octopus-studio-read>`;
      const result = parseFilesFromMessage(input);
...
describe("processChatMessagesWithVersionedFiles", () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'I found this file: <octopus-studio-read path="src/old.ts"></octopus-studio-read>',
          providerOptions: {
      ...
      const chatMessages: ModelMessage[] = [
        {
          ...
          content: [
            {
              type: "text",
              text: 'Here is the file: <octopus-studio-read path="src/array.ts"></octopus-studio-read>',
            },
            {
              type: "text",
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "user",
          content:
            'Check this: <octopus-studio-read path="src/user-file.ts"></octopus-studio-read>',
        },
      ];
      const appPath = "/test/app";
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'File here: <octopus-studio-read path="src/no-commit.ts"></octopus-studio-read>',
          // No providerOptions
        },
        {
          role: "assistant",
          content:
            'Another file: <octopus-studio-read path="src/no-commit2.ts"></octopus-studio-read>',
          providerOptions: {
            // octopus-studio-engine not set
          },
        },
      ];
      const appPath = "/test/app";
  ...
  describe("parsing multiple file paths", () => {
    it("should process multiple files from octopus-studio-code-search-result", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `<octopus-studio-code-search-result>
src/file1.ts
src/file2.ts
</octopus-studio-code-search-result>`,
          providerOptions: {
    ...
    it("should process mixed octopus-studio-read and octopus-studio-code-search-result tags", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `
<octopus-studio-read path="src/file1.ts"></octopus-studio-read>
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Missing file: <octopus-studio-read path="src/missing.ts"></octopus-studio-read>',
          providerOptions: {
            "octopus-studio-engine": {
              sourceCommitHash: "commit1",
            },
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Error file: <octopus-studio-read path="src/error.ts"></octopus-studio-read>',
          providerOptions: {
            "octopus-studio-engine": {
              sourceCommitHash: "commit1",
            },
    ...
    it("should process multiple messages with different commits", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        ...
        },
        {
          role: "assistant",
          content:
            'Here it is: <octopus-studio-read path="src/file1.ts"></octopus-studio-read>',
          providerOptions: {
            "octopus-studio-engine": {
              sourceCommitHash: "commit1",
            },
        ...
        },
        {
          role: "assistant",
          content:
            'Here it is again: <octopus-studio-read path="src/file1.ts"></octopus-studio-read>',
          providerOptions: {
            "octopus-studio-engine": {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Old version: <octopus-studio-read path="src/old.ts"></octopus-studio-read>',
          providerOptions: {
            "octopus-studio-engine": {
              sourceCommitHash: "abc123",
            },
