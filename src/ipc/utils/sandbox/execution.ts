...
import { getOctopusStudioMediaDir } from "@/ipc/utils/media_path_utils";
import {
...
export async function executeSandboxScriptInProcess(
  ...
  let wallClockTimeoutError: OctopusStudioError | undefined;
  ...
  try {
    ...
    const context = new ExecutionContext({
      ...
      snapshotKey: `octopus-studio-sandbox:${params.appPath}`,
    });
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        wallClockTimeout = setTimeout(() => {
          wallClockTimeoutError = new OctopusStudioError(
