...
describe("sandbox capabilities", () => {
  ...
  beforeEach(async () => {
    appPath = await fs.mkdtemp(
      path.join(os.tmpdir(), "octopus-studio-sandbox-"),
    );
  ...
  it("rejects sandbox writes that escape the app through symlinks", async () => {
    const outsideDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "octopus-studio-outside-"),
    );
    try {
  ...
  it("allows reading and listing under .octopusStudio/", async () => {
    await expect(
      sandboxReadFile(appPath, ".octopusStudio/media/stored-log.txt"),
    ).resolves.toBe("line1\nline2\n");
    await expect(
      sandboxReadFile(
        appPath,
        ".octopusStudio/media/attachments-manifest.json",
      ),
    ).resolves.toContain("server.log");
    await expect(
      sandboxListFiles(appPath, ".octopusStudio/media"),
    ).resolves.toEqual(
      expect.arrayContaining([
        ".octopusStudio/media/attachments-manifest.json",
        ".octopusStudio/media/stored-log.txt",
      ]),
    );
  });
