...
import { ECONOMY_MAX_OUTPUT_TOKENS } from "@/constants/settings_constants";
...
/**
 * Resolve the max output tokens for a stream, honoring economy mode's hard cap.
 * When economy mode is on, output is clamped to ECONOMY_MAX_OUTPUT_TOKENS even
 * if the model allows more.
 */
export async function getMaxOutputTokens(
  model: LargeLanguageModel,
  economyMode: boolean,
): Promise<number | undefined> {
  const max = await getMaxTokens(model);
  if (!economyMode) {
    return max;
  }
  if (max === undefined) {
  ...
  return Math.min(max, ECONOMY_MAX_OUTPUT_TOKENS);
}
