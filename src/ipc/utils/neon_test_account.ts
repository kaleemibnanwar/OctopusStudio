 ...
 *
 * Throws `OctopusStudioError` when signup is rejected, and when it succeeds but produces
...
export async function createNeonTestAccount({
  ...
  const email = `octopus-studio-test+${appId}-${Date.now()}@octopusStudio.test`;
  const password = crypto.randomBytes(24).toString("base64url");

  if (IS_TEST_BUILD) {
    // Don't hit the network in OctopusStudio's own E2E build.
