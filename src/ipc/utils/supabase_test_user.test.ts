...
describe("createTempTestUser", () => {
  it("creates a confirmed admin user and persists the id immediately", async () => {
    ...
    expect(body.app_metadata).toMatchObject({ octopus_studio_test: true });
    ...
    expect(result.email).toMatch(
      /^octopus-studio-test\+7-\d+@octopus-studio\.test$/,
    );
  ...
  it("explains how to fix a project whose legacy keys are disabled", async () => {
    ...
    // User-fixable setup problem, not a OctopusStudio failure worth reporting.
    expect(error.kind).toBe(OctopusStudioErrorKind.Precondition);
  });
});
...
describe("deleteTempTestUser", () => {
  it("sweeps owned rows, deletes the user, and clears the column on success", async () => {
    ...
    // a `DO $octopus_studio_cleanup$ ... EXECUTE format('DELETE FROM ...') ...` block, so match on
    ...
    expect(deleteCall?.[0].query).toContain("DO $octopus_studio_cleanup$");
