...
async function parseMultipartUpload(
  ...
  const request = new Request("https://octopusStudio.test/upload", {
    method: "POST",
...
describe("cloud_sandbox_provider incremental sync", () => {
  ...
  beforeEach(async () => {
    ...
    appPath = await fs.mkdtemp(
      path.join(os.tmpdir(), "octopus-studio-cloud-sync-"),
    );
...
describe("cloud_sandbox_provider response validation", () => {
  ...
  it("uses status-based messages instead of surfacing raw error bodies", async () => {
    ...
    expect(error).toMatchObject({
      message:
        "OctopusStudio’s cloud sandbox service is temporarily unavailable. Please try again.",
      status: 503,
    });
  });
});
