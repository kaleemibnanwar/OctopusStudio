...
// The OctopusStudio Engine is backed by LiteLLM using the
// OpenAI-compatible chat completions API. This means
// we need to configure thinking differently depending
// on whether user is enabling OctopusStudio Pro (uses engine)
...
export function getExtraProviderOptionsForEngine(
  ...
  if (providerId === "openai") {
    // OpenAI uses the same provider options because the OctopusStudio Engine
