...
export function getEnvFilePath({ appPath }: { appPath: string }): string {
  return path.join(getOctopusStudioAppPath(appPath), ENV_FILE_NAME);
}
...
export async function updateDbPushEnvVar({
  ...
  try {
    ...
    // Update or add OCTOPUS_STUDIO_DISABLE_DB_PUSH
    const existingVar = envVars.find(
      (envVar) => envVar.key === "OCTOPUS_STUDIO_DISABLE_DB_PUSH",
    );
    if (existingVar) {
      existingVar.value = disabled ? "true" : "false";
    } else {
      envVars.push({
        key: "OCTOPUS_STUDIO_DISABLE_DB_PUSH",
