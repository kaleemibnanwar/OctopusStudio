...
function renderCallbackPage(options: {
  ...
  const returnUrl = "octopusStudio://mcp-oauth-return";
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1" />
<title>${safeTitle} — OctopusStudio</title>
<style>
...
<body>
  <div class="card">
    ...
    ${
      isSuccess
        ? `<a class="btn" href="${returnUrl}">Open OctopusStudio</a>
    <script>
      setTimeout(function () { window.location.href = ${JSON.stringify(returnUrl)}; }, 500);
    </script>`
        : `<p class="muted">You can close this window and return to OctopusStudio.</p>`
    }
  </div>
</body>
</html>`;
      ...
      if (!claim.claimed) {
        ...
        res.writeHead(400, { "Content-Type": "text/html" }).end(
          renderCallbackPage({
            ...
            message:
              "The browser's response didn't match the request OctopusStudio started. You can close this window.",
          }),
        );
        return;
      }
      ...
      if (code) {
        res.writeHead(200, { "Content-Type": "text/html" }).end(
          renderCallbackPage({
            ...
            message: "You can close this tab and return to OctopusStudio.",
          }),
        );
        return;
      }
...
async function prepareAndRunOAuthFlow(
  ...
  const provider = new OctopusStudioOAuthClientProvider({
    serverId: server.id,
    callbackPort,
    scope,
