...
describe("agent Git utilities", () => {
  ...
  it("includes pnpm workspace patches while omitting OctopusStudio-internal patches", async () => {
    await fs.promises.mkdir(path.join(repo, ".octopusStudio"));
    await fs.promises.writeFile(
      path.join(repo, "pnpm-workspace.yaml"),
      'packages:\n  - "app"\n',
    );
    await fs.promises.writeFile(
      path.join(repo, ".octopusStudio", "internal.json"),
      '{"version":1}\n',
    );
    await git(
      repo,
