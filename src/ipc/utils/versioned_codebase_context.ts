...
interface OctopusStudioEngineProviderOptions {
 ...
 * Extracts files from <octopus-studio-read> and <octopus-studio-code-search-result> tags.
 */
export function parseFilesFromMessage(content: string): string[] {
  ...
  // Parse <octopus-studio-read path="$filePath"></octopus-studio-read>
  const octopusStudioReadRegex =
    /<octopus-studio-read\s+path="([^"]+)"[^>]*><\/octopus-studio-read>/gs;
  let match: RegExpExecArray | null;
  while ((match = octopusStudioReadRegex.exec(content)) !== null) {
  ...
  // Parse <octopus-studio-code-search-result>...</octopus-studio-code-search-result>
  const codeSearchRegex =
    /<octopus-studio-code-search-result>(.*?)<\/octopus-studio-code-search-result>/gs;
...
export async function processChatMessagesWithVersionedFiles({
  ...
  for (
    ...
    ] as unknown as OctopusStudioEngineProviderOptions;
