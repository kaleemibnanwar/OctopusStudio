...
import {
  createOctopusStudioEngine,
  type OctopusStudioEngineProvider,
...
const AUTO_OCTOPUS_STUDIO_PRO_MODEL_ALIASES = [
  "octopusStudio/auto/openai",
  "octopusStudio/auto/anthropic",
  "octopusStudio/auto/google",
] as const;

const AUTO_MODEL_ALIASES = [
  ...AUTO_OCTOPUS_STUDIO_PRO_MODEL_ALIASES,
  "octopusStudio/auto/openrouter",
] as const;
...
export async function getModelClient(
  ...
  const octopusStudioApiKey = settings.enableOctopusStudioPro
    ? getProviderApiKeyForRequest(
        settings.providerSettings?.auto?.apiKey?.value,
        "OctopusStudio",
      )
    : undefined;
  const isOctopusStudioProEnabledForRequest = Boolean(
    octopusStudioApiKey && settings.enableOctopusStudioPro,
  );
  ...
  if (
    isFreeProModel(model) &&
    (!settings.enableOctopusStudioPro || !octopusStudioApiKey)
  ) {
    throw new OctopusStudioError(
      "OctopusStudio Free requires an active OctopusStudio Pro API key. Switch to another model or enable OctopusStudio Pro.",
      OctopusStudioErrorKind.Auth,
    );
  }

  // Handle OctopusStudio Pro override
  if (isOctopusStudioProEnabledForRequest) {
    const octopusStudioEngineUrl = process.env.OCTOPUS_STUDIO_ENGINE_URL;
    // Check if the selected provider supports OctopusStudio Pro (has a gateway prefix) OR
    // we're using local engine.
    // IMPORTANT: some providers like OpenAI have an empty string gateway prefix,
    // so we do a nullish and not a truthy check here.
    if (providerConfig.gatewayPrefix != null || octopusStudioEngineUrl) {
      const enableSmartFilesContext = settings.enableProSmartFilesContextMode;
      const provider = createOctopusStudioEngine({
        apiKey: octopusStudioApiKey,
      ...
      logger.debug(
        `\x1b[1;97;44m Using OctopusStudio Pro API key for model: ${model.name} \x1b[0m`,
      );

      logger.debug(
        `\x1b[1;30;42m Using OctopusStudio Pro engine: ${octopusStudioEngineUrl ?? "<prod>"} \x1b[0m`,
      );
      ...
      logger.warn(
        `OctopusStudio Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
    ...
    for (const autoModelAlias of AUTO_MODEL_ALIASES) {
      ...
      if (apiKey) {
        ...
        if (
          ...
          !isOctopusStudioProEnabledForRequest &&
          providerInfo
        ) {
          return {
...
async function getProModelClient({
  ...
  model: LargeLanguageModel;
  settings: UserSettings;
  provider: OctopusStudioEngineProvider;
  modelId: string;
}): Promise<ModelClient> {
  ...
  if (
    ...
    const fallbackModels = await Promise.all(
      AUTO_OCTOPUS_STUDIO_PRO_MODEL_ALIASES.map(async (aliasId) => {
...
export function getProviderApiKeyForRequest(
