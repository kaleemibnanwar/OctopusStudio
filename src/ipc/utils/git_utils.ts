...
function isUserVisibleGitPath(filePath: string) {
  return (
    !filePath.startsWith(".octopusStudio/") &&
 ...
 *
 * Issue: https://github.com/octopus-studio-sh/octopusStudio/issues/2194
 ...
 *
 * Defaults to {@link OctopusStudioErrorKind.External} so unexpected failures (network, permissions,
 * corrupted repos) surface in telemetry. Use {@link OctopusStudioErrorKind.Conflict} only when the
...
async function execOrThrow(
  ...
  path: string,
  errorMessage?: string,
  kind: OctopusStudioErrorKind = OctopusStudioErrorKind.External,
  options?: IGitStringExecutionOptions,
): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    ...
    throw new OctopusStudioError(error, kind);
  }
}
  ...
  if (writeGitattributes) {
    ...
    try {
      await fsPromises.writeFile(
        gitattributesPath,
        "# Normalize text files to LF so Octopus Studio commits are stable across platforms.\n* text=auto eol=lf\n",
        { flag: "wx" },
      );
 ...
 * the user was already editing must not have those edits folded into OctopusStudio's
 * commit, because `git commit -- <path>` records the whole working-tree version
 * of that path, not just the hunk OctopusStudio changed.
 *
...
export async function gitStageToRevert({
  ...
  const userVisibleChanges = statusResult.stdout
    ...
    // from a managed `.octopusStudio/` file to a user-visible destination must still
...
export async function getGitUncommittedFiles({
  ...
  return (
    result.stdout
      ...
      // non-ASCII paths are returned verbatim and `.octopusStudio/` filtering matches.
      .flatMap(getPorcelainPaths)
      .filter(isUserVisibleGitPath)
  );
}
 ...
 * user-visible (e.g. .octopusStudio/, pnpm-workspace.yaml) for parity with the rest of
 * the file.
 */
...
function assertAgentGitSuccess(
  ...
  message: string,
  kind: OctopusStudioErrorKind = OctopusStudioErrorKind.Conflict,
): void {
  if (result.exitCode !== 0) {
    throw new OctopusStudioError(
      `${message}: ${agentGitErrorDetails(result)}`,
      kind,
    );
  }
}
...
async function renderSafeAgentDiff({
  ...
  if (sensitive.length > 0) {
    notices.push(
      `[Diff omitted for sensitive or OctopusStudio-managed paths: ${[...new Set(sensitive)].join(", ")}]`,
    );
  }
  if (omittedByCount) {
    notices.push(
...
export async function restoreAgentGitFile({
  ...
  const temporaryDirectory = await fsPromises.mkdtemp(
    pathModule.join(parent, ".octopus-studio-git-restore-"),
  );
...
class CodedGitError extends OctopusStudioError {
...
class GitConflictErrorImpl extends OctopusStudioError {
  readonly code = GIT_ERROR_CODES.MERGE_CONFLICT;

  constructor(message: string) {
    super(message, OctopusStudioErrorKind.Conflict);
