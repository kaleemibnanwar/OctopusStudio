...
export function createOllamaProvider(
  ...
  const provider = createOpenAICompatible({
    ...
    fetch: options?.fetch,
    // Ollama only reports usage in streaming responses when the OpenAI-
    // compatible `stream_options.include_usage` flag is set. Without it the
    // final chunk has no `usage`, so `maxTokensUsed` stays null and the chat
    // token cap cannot show actual AI usage.
    includeUsage: true,
  });
