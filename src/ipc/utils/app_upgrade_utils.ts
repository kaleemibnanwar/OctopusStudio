...
export async function applyComponentTagger(
  ...
  if (pluginsIdx !== -1) {
    if (!content.includes("octopusStudioComponentTagger()")) {
      const bracketIdx = pluginsIdx + matchStr.indexOf("[");
      content =
        content.slice(0, bracketIdx) +
        "[octopusStudioComponentTagger(), " +
  ...
  if (installDependencies) {
    try {
      await simpleSpawnWithDeniedPnpmBuildSelfHeal({
        command: `pnpm ${PNPM_PM_ON_FAIL_IGNORE_ARG} add --ignore-workspace-root-check -D @octopus-studio-sh/react-vite-component-tagger`,
        cwd: appPath,
        successMessage:
      ...
      try {
        await simpleSpawn({
          command:
            "npm install --save-dev --legacy-peer-deps @octopus-studio-sh/react-vite-component-tagger",
          cwd: appPath,
          successMessage:
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "add OctopusStudio component tagger",
    });
