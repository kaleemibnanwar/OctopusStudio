...
import {
  ...
  transcribeWithOctopusStudioEngine,
} from "./llm_engine_provider";

describe("createOctopusStudioEngine", () => {
  test("uses Anthropic messages API for Anthropic engine models", async () => {
    ...
    await model.doGenerate({
      ...
      providerOptions: {
        "octopus-studio-engine": {
          octopusStudioAppId: 42,
          octopusStudioRequestId: "request-1",
          octopusStudioFiles: [{ path: "src/App.tsx", content: "export {}" }],
        },
      },
    ...
    expect(request.init?.headers).toMatchObject({
      authorization: "Bearer octopus-studio-pro-key",
      "X-OctopusStudio-Request-Id": "request-1:attempt-1",
    });
    ...
    expect(body).not.toHaveProperty("octopusStudioAppId");
    expect(body).not.toHaveProperty("octopusStudioRequestId");
    expect(body).not.toHaveProperty("octopusStudioFiles");
  ...
  test("sends stable free quota key separately from attempt request id", async () => {
    ...
    await model.doGenerate({
      ...
      providerOptions: {
        "octopus-studio-engine": {
          octopusStudioRequestId: "visible-turn-1",
        },
      },
    } satisfies LanguageModelV3CallOptions);
    ...
    expect(requests[0].init?.headers).toMatchObject({
      "X-OctopusStudio-Request-Id": "visible-turn-1:attempt-1",
      "X-OctopusStudio-Free-Quota-Key": "visible-turn-1",
    });
  });
});

describe("transcribeWithOctopusStudioEngine", () => {
  test("uses the OctopusStudio transcription model alias", async () => {
    ...
    expect(formData.get("model")).toBe("octopusStudio/transcribe");
  });
});
