...
// Gates whether OctopusStudio may auto-commit a file it rewrote: `git commit -- <path>`
...
describe("isGitPathClean", () => {
  ...
  async function makeRepo(): Promise<string> {
    ...
    await runGit(repoDir, ["init"]);
    await runGit(repoDir, ["config", "user.email", "test@octopusStudio.sh"]);
  ...
  // that would sweep every line the user wrote into OctopusStudio's commit.
...
describe("getGitUncommittedFiles", () => {
  ...
  it("ignores OctopusStudio-managed runtime files in native git status", async () => {
    repoDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "git-utils-"));

    await runGit(repoDir, ["init"]);
    await fs.promises.mkdir(path.join(repoDir, ".octopusStudio"), {
      recursive: true,
    });
    await fs.promises.writeFile(
    ...
    await fs.promises.writeFile(
      path.join(repoDir, ".octopusStudio", "screenshot.png"),
      "generated",
    );
  ...
  it("ignores OctopusStudio-managed runtime files in native status details", async () => {
    repoDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "git-utils-"));
    ...
    await fs.promises.writeFile(
      path.join(repoDir, ".octopusStudio", "foo [bar]"),
      "generated",
    );
    await fs.promises.writeFile(path.join(repoDir, "src.ts"), "user change");
  ...
  it("decodes git's octal-escaped non-ASCII paths in native git status", async () => {
    ...
    await fs.promises.mkdir(path.join(nextRepoDir, ".octopusStudio"), {
      recursive: true,
    });
    ...
    // user-visible file is reported and the `.octopusStudio/` one is still filtered out.
    await fs.promises.writeFile(
    ...
    await fs.promises.writeFile(
      path.join(nextRepoDir, ".octopusStudio", "naïve.png"),
      "generated",
    );
...
describe("gitStageToRevert", () => {
  ...
  it("ignores untracked OctopusStudio-managed runtime files", async () => {
    const repo = await createTwoVersionRepo();
    await fs.promises.mkdir(path.join(repo.repoDir, ".octopusStudio"), {
      recursive: true,
    });
    await fs.promises.writeFile(
      path.join(repo.repoDir, ".octopusStudio", "screenshot.png"),
      "generated",
    );
    await fs.promises.writeFile(
      path.join(repo.repoDir, ".octopusStudio", "foo [bar]"),
      "generated",
    );
    await fs.promises.writeFile(
