...
const CLEANUP_ONLY_BRANCH_PREFIX = "octopus-studio-cleanup-only:v1:";
...
async function appUsesNeonAuth(appData: AppRow): Promise<boolean> {
  ...
  try {
    const appPath = getOctopusStudioAppPath(appData.path);
 ...
 *
 * Throws `OctopusStudioError` if the app has no Neon project or no parent branch.
 */
export async function createTempTestBranch(
  ...
  const branchName = `octopus-studio-test-${appData.id}-${Date.now()}`;
...
export async function deleteTempTestBranch(appData: AppRow): Promise<boolean> {
  ...
  if (!projectId) {
    ...
    logger.error(
      `App ${appData.id} still tracks temporary Neon test branch ${branchId}, but the app is no longer linked to a Neon project; Octopus Studio cannot delete it and it must be removed manually.`,
    );
    return false;
  }
