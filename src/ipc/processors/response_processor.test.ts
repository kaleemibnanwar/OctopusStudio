...
vi.mock("../../paths/paths", () => ({
  getOctopusStudioAppPath: vi.fn((appPath: string) => `/mock/apps/${appPath}`),
}));
...
describe("processFullResponseActions add dependency errors", () => {
  ...
  it("writes a Supabase migration file for schema-mutating SQL", async () => {
    ...
    await processFullResponseActions(
      '<octopus-studio-execute-sql description="create users">CREATE TABLE users (id bigint);</octopus-studio-execute-sql>',
      1,
      {
  ...
  it("skips Supabase migration files for non-schema SQL", async () => {
    ...
    await processFullResponseActions(
      '<octopus-studio-execute-sql description="lookup users">SELECT * FROM users;</octopus-studio-execute-sql>',
      1,
      {
        chatSummary: undefined,
        messageId: 1,
  ...
  it("commits only dependency groups completed before a later failure", async () => {
    ...
    await processFullResponseActions(
      '<octopus-studio-add-dependency packages="react zod@999.0.0"></octopus-studio-add-dependency>',
      1,
      {
        chatSummary: undefined,
        messageId: 1,
  ...
  it("queues delete tags for cloud sync even when the local path is already missing", async () => {
    ...
    const result = await processFullResponseActions(
      `
      <octopus-studio-write path="src/file1.js">console.log("Hello");</octopus-studio-write>
      <octopus-studio-delete path="src/missing.js"></octopus-studio-delete>
      `,
      1,
      {
        chatSummary: undefined,
  ...
  it("queues the canonical physical delete path through an in-project alias", async () => {
    ...
    const result = await processFullResponseActions(
      `
      <octopus-studio-write path="src/file1.js">console.log("Hello");</octopus-studio-write>
      <octopus-studio-delete path="self/src/missing.js"></octopus-studio-delete>
      `,
      1,
      {
        chatSummary: undefined,
  ...
  it("delegates shared-affected and skipped direct Supabase functions to the shared deploy helper", async () => {
    ...
    const result = await processFullResponseActions(
      `
      <octopus-studio-write path="supabase/functions/_shared/foo.ts">export const foo = 1;</octopus-studio-write>
      <octopus-studio-write path="supabase/functions/beta/index.ts">Deno.serve(() => new Response("ok"));</octopus-studio-write>
      `,
      1,
      {
        chatSummary: undefined,
