...
import {
  ...
  getOctopusStudioExecuteSqlTags,
  getOctopusStudioSearchReplaceTags,
  getOctopusStudioCopyTags,
} from "../utils/octopus_studio_tag_parser";
const applySearchReplace = (c: string, ...args: any[]) => ({
  success: true,
  content: c,
  error: "",
});
...
function formatSkippedActionSummary(fullResponse: string): string {
  const counts: Array<[number, string, string]> = [
    [getOctopusStudioWriteTags(fullResponse).length, "write", "writes"],
    [getOctopusStudioRenameTags(fullResponse).length, "rename", "renames"],
    [getOctopusStudioDeleteTags(fullResponse).length, "delete", "deletes"],
    [
      getOctopusStudioAddDependencyTags(fullResponse).length,
      "dependency install",
      "dependency installs",
    ],
    [
      getOctopusStudioExecuteSqlTags(fullResponse).length,
      "SQL query",
      "SQL queries",
    ],
    [
      getOctopusStudioSearchReplaceTags(fullResponse).length,
      "search-replace",
      "search-replaces",
    ],
    [getOctopusStudioCopyTags(fullResponse).length, "copy", "copies"],
  ];
...
export async function dryRunSearchReplace({
  ...
  const octopusStudioSearchReplaceTags =
    getOctopusStudioSearchReplaceTags(fullResponse);
  for (const tag of octopusStudioSearchReplaceTags) {
...
export async function processFullResponseActions(
  ...
  const appPath = getOctopusStudioAppPath(chatWithApp.app.path);
  const octopusStudioDeletePaths = getOctopusStudioDeleteTags(fullResponse);
  ...
  try {
    ...
    for (const filePath of octopusStudioDeletePaths) {
      assertNotProjectRootPath(appPath, filePath);
    }
    preparedDeletePaths = await Promise.all(
      octopusStudioDeletePaths.map((filePath) =>
  ...
  try {
    // Extract all tags
    const octopusStudioWriteTags = getOctopusStudioWriteTags(fullResponse);
    const octopusStudioRenameTags = getOctopusStudioRenameTags(fullResponse);
    const octopusStudioAddDependencyPackages =
      getOctopusStudioAddDependencyTags(fullResponse);
    let installedOrUpdatedDependencyPackages: string[] = [];
    const hasDbProvider =
      chatWithApp.app.supabaseProjectId || chatWithApp.app.neonProjectId;
    const octopusStudioExecuteSqlQueries = hasDbProvider
      ? getOctopusStudioExecuteSqlTags(fullResponse)
      : [];
    ...
    if (octopusStudioExecuteSqlQueries.length > 0) {
      for (const query of octopusStudioExecuteSqlQueries) {
        try {
      ...
      logger.log(
        `Executed ${octopusStudioExecuteSqlQueries.length} SQL queries`,
      );
    }

    // TODO: Handle add dependency tags
    if (octopusStudioAddDependencyPackages.length > 0) {
      try {
        const addDependencyResult = await executeAddDependency({
          packages: octopusStudioAddDependencyPackages,
          message: message,
          appPath,
        });
        warningMessages.push(...addDependencyResult.warningMessages);
        installedOrUpdatedDependencyPackages =
          octopusStudioAddDependencyPackages;
      } catch (error) {
        if (error instanceof ExecuteAddDependencyError) {
          ...
          errors.push({
            message:
              error.completedPackages.length > 0
                ...
                : `Failed to add dependencies: ${octopusStudioAddDependencyPackages.join(", ")}. ${error.displaySummary}`,
            error: error.displayDetails,
          });
        } else {
          errors.push({
            message: `Failed to add dependencies: ${octopusStudioAddDependencyPackages.join(", ")}`,
            error: error,
          });
        }
      }
    ...
    for (const tag of octopusStudioRenameTags) {
    ...
    for (const tag of octopusStudioSearchReplaceTags) {
      ...
      try {
        if (!fs.existsSync(fullFilePath)) {
          // Do not show warning to user because we already attempt to do a <octopus-studio-write> tag to fix it.
        ...
        const result = applySearchReplace(original, tag.content);
        if (!result.success || typeof result.content !== "string") {
          // Do not show warning to user because we already attempt to do a <octopus-studio-write> and/or a subsequent <octopus-studio-search-replace> tag to fix it.
          logger.warn(
    ...
    const octopusStudioCopyTags = getOctopusStudioCopyTags(fullResponse);
    for (const tag of octopusStudioCopyTags) {
      try {
    ...
    for (const tag of octopusStudioWriteTags) {
      const filePath = tag.path;
    ...
    hasChanges =
      ...
      octopusStudioAddDependencyPackages.length > 0;
        ...
        if (uncommittedFiles.length > 0) {
          ...
          try {
            commitHash = await gitCommit({
              path: appPath,
              message:
                message + " + extra files edited outside of Octopus Studio",
              amend: true,
            });
            logger.log(
              `Amend commit with changes outside of Octopus Studio: ${uncommittedFiles.join(", ")}`,
            );
          } catch (error) {
            // Just log, but don't throw an error because the user can still
            // commit these changes outside of OctopusStudio if needed.
            logger.error(
              `Failed to commit changes outside of Octopus Studio: ${uncommittedFiles.join(", ")}`,
            );
    ...
    if (hasChanges) {
      queueCloudSandboxSnapshotSync({
        ...
        deletedPaths: [
          ...
          ...octopusStudioRenameTags.map((renameTag) => renameTag.from),
        ],
      });
    }
    ...
    ${warnings
      .map(
        (warning) =>
          `<octopus-studio-output type="warning" message="${escapeXmlAttr(warning.message)}">${escapeXmlContent(formatOutputError(warning.error))}</octopus-studio-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<octopus-studio-output type="error" message="${escapeXmlAttr(error.message)}">${escapeXmlContent(formatOutputError(error.error))}</octopus-studio-output>`,
      )
      .join("\n")}
    `;
