...
const templates: Template[] = [
  ...
  {
    id: "octopus-studio-sh/community-template",
  ...
  {
    id: "octopus-studio-sh/selected-community-template",
  ...
  it("keeps the current community template visible", () => {
    expect(
      getAppBlueprintTemplateOptions(
        templates,
        "octopus-studio-sh/selected-community-template",
      ).map((template) => template.id),
    ).toEqual(["react", "octopus-studio-sh/selected-community-template"]);
  });
});
