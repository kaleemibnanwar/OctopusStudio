...
// content string it received. The OctopusStudioMarkdownParser wraps ReactMarkdown
...
import { OctopusStudioMarkdownParser } from "./OctopusStudioMarkdownParser";

describe("OctopusStudioMarkdownParser octopus-studio-status", () => {
  afterEach(() => {
  ...
  it("honors explicit aborted state on closed status tags", () => {
    render(
      <OctopusStudioMarkdownParser
        content={
          '<octopus-studio-status title="Supabase functions failed" state="aborted">\n0 succeeded\n1 failed\n</octopus-studio-status>'
        }
      />,
    );
  ...
  it("renders incomplete type checks as amber warnings", () => {
    const { container } = render(
      <OctopusStudioMarkdownParser
        content={
          '<octopus-studio-status title="Type check incomplete" state="warning">Fix the configuration error, then rerun type checking.</octopus-studio-status>'
        }
      />,
    );
  ...
  it("keeps completed checks with source errors in the green finished state", () => {
    const { container } = render(
      <OctopusStudioMarkdownParser
        content={
          '<octopus-studio-status title="Type errors found" state="finished">Found 1 type error.</octopus-studio-status>'
        }
      />,
    );
...
describe("OctopusStudioMarkdownParser octopus-studio-command", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders the add TypeScript command as a supported action button", () => {
    render(
      <OctopusStudioMarkdownParser
        content={
          '<octopus-studio-command type="add-typescript"></octopus-studio-command>'
        }
      />,
    );
...
describe("OctopusStudioMarkdownParser octopus-studio-explore-chat-history", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it.each(["partial", "no_match"])(
    ...
    (outcome) => {
      render(
        <OctopusStudioMarkdownParser
          content={`<octopus-studio-explore-chat-history query="auth" chats="1" evidence="2" outcome="${outcome}">Historical evidence</octopus-studio-explore-chat-history>`}
        />,
      );

      expect(
        screen
          .getByTestId("octopus-studio-explore-chat-history")
...
describe("OctopusStudioMarkdownParser octopus-studio-git", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it.each([
    ...
    render(
      <OctopusStudioMarkdownParser
        content={`<octopus-studio-git operation="${operation}" revision="abc123" path="src/main.ts"></octopus-studio-git>`}
      />,
    );
  ...
  it("renders a pending state while the Git tag streams", () => {
    ...
    render(
      <Provider store={store}>
        <OctopusStudioMarkdownParser
          content={'<octopus-studio-git operation="status" state="pending">'}
        />
      </Provider>,
    );
  ...
  it("shows a useful status summary and expands structured file details", () => {
    ...
    render(
      <OctopusStudioMarkdownParser
        content={`<octopus-studio-git operation="status" branch="main" changed_count="2" untracked_count="1" conflicted_count="0" detail_format="status">${status}</octopus-studio-git>`}
      />,
    );
  ...
  it("does not render malformed status detail as structured content", () => {
    render(
      <OctopusStudioMarkdownParser
        content={
          '<octopus-studio-git operation="status" detail_format="status">{"branch":"main","detached":false,"staged":[{}],"unstaged":[],"untracked":[],"conflicted":[]}</octopus-studio-git>'
        }
      />,
    );
...
describe("OctopusStudioMarkdownParser octopus-studio-security-finding", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders a card with the title and severity level", () => {
    render(
      <OctopusStudioMarkdownParser
        content={
          '<octopus-studio-security-finding title="SQL Injection in User Lookup" level="critical">\nUser input is concatenated into a query.\n</octopus-studio-security-finding>'
        }
      />,
    );
  ...
  it("falls back gracefully when the level is missing or invalid", () => {
    render(
      <OctopusStudioMarkdownParser
        content={'<octopus-studio-security-finding title="Partial finding">'}
      />,
    );
...
describe("OctopusStudioMarkdownParser closed-block render counts", () => {
  beforeEach(() => {
  ...
  // Three markdown segments separated by two closed octopus-studio-status tags.
  ...
  const TAG1 =
    '<octopus-studio-status title="S1" state="finished">ok</octopus-studio-status>';
  const MD2 = "\n\nSecond paragraph content.\n\n";
  const TAG2 =
    '<octopus-studio-status title="S2" state="finished">ok</octopus-studio-status>';
  const MD3 = "\n\nThird paragraph content.";
  const FULL = MD1 + TAG1 + MD2 + TAG2 + MD3;

  it("renders each markdown block exactly once for a one-shot parse", () => {
    render(<OctopusStudioMarkdownParser content={FULL} />);
  ...
  it("does not re-render closed markdown blocks as later content streams in", () => {
    ...
    const { rerender } = render(
      <OctopusStudioMarkdownParser content={FULL.slice(0, md1ClosesAt)} />,
    );
    ...
    for (let i = md1ClosesAt + 1; i <= md2ClosesAt; i++) {
      rerender(<OctopusStudioMarkdownParser content={FULL.slice(0, i)} />);
    }
...
describe("OctopusStudioMarkdownParser parser-cache perf metrics", () => {
  ...
  function streamFully() {
    const { rerender } = render(
      <OctopusStudioMarkdownParser content={FULL.slice(0, 1)} />,
    );
