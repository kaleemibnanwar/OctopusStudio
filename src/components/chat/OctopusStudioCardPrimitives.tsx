...
/**
 * Accent color configuration for OctopusStudioCard components.
 * Maps to Tailwind color classes for border, background, and text.
 */
export type OctopusStudioAccentColor =
  | "blue"
  | "purple"
  | "violet"
  | "red"
...
const ACCENT_BORDER: Record<OctopusStudioAccentColor, string> = {
...
const ACCENT_ICON_BG: Record<OctopusStudioAccentColor, string> = {
...
const ACCENT_BADGE: Record<OctopusStudioAccentColor, string> = {
...
interface OctopusStudioCardProps {
  children: React.ReactNode;
  state?: CustomTagState;
  accentColor?: OctopusStudioAccentColor;
  showAccent?: boolean;
...
/**
 * Premium container for all OctopusStudio markdown action cards.
 ...
 */
export function OctopusStudioCard({
  children,
  state,
  accentColor = "blue",
  showAccent,
...
interface OctopusStudioCardHeaderProps {
...
/**
 * Header row for OctopusStudioCard. Contains a tinted icon circle and flexible content area.
 */
export function OctopusStudioCardHeader({
  icon,
  accentColor = "blue",
  children,
}: OctopusStudioCardHeaderProps) {
  return (
...
interface OctopusStudioBadgeProps {
  children: React.ReactNode;
  color?: OctopusStudioAccentColor;
}

/**
 * Small pill badge for labeling card types (e.g. "GREP", "Turbo Edit", "SQL").
 */
export function OctopusStudioBadge({
  children,
  color = "blue",
}: OctopusStudioBadgeProps) {
  return (
    <span
...
interface OctopusStudioExpandIconProps {
  isExpanded: boolean;
}
 ...
 */
export function OctopusStudioExpandIcon({
  isExpanded,
}: OctopusStudioExpandIconProps) {
  return (
    <ChevronRight
      size={16}
...
interface OctopusStudioStateIndicatorProps {
  state: CustomTagState;
  pendingLabel?: string;
  warningLabel?: string;
  abortedLabel?: string;
 ...
 */
export function OctopusStudioStateIndicator({
  state,
  pendingLabel,
  warningLabel,
  abortedLabel,
  errorLabel,
  finishedLabel,
}: OctopusStudioStateIndicatorProps) {
  if (state === "pending") {
    return (
 ...
 */
export function OctopusStudioFinishedIcon() {
  return (
...
interface OctopusStudioCardContentProps {
  isExpanded: boolean;
  children: React.ReactNode;
  className?: string;
}
 ...
 */
export function OctopusStudioCardContent({
  isExpanded,
  children,
  className = "",
}: OctopusStudioCardContentProps) {
...
interface OctopusStudioFilePathProps {
  path: string;
}
 ...
 */
export function OctopusStudioFilePath({ path }: OctopusStudioFilePathProps) {
  if (!path) return null;
  return (
...
interface OctopusStudioDescriptionProps {
  children: React.ReactNode;
}
 ...
 */
export function OctopusStudioDescription({
  children,
}: OctopusStudioDescriptionProps) {
  return (
