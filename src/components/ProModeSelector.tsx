...
import { hasOctopusStudioProKey, type UserSettings } from "@/lib/schemas";
...
export function ProModeSelector() {
  ...
  const toggleProEnabled = () => {
    updateSettings({
      enableOctopusStudioPro: !settings?.enableOctopusStudioPro,
    });
  };

  const hasProKey = settings ? hasOctopusStudioProKey(settings) : false;
  const proModeTogglable =
    hasProKey && Boolean(settings?.enableOctopusStudioPro);
  ...
  return (
    <Popover>
      <Tooltip>
        ...
        <TooltipContent>Configure OctopusStudio Pro settings</TooltipContent>
      </Tooltip>
      <PopoverContent className="w-80 border-primary/20">
        <div className="space-y-4">
          ...
          {!hasProKey && (
            <div className="text-sm text-center text-muted-foreground">
              <a
                ...
                onClick={() => {
                  ipc.system.openExternalUrl("https://octopusStudio.sh/pro#ai");
                }}
                title="Visit octopusStudio.sh/pro to unlock Pro features"
              >
                Unlock Pro modes
              </a>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <SelectorRow
              id="pro-enabled"
              label="Enable OctopusStudio Pro"
              tooltip="Uses OctopusStudio Pro AI credits for the main AI model and Pro modes."
              isTogglable={hasProKey}
              settingEnabled={Boolean(settings?.enableOctopusStudioPro)}
                ...
                <AccordionContent className="pb-3">
                  <div className="flex flex-col gap-5 pt-2">
                    <SelectorRow
                      id="web-search"
                      label="Web Access"
                      tooltip="Allows OctopusStudio to access the web (e.g. search for information)"
