...
export function SupabaseConnector({ appId }: { appId: number }) {
  ...
  // A octopusStudio://supabase-oauth-return processed with no active flow (cold
      ...
      try {
        if (settings?.isTestMode) {
          ...
          await ipc.system.openExternalUrl(
            "https://supabase-oauth.octopusStudio.sh/api/connect-supabase/login",
          );
        }
      } catch (error) {
  ...
  const handleUpdateApiKey = async () => {
    try {
      ...
      if (outcome === "switched") {
        ...
        // The key is still legacy and OctopusStudio couldn't act on it. Reporting
