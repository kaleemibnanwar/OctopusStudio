...
interface OctopusStudioAppMediaFolderProps {
  appName: string;
  appId: number;
  appPath: string;
  files: MediaFile[];
...
export function OctopusStudioAppMediaFolder({
  appName,
  appId,
  appPath,
  files,
  ...
  searchQuery,
}: OctopusStudioAppMediaFolderProps) {
  ...
  if (isOpen) {
    return (
      <>
        ...
        {previewFile && (
          <ImageLightbox
            imageUrl={buildOctopusStudioMediaUrl(appPath, previewFile.fileName)}
            alt={previewFile.fileName}
            filePath={`${appPath}/.octopusStudio/media/${previewFile.fileName}`}
