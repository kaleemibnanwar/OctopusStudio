...
type ApiKeyValidationDialogState = {
  ...
  errorKind?: OctopusStudioErrorKind;
};
...
function getErrorKind(error: unknown): OctopusStudioErrorKind | undefined {
  const kind =
  ...
  return typeof kind === "string" &&
    Object.values(OctopusStudioErrorKind).includes(
      kind as OctopusStudioErrorKind,
    )
    ? (kind as OctopusStudioErrorKind)
    : undefined;
}
...
function getApiKeyValidationDialogTitle(
  ...
  return dialog?.errorKind === OctopusStudioErrorKind.Auth
    ? "API key rejected"
...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  const isOctopusStudio = provider === "auto";
  ...
  // Use fetched data (or defaults for OctopusStudio)
  const providerDisplayName = isOctopusStudio
    ? "OctopusStudio"
    : (providerData?.name ?? "Unknown Provider");
  const providerWebsiteUrl = providerData?.websiteUrl;
  const hasFreeTier = isOctopusStudio ? false : providerData?.hasFreeTier;
  const envVarName = isOctopusStudio ? undefined : providerData?.envVarName;
  ...
  const handleSaveKey = async (
    ...
    try {
      if (shouldValidateApiKey && !options.skipValidation) {
        try {
          ...
          setApiKeyValidationDialog({
            message:
              error?.message ||
              `Octopus Studio could not verify this ${providerDisplayName} API key.`,
            apiKey: normalizedValue,
      ...
      // Check if this is the first time user is setting up OctopusStudio Pro
      const isNewOctopusStudioProSetup =
        isOctopusStudio && settings && !hasOctopusStudioProKey(settings);
      ...
      if (isOctopusStudio) {
        settingsUpdate.enableOctopusStudioPro = true;
        // Set default chat mode to local-agent when user upgrades to pro
        if (isNewOctopusStudioProSetup) {
      ...
      // Refetch user budget when OctopusStudio Pro key is saved
      if (isOctopusStudio) {
  ...
  // --- Toggle OctopusStudio Pro Handler ---
  const handleToggleOctopusStudioPro = async (enabled: boolean) => {
    setIsSaving(true);
    try {
      await updateSettings({
        enableOctopusStudioPro: enabled,
      });
    } catch (error: any) {
      showError(`Error toggling OctopusStudio Pro: ${error}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (!providerData && !isOctopusStudio) {
    return (
      <div className="min-h-screen px-8 py-4">
        <div className="max-w-4xl mx-auto">
          <BackButton />
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-green-800/80 dark:text-green-200/80">
                  You can now start building with Octopus Studio.
                </p>
              </div>
            </div>
      ...
      <div className="px-8 py-4">
        <div className="max-w-4xl mx-auto">
          <ProviderSettingsHeader
            ...
            isOctopusStudio={isOctopusStudio}
          ...
          {isOctopusStudio && !settingsLoading && (
            <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
              <div>
                <h3 className="font-medium">Enable OctopusStudio Pro</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Toggle to enable OctopusStudio Pro
                </p>
              </div>
              <Switch
                aria-label="Enable OctopusStudio Pro"
                checked={settings?.enableOctopusStudioPro}
                onCheckedChange={handleToggleOctopusStudioPro}
                disabled={isSaving}
              />
            </div>
          )}
