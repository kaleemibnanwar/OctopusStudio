...
interface OctopusStudioProTrialDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export function OctopusStudioProTrialDialog({
  isOpen,
  onClose,
  utmCampaign = "setup-dialog-v2",
}: OctopusStudioProTrialDialogProps) {
  const utmParams = `utm_source=octopus-studio-app&utm_medium=app&utm_campaign=${encodeURIComponent(utmCampaign)}`;

  const handleStartTrial = () => {
    ipc.system.openExternalUrl(
      `https://academy.octopusStudio.sh/redirect-to-checkout?trialCode=7PRO30&${utmParams}`,
    );
    onClose();
  };

  const handleLearnMore = () => {
    ipc.system.openExternalUrl(`https://www.octopusStudio.sh/pro?${utmParams}`);
  };
