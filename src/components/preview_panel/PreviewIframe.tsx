...
interface ErrorBannerProps {
  error:
    | {
        message: string;
        source: "preview-app" | "octopus-studio-app" | "octopus-studio-sync";
      }
    | undefined;
  onDismiss: () => void;
  onAIFix: () => void;
...
const ErrorBanner = ({ error, onDismiss, onAIFix }: ErrorBannerProps) => {
  ...
  const isInternalOctopusStudioError = error.source === "octopus-studio-app";
  const isSyncError = error.source === "octopus-studio-sync";
  ...
  return (
    ...
    >
      ...
      {(isInternalOctopusStudioError || isSyncError) && (
        <div className="absolute top-1 right-1 p-1 bg-red-100 dark:bg-red-900 rounded-md text-xs font-medium text-red-700 dark:text-red-300">
          {isSyncError ? "Cloud sync issue" : "Internal Octopus Studio error"}
        </div>
      )}
      ...
      <div
        className={cn(
          "px-6 py-1 text-sm",
          (isInternalOctopusStudioError || isSyncError) && "pt-6",
        )}
      >
        <div
        ...
        <div className="relative p-2 bg-red-100 dark:bg-red-900 rounded-sm flex gap-1 items-center">
          ...
          <span className="text-sm text-red-700 dark:text-red-200">
            ...
            {isDockerError
              ...
              : isSyncError
                ? "OctopusStudio could not upload your latest local changes to the cloud sandbox. Check your network connection or wait for sync to recover."
                : isInternalOctopusStudioError
                  ? "Try restarting the Octopus Studio app or restarting your computer to see if that fixes the error."
...
export const PreviewIframe = ({
  ...
  const handleRecordClick = () => {
    ...
    //
    // And only for a route the user picked through OctopusStudio's chrome: one the app
    ...
    recorder.requestStartRecording(
      iframeState.currentUrlSource === "octopus-studio"
  ...
  const requestAnnotatorScreenshot = () => {
    ...
    postPreviewMessage({ type: "octopus-studio-take-screenshot", requestId });
  };
      ...
      sendIframeEvent({
        ...
        message: cloudSandboxStatus.lastErrorMessage
          ? cloudSandboxStatus.lastErrorMessage.includes(
              "OctopusStudio stopped",
            )
            ? cloudSandboxStatus.lastErrorMessage
            : cloudSandboxStatus.terminationReason === "credits_exhausted"
              ? "This cloud sandbox was stopped because your OctopusStudio Pro credits ran out. Add credits and start it again."
              : "This cloud sandbox was stopped because OctopusStudio could not confirm billing. Please try starting it again."
          : cloudSandboxStatus.terminationReason === "credits_exhausted"
            ? "This cloud sandbox was stopped because your OctopusStudio Pro credits ran out. Add credits and start it again."
            : "This cloud sandbox was stopped because OctopusStudio could not confirm billing. Please try starting it again.",
        source: "octopus-studio-app",
      });
    }
    ...
    try {
      ...
      if (result.hasStaticText && iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          {
            type: "enable-octopus-studio-text-editing",
            data: {
  ...
  const getCurrentElementStyles = () => {
    ...
    try {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: "get-octopus-studio-component-styles",
          data: {
  ...
  useEffect(() => {
    if (iframeRef.current?.contentWindow && isComponentSelectorInitialized) {
      iframeRef.current.contentWindow.postMessage(
        { type: "octopus-studio-pro-mode", enabled: isProMode },
        "*",
      );
    }
  ...
  const handleComponentMessage = useCallback(
    (event: MessageEvent) => {
      ...
      if (
        event.data?.type === "octopus-studio-component-selector-initialized"
      ) {
        iframeRef.current?.contentWindow?.postMessage(
          { type: "octopus-studio-pro-mode", enabled: isProMode },
          "*",
      ...
      if (event.data?.type === "octopus-studio-text-updated") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "octopus-studio-text-finalized") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "octopus-studio-component-selected") {
      ...
      if (event.data?.type === "octopus-studio-component-deselected") {
        ...
        if (componentId) {
          ...
          if (iframeRef.current?.contentWindow) {
            iframeRef.current.contentWindow.postMessage(
              {
                type: "disable-octopus-studio-text-editing",
                data: { componentId },
              },
              "*",
            );
      ...
      if (event.data?.type === "octopus-studio-image-load-error") {
      ...
      if (event.data?.type === "octopus-studio-component-coordinates-updated") {
      ...
      if (event.data?.type === "octopus-studio-screenshot-response") {
        const requestId =
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="relative flex-grow overflow-hidden">
        ...
        <PreviewLoadingScreen
          ...
          hasStartupError={
            !loading && errorMessage?.source === "octopus-studio-app"
          }
        />
        {!loading && appUrl && (
          <div
      ...
      <div className="flex max-w-sm flex-col items-center gap-2.5 rounded-xl border border-border bg-(--background-lightest) px-6 py-5 shadow-lg">
        ...
        <p className="text-sm text-muted-foreground">
          {recorder.phase === "starting" || recorder.phase === "authenticating"
            ? "OctopusStudio is preparing an isolated environment for your recording — hold off on interacting with the preview until it's ready."
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (!data || data.type !== "octopus-studio-component-selected") {
    return null;
  }
