...
export function ChatList({
  ...
  onViewAllApps?: () => void;
  // When provided, list chats for this project instead of the selected app.
  appIdOverride?: number | null;
  // Header text to show when overriding the app list (e.g. "Chats").
  headerTitle?: string;
}) {
  ...
  const isOverride = appIdOverride !== undefined;
  const effectiveAppId = isOverride ? appIdOverride : selectedAppId;

  const { chats, loading, invalidateChats } = useChats(effectiveAppId);
  const { apps } = useLoadApps();
  const selectedApp = apps.find((app) => app.id === effectiveAppId);
  const effectiveAppName = headerTitle ?? selectedApp?.name;
  ...
  const handleNewChat = async () => {
    ...
    if (effectiveAppId) {
      try {
        ...
        const chatId = await ipc.chat.createChat({ appId: effectiveAppId });
        ...
        selectChat({ chatId, appId: effectiveAppId });
      } catch (error) {
        // DO A TOAST
  ...
  const handleDeleteChat = async (chatId: number) => {
    try {
      ...
      if (selectedChatId === chatId) {
        ...
        if (effectiveAppId) {
          if (isOverride) {
            // Projectless chats live in the default project; stay on its list.
            navigate({ to: "/chat", search: { appId: effectiveAppId } });
          } else {
            navigate({
              to: "/app-details",
              search: { appId: effectiveAppId },
            });
          }
        } else {
          navigate({ to: "/" });
  ...
  return (
    <>
      ...
      >
        {(showViewAllAppsButton || isOverride) && (
