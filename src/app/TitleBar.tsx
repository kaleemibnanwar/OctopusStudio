...
import { OctopusStudioProSuccessDialog } from "@/components/OctopusStudioProSuccessDialog";
...
export const TitleBar = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "octopus-studio-pro-return") {
        await refreshSettings();
        // Refetch user budget when OctopusStudio Pro key is set via deep link
  ...
  const isOctopusStudioPro = !!settings?.providerSettings?.auto?.apiKey?.value;
  const isOctopusStudioProEnabled = Boolean(settings?.enableOctopusStudioPro);
    ...
    <>
      <div className="@container z-11 w-full h-[calc(var(--layout-title-bar-offset)+1px)] pt-1 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <div className="flex items-center shrink-0">
          ...
          {isOctopusStudioPro && (
            <OctopusStudioProButton
              isOctopusStudioProEnabled={isOctopusStudioProEnabled}
            />
          )}
        </div>
...
export function OctopusStudioProButton({
  ...
  return (
    <Button
      data-testid="title-bar-octopus-studio-pro-button"
      onClick={() => {
        navigate({
    ...
    >
      {isOctopusStudioProEnabled
        ? userBudget?.isTrial
          ...
          : "Pro"
        : "Pro (off)"}
      {userBudget && isOctopusStudioProEnabled && (
