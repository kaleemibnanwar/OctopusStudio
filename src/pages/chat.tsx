...
import { ChatProjectPreview } from "../components/chat_project/ChatProjectPreview";
...
export default function ChatPage() {
  ...
  const [isChatProject, setIsChatProject] = useState<boolean | undefined>(
    undefined,
  );
  ...
  const chatProjectName =
    apps.find((app) => app.id === selectedAppId)?.name ?? undefined;
  ...
  }, [chatId, chats, loading, navigate, selectChat, selectedAppId]);
  ...
  // Chat projects have no codebase, so hide the app preview for them.
  useEffect(() => {
    if (!chatId) {
      setIsChatProject(false);
      return;
    }
    let cancelled = false;
    ipc.chat
      .getChat(chatId)
      .then((chat) => {
        if (!cancelled) setIsChatProject(chat.appType === "chat");
      })
      .catch(() => {
        if (!cancelled) setIsChatProject(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (isChatProject) {
      // Codeless projects show their docs/file preview pane instead of the app
      // preview, so it stays visible.
      ref.current?.expand();
