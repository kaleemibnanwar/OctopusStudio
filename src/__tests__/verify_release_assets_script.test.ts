...
function createFixture() {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "octopus-studio-release-verification-"),
  );
  temporaryDirectories.push(directory);

  const artifact = {
    name: "octopus-studio-1.10.0-beta.1.Setup.exe",
...
describe("release asset provenance verification", () => {
  ...
  it("rejects an uploaded asset whose sanitized name is absent from provenance", () => {
    const { assets, directory } = createFixture();
    assets[0].name = "octopus-studio-1.10.0-beta.1 Setup.exe";

    expect(() => verifyReleaseAssetProvenance(assets, directory)).toThrow(
      "Release asset octopus-studio-1.10.0-beta.1 Setup.exe does not match provenance",
    );
  });
