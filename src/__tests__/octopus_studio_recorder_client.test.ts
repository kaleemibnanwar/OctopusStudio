...
const recorderSource = fs.readFileSync(
  path.resolve(process.cwd(), "worker/octopus-studio-recorder-client.js"),
  "utf8",
);
...
function setup({ allowUntrusted = true }: { allowUntrusted?: boolean } = {}) {
  ...
  Object.defineProperty(doc, "currentScript", {
    configurable: true,
    value: {
      dataset: { octopusStudioRecorderToken: RECORDER_TOKEN },
  ...
  const parent = {
    postMessage: (msg: any) => {
      messages.push(msg);
      if (msg && msg.type === "octopus-studio-recorder-action")
  ...
  const win: any = {
    ...
    __OCTOPUS_STUDIO_RECORDER_ALLOW_UNTRUSTED__: allowUntrusted,
  ...
  if (!messages.some((m) => m.type === "octopus-studio-recorder-initialized")) {
  ...
  const activate = (token = RECORDER_TOKEN) =>
    messageHandler!({
      source: parent,
      data: { type: "activate-octopus-studio-recorder", token },
    });
  const deactivate = () =>
    messageHandler!({
      source: parent,
      data: {
        type: "deactivate-octopus-studio-recorder",
  ...
  const flush = (requestId: string) =>
    messageHandler!({
      source: parent,
      data: {
        type: "flush-octopus-studio-recorder",
        token: RECORDER_TOKEN,
        requestId,
      },
    });
  ...
  const overlays = () =>
    doc.querySelectorAll(".__octopus_studio_recorder_overlay__");
...
describe("octopus-studio recorder client", () => {
  ...
  it("takes the capability out of the DOM once it has read it", () => {
    ...
    expect(r.removedAttributes).toContain("data-octopus-studio-recorder-token");
  ...
  it("acknowledges a flush after actions already sent to the parent", () => {
    ...
    expect(r.messages.slice(-2)).toEqual([
      expect.objectContaining({ type: "octopus-studio-recorder-action" }),
      { type: "octopus-studio-recorder-flushed", requestId: "flush-1" },
    ]);
  });
  ...
  it("falls back to a CSS path rather than a data-octopus-studio-id", () => {
    const r = setup();
    // The attribute is a source location injected only by OctopusStudio's dev plugin, so
    // a locator built from it points at a moving target the replayed build
    // doesn't even carry.
    r.setHtml(
      `<div data-octopus-studio-id="src/App.tsx:12:4"><span></span></div>`,
    );
    r.activate();
    r.click(r.doc.querySelector("span"));
    r.settleClick();
  ...
  it("cannot be switched into accepting untrusted events after load", () => {
    ...
    const r = setup({ allowUntrusted: false });
    r.win.__OCTOPUS_STUDIO_RECORDER_ALLOW_UNTRUSTED__ = true;
    r.setHtml(`<button>Add</button>`);
    r.activate();
    r.click(r.doc.querySelector("button"));
    r.settleClick();
  ...
  it("removes the escape-hatch global once it has been read", () => {
    const r = setup();
    expect(r.win.__OCTOPUS_STUDIO_RECORDER_ALLOW_UNTRUSTED__).toBeUndefined();
  });
});
