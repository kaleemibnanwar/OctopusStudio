...
All models are routed through the OctopusStudio Engine gateway, so you only need one
credential: a OctopusStudio Pro API key, exposed as `OCTOPUS_STUDIO_PRO_API_KEY`.

The suite is skipped entirely when `OCTOPUS_STUDIO_PRO_API_KEY` is unset — no tests will
...
```bash
export OCTOPUS_STUDIO_PRO_API_KEY="..."
...
```bash
OCTOPUS_STUDIO_PRO_API_KEY="..." EVAL_SUITE=all EVAL_MODEL=all npm run eval
```

Optional: override the gateway URL with `OCTOPUS_STUDIO_ENGINE_URL` (defaults to
`https://engine.octopus-studio.sh/v1`).
...
EVAL_SUITE=all EVAL_MODEL=all OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval
```
...
EVAL_SUITE=search_replace EVAL_MODEL=all OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval

# The basic_agent suite (Basic agent prompt, search_replace + write_file)
EVAL_SUITE=basic_agent EVAL_MODEL=all OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval

# The pro_agent suite (Pro agent prompt, search_replace + write_file)
EVAL_SUITE=pro_agent EVAL_MODEL=all OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval
```
...
```bash
EVAL_SUITE=all EVAL_MODEL=all OCTOPUS_STUDIO_PRO_API_KEY="..." \
...
```bash
EVAL_SUITE=all EVAL_MODEL=all OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval -- -t "zod"
```
...
```bash
EVAL_SUITE=all EVAL_MODEL=sonnet OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval
```
...
EVAL_SUITE=search_replace EVAL_MODEL=sonnet \
  OCTOPUS_STUDIO_PRO_API_KEY="..." npm run eval -- -t "Extract a helper function"
```
