...
// because OctopusStudio's production picker does not currently surface GPT 5.4 —
// it had refusal/routing issues — but the eval harness still uses it as
// the judge model.
export const GPT_5_4 = "gpt-5.4";

// Single source of truth for the OctopusStudio Engine URL across the eval helpers.
export const OCTOPUS_STUDIO_ENGINE_URL =
...
export function hasOctopusStudioProKey(): boolean {
  return !!process.env.OCTOPUS_STUDIO_PRO_API_KEY;
}

let _provider: OctopusStudioEngineProvider | null = null;
 ...
 *
 * The OctopusStudio Engine only supports `stream: true`, but the AI SDK sends
...
/**
 * Fetch wrapper that adapts requests for the OctopusStudio Engine, which only supports
...
const evalFetch: typeof fetch = async (input, init) => {
  ...
  // Force streaming — the OctopusStudio Engine returns 500 for non-streaming requests
  parsed.stream = true;
...
function getProvider(): OctopusStudioEngineProvider {
  if (!_provider) {
    _provider = createOctopusStudioEngine({
      apiKey: process.env.OCTOPUS_STUDIO_PRO_API_KEY,
      baseURL: OCTOPUS_STUDIO_ENGINE_URL,
...
export function getEvalModel(
  ...
  const octopusStudioProvider = getProvider();
  const modelId = `${GATEWAY_PREFIXES[provider]}${modelName}`;

  // Always use the chat completions model (not .responses()) because:
  // 1. The OctopusStudio Engine only supports streaming for chat completions, and the
  ...
  return octopusStudioProvider(modelId, { providerId: provider });
}
