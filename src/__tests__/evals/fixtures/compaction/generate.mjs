 ...
 * session narrative phase-by-phase, materializes a OctopusStudio-format message list
 * (full <octopus-studio-write> contents, MCP tool tags), amplifies to the target
 ...
 *
 * Env: OCTOPUS_STUDIO_PRO_KEY or OCTOPUS_STUDIO_PRO_API_KEY (required), OCTOPUS_STUDIO_ENGINE_URL (optional),
...
const API_KEY =
  process.env.OCTOPUS_STUDIO_PRO_API_KEY || process.env.OCTOPUS_STUDIO_PRO_KEY;
const ENGINE_URL =
  process.env.OCTOPUS_STUDIO_ENGINE_URL || "https://engine.octopusStudio.sh/v1";
const MODEL = process.env.CMPGEN_MODEL || "gpt-5.6-sol";

if (!SPEC_PATH) fail("Missing --spec <path>");
if (!API_KEY)
  fail("Missing OCTOPUS_STUDIO_PRO_KEY / OCTOPUS_STUDIO_PRO_API_KEY in env");
...
function transformToolTags(content) {
  let result = content.replace(
    /<octopus-studio-mcp-tool-call\b[^>]*?\bserver="([^"]*)"[^>]*?\btool="([^"]*)"[^>]*>\n([\s\S]*?)\n<\/octopus-studio-mcp-tool-call>/g,
    '<tool-use name="$2" server="$1">\n$3\n</tool-use>',
  );
  result = result.replace(
    /<octopus-studio-mcp-tool-result\b[^>]*?\bserver="([^"]*)"[^>]*?\btool="([^"]*)"[^>]*>\n([\s\S]*?)\n<\/octopus-studio-mcp-tool-result>/g,
    (_m, server, tool, rc) => {
...
function toolCallXml(tool, args, result) {
  return (
    `<octopus-studio-mcp-tool-call server="octopus-studio" tool="${tool}">\n${JSON.stringify(args)}\n</octopus-studio-mcp-tool-call>\n` +
    `<octopus-studio-mcp-tool-result server="octopus-studio" tool="${tool}">\n${result}\n</octopus-studio-mcp-tool-result>`
  );
}

function octopusStudioWriteXml(p, description, content) {
  return `<octopus-studio-write path="${p}" description="${description}">\n${content}\n</octopus-studio-write>`;
}

// ---------------------------------------------------------------------------
// Phase authoring
// ---------------------------------------------------------------------------
const AUTHOR_SYSTEM = `You are authoring one phase of a realistic, long AI-pair-programming session transcript for OctopusStudio (an AI app builder). The session is between a USER (a real developer: terse, sometimes changes their mind, pastes errors) and an ASSISTANT (a coding agent that narrates briefly, calls tools, and writes full files).
