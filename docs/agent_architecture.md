...
- `src/pro/main/ipc/handlers/local_agent/tool_definitions.ts` contains the list of all the tools available to the OctopusStudio local agent.
...
Finally, you will need to define how to render the custom XML tag (e.g. `<octopus-studio-$foo-tool-name>`) inside `src/components/chat/OctopusStudioMarkdownParser.tsx` which will typically involve creating a new React component to render the custom XML tag.
